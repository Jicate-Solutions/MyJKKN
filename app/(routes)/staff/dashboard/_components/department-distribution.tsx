...
import { StaffDepartmentStats } from '@/types/staff';

interface DepartmentDistributionProps {
  data?: StaffDepartmentStats[];
  isLoading: boolean;
}
...
export function DepartmentDistribution({
  ...
  const chartData = useMemo(() => {
    ...
    return data.map((item, index) => ({
      ...
      uniqueKey: `department-${item.id}-${index}` // Ensure unique keys
    }));
  }, [data]);
      ...
      <CardHeader>
        <div className='flex items-center justify-between'>
          <div>
            ...
            <CardDescription>
              Facilitators distribution across {data.length} departments
            </CardDescription>
          </div>
      ...
      <CardContent>
        <Tabs defaultValue='bar' className='space-y-4'>
          <TabsList>
            ...
            <TabsTrigger value='list'>Top Departments</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value='list' className='space-y-4'>
            <div className='space-y-4'>
              {topDepartments.map((department, index) => (
                <div
                  key={`department-list-${department.id || index}`}
                    ...
                    <div>
                      ...
                      <div className='flex items-center gap-4 text-sm text-muted-foreground'>
                        <span className='flex items-center gap-1'>
                          ...
                          {department.staffCount.toLocaleString()} Facilitators
                        </span>
                        <span className='flex items-center gap-1'>
              ...
              {data.length > 5 && (
                <div className='text-center text-sm text-muted-foreground pt-4 border-t'>
                  And {data.length - 5} more departments
                </div>
              )}
            </div>
          </TabsContent>
