-- Admission Year → institution-wide, step 3/4: final schema.
-- v_learner_hostelites references program_start_year/program_end_year, so drop+recreate it
-- around the column drop/rename, deriving the cohort window from programs.program_duration_yrs.

DROP VIEW IF EXISTS v_learner_hostelites;

ALTER TABLE admission_years DROP CONSTRAINT IF EXISTS admission_years_unique_per_program;
ALTER TABLE admission_years DROP COLUMN IF EXISTS program_id;
ALTER TABLE admission_years DROP COLUMN IF EXISTS program_end_year;
ALTER TABLE admission_years DROP COLUMN IF EXISTS sanctioned_intake;
ALTER TABLE admission_years RENAME COLUMN program_start_year TO year;
ALTER TABLE admission_years
  ADD CONSTRAINT admission_years_institution_year_unique UNIQUE (institution_id, year);

-- Recreate the view with the new schema. Output columns are UNCHANGED (program_start_year
-- and program_end_year are now derived) so consumers are unaffected.
