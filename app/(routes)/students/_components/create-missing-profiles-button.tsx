  ...
  const handleCreateProfiles = async () => {
    try {
      ...
      if (data.results.successful > 0) {
        toast.success(
          `Successfully created ${data.results.successful} student user profiles!`
        );
      }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleDialogOpen}>
      ...
      <DialogContent className='max-w-2xl max-h-[80vh] overflow-y-auto'>
        <DialogHeader>
          <DialogTitle>Create Missing Student User Profiles</DialogTitle>
          <DialogDescription>
            {isChecking
              ? 'Checking current status...'
              : checkData
              ? `Found ${checkData.summary.without_profiles} students without profiles out of ${checkData.summary.total_students} total.`
              : "This will create user accounts and profiles for students who don't have them yet."}
            {!result &&
              !isChecking &&
              checkData &&
              checkData.summary.without_profiles > 0 &&
              ' Each student will receive a temporary password.'}
          </DialogDescription>
        </DialogHeader>
          ...
          <>
            {checkData.summary.without_profiles === 0 ? (
              <Alert className='border-green-200 bg-green-50'>
                ...
                <AlertDescription>
                  All {checkData.summary.total_students} students already have
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    <div className='space-y-2 mt-2'>
                      <div>
                        Total Students:{' '}
                        <strong>{checkData.summary.total_students}</strong>
                      </div>
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    <ul className='list-disc list-inside mt-2 space-y-1'>
                      <li>
                        Create authentication accounts for students without them
                      </li>
                      <li>
                        Create profiles for all{' '}
                        {checkData.summary.without_profiles} missing students
                      </li>
                      <li>Generate temporary passwords for new accounts</li>
                      <li>
                        Set all new accounts with &apos;student&apos; role
                      </li>
          ...
          <>
            <Alert>
              ...
              <AlertDescription>
                ...
                <ul className='list-disc list-inside mt-2 space-y-1'>
                  <li>
                    Create authentication accounts for students without profiles
                  </li>
                  <li>Generate temporary passwords for new accounts</li>
                  <li>Set all new accounts with &apos;student&apos; role</li>
                  <li>Link accounts to their respective institutions</li>
                </ul>
              </AlertDescription>
            </Alert>
