...
--
-- Date: 2026-09-12
-- Supersedes 20260908170000_leave_approval_org_scope.sql, which is the NEWEST
-- authored CREATE OR REPLACE of this function. CREATE OR REPLACE of the SAME
-- function, WIDENING the sixth source ('offer') to a second status, restarting
-- that branch's clock when an offer is issued, and changing nothing else: the
-- other five branches below are byte-for-byte the text of 20260908170000.
--
-- ⚠️ WHICH PREDECESSOR — this is not a detail, review round 1 found a silent
-- REGRESSION here (P4). The obvious predecessor by version number is
-- 20261018030000 (the migration that ADDED the offer branch), and the first
-- draft of this file was built on it. But version prefixes in this repo are a
-- SEQUENCE, not a date, and they do not match authorship order:
--
...
--
-- 20260908170000 is FIVE DAYS LATER despite the lower prefix, and it is itself a
-- CREATE OR REPLACE of fn_my_desk_waiting: it adds the per-applicant
-- `fn_hr_leave_scope_admits(a.employee_id, m.scope_role)` test to the LEAVE
-- branch, plus the `scope_role` aggregate that feeds it, wrapped in CASE rather
-- than ANDed so the per-row DEFINER call stays off rows that already failed the
-- cheap set-based tests. Its own header records why: an unguarded per-row DEFINER
-- predicate in exactly this area "previously timed the queue out for 94 HODs and
-- rendered 0 records".
--
-- Rebuilding this replace on 20261018030000's text would therefore have DROPPED
-- that scope test on apply — WIDENING which leave applications an approver sees,
-- and re-opening the timeout. A CREATE OR REPLACE reverts every later hardening
-- of the object it replaces unless it is built on the latest body (the same class
-- of failure as renaming an applied migration). This file is built on
-- 20260908170000's body verbatim, which is also the form mirrored into
-- supabase/setup/02_functions.sql, and
-- __tests__/lib/services/hr/my-desk-offer-branch-widening.test.ts now asserts the
-- scope test is present so the revert cannot come back silently.
--
-- WHAT CHANGED, AND WHY
--   Director decision, 2026-09-12, chosen from a tappable question: BUILD the
--   Issue Offer button. He rejected the alternative (retire the unused
--   'offer_issued' status).
--
--   That decision invalidates the premise 20261018030000 was written on. Its
--   header said the queue must not be named for 'offer_issued' because the
--   status had never been used and "nor is there anywhere to issue one: a grep
--   of app/ for 'offer_issued' finds status COLOURS and read-only groupings
--   and never a control." True when written. As of the pull request carrying
--   this migration there ARE two controls — one on the candidate detail page
--   and one in the job workspace's candidates tab — so the transition
--   package_fixed -> offer_issued can now actually be performed.
--
--   THE WIDENING IS WHAT KEEPS THE BUTTON FROM MAKING PEOPLE VANISH.
--   The branch selected status = 'package_fixed' only. Issuing an offer moves
--   the candidate to 'offer_issued', which no branch of this function
--   matched — so the very act the new button performs would have deleted the
--   person from every desk that was tracking them. Strictly worse than having
--   no button: the hire stops being anybody's item at the moment someone
--   finally acts on it. The branch now spans BOTH post-package statuses, and
--   `detail` says which one a row is in.
--
--   EXACTLY THREE EDITS inside the offer CTE, and nothing anywhere else:
--     1. WHERE  c.status = 'package_fixed'
--        ->     c.status IN ('package_fixed', 'offer_issued')
--     2. `detail` gains ONE leading WHEN — status = 'offer_issued' reads
--        'offer issued — waiting for them to join'. The three existing
--        package_fixed sentences are untouched and still the only thing a
--        package_fixed row can read.
--     3. waiting_since: c.submitted_at
--        ->             COALESCE(c.offer_issued_at, c.submitted_at)
--        Review round 1, P6. Widening the branch alone was not enough: the row
--        survived the click but kept reading "162 days" and climbing, with only
--        one sentence of `detail` changed. A desk that shows an un-moved number
--        after someone finally acted reads as "nothing happened". The two
--        columns COALESCE'd here are added by this same migration, above.
--        age_days is still floored at 0, and rows that reached offer_issued
--        before the control existed have a NULL stamp (no backfill) so they keep
--        exactly the age they show today.
--   The gate, the org scoping, the staff_record_id half of isPostApproval,
--   amount and the href CASE are all unchanged. In particular
--   the href still points at /hr/recruitment/approvals/<job_id> when the soft
--   job link is uuid-shaped and /hr/recruitment/candidates/<id> otherwise —
--   and BOTH of those pages now carry the Issue Offer control, which is why
--   the pull request had to add it twice. A button on one surface only would
--   have left the rows routed to the other still dead-ended.
--
--   The SOURCE VALUE stays 'offer'; the UI heading stays "Hires to bring on
--   board". Both remain correct for the widened set: a row at either status is
--   a hire who is not on board yet. The heading is still not named for the
--   transition, because the queue no longer holds only rows that need it.
--
-- Header of 20261018030000 follows.
--
-- Date: 2026-09-02
...
-- ---------------------------------------------------------------------------
-- 1. Record WHEN the offer went out and WHO sent it.
-- ---------------------------------------------------------------------------
-- Review round 1, P6. Before this, clicking Issue Offer wrote ONLY `status`, the
-- table had no column for either fact, and the offer branch below kept
-- `waiting_since = submitted_at` — so the Director's desk still read "162 days"
-- and climbing after HR acted, with one sentence of `detail` changed. That reads
-- as "nothing happened" and defeats the queue this control exists to serve.
--
-- Nullable, NO backfill, and deliberately so: every row that reached
-- 'offer_issued' before the control existed has no such moment to record, and
-- inventing one would put a false date on a real person's record. The offer CTE
-- reads COALESCE(offer_issued_at, submitted_at), so those rows keep exactly the
-- age they show today.
--
-- IF NOT EXISTS on both: this migration is a CREATE OR REPLACE that may be
-- re-run, and an ALTER that raises on the second run would block the replace.
ALTER TABLE public.hr_recruitment_candidates
  ADD COLUMN IF NOT EXISTS offer_issued_at timestamptz;

ALTER TABLE public.hr_recruitment_candidates
  ADD COLUMN IF NOT EXISTS offer_issued_by uuid REFERENCES public.profiles(id);

COMMENT ON COLUMN public.hr_recruitment_candidates.offer_issued_at IS
  'When the offer was issued (status -> offer_issued), written by RecruitmentService.updateStatus. NULL for rows that reached offer_issued before the Issue Offer control existed (2026-09-12) — not backfilled, because no such moment exists for them. fn_my_desk_waiting reads COALESCE(offer_issued_at, submitted_at) as waiting_since.';

COMMENT ON COLUMN public.hr_recruitment_candidates.offer_issued_by IS
  'Who issued the offer, resolved from the caller''s own session (never client-supplied). NULL for rows that predate the Issue Offer control.';
