...
/**
 * NotificationRulesSection — chrome-less editor body for the campus-living
 * notification-rules settings.
 *
 * Extracted (behavior-preserving) from
 * app/(routes)/campus-living/settings/notification-rules/page.tsx so the same
 * editor can be inlined inside the unified Campus Living config sections
 * (which cannot host a page that wraps its own ContentLayout). The standalone
 * page now renders <ContentLayout title="Notification Rules"><NotificationRulesSection /></ContentLayout>
 * and looks identical; this component owns all state/hooks/handlers and the
 * working JSX, but renders NO ContentLayout / PermissionGuard / chrome.
 *
 * History (from the original page):
