...
import type { JkknDepartmentUpstreamResponse } from '@/types/jkkn-api/departments';
 ...
 *
 * Secure server-side proxy for the JKKN departments endpoint.
 * JKKN_API_KEY is a server-only env var — never sent to the browser.
 * Accepts ?page, ?limit, ?search, ?institution_id, ?isActive.
 *
...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(
      `${JKKN_API_BASE_URL}/api-management/organizations/departments?${upstreamParams}`,
      {
        headers: {
