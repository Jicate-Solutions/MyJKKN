-- ============================================================================
-- Evidence mappings: the natural key must include institution_id
-- Created: 2026-08-04
-- STATUS: FILE ONLY — **NOT APPLIED**. See "HOW TO APPLY" at the foot.
-- ----------------------------------------------------------------------------
-- WHAT IS BROKEN
--
...
--
-- institution_id is NOT in that key. So one source row can be claimed by at
-- most ONE institution per body/metric/programme. The second college to map the
-- same shared artifact does not get a row of its own — its upsert lands on the
-- first college's row and overwrites it.
--
-- The whole point of a single walkable campus is that evidence is shared:
-- one guest lecture, one MoU, one anti-ragging committee, one sustainability
-- reading, counted by every college it actually serves. Today the schema
-- forbids saying so.
--
-- ----------------------------------------------------------------------------
-- WHAT IT COSTS TODAY — the cross-college distinction can never fire
--
-- public.fn_accreditation_evidence_scope(p_body_code, p_period_label) computes,
-- per source row:
--     count(DISTINCT institution_id) AS claiming_institutions
-- and then
--     shared_count = count(*) FILTER (WHERE claiming_institutions > 1)
--
-- Because the unique key stops any source from being claimed twice,
-- claiming_institutions is ALWAYS 1, so shared_count is ALWAYS 0 and
-- college_total ALWAYS equals cluster_total. Measured on production
-- 2026-08-04: every NIRF metric returns shared_count = 0, and
--   SELECT count(*) FROM (
--     SELECT source_table, source_id, body_code, metric_code, programme_id
--       FROM quality_evidence_mappings
--      GROUP BY 1,2,3,4,5 HAVING count(DISTINCT institution_id) > 1) x
-- returns 0 across all 11,608 rows. The shared-evidence readout is structurally
-- dead code, not an empty result.
--
-- ----------------------------------------------------------------------------
-- WHY THE CONSTRAINT AND THE 22 WRITERS ARE ONE ARTIFACT IN TWO PLACES
--
-- ***READ THIS BEFORE EDITING ANYTHING BELOW.***
--
-- That five-column constraint is the ON CONFLICT arbiter of 22 SECURITY
-- DEFINER writer functions (35 conflict clauses). PostgreSQL matches an
-- inferred ON CONFLICT target to a unique constraint EXACTLY: name one column
-- too few or too many and the statement raises
--     42P10  there is no unique or exclusion constraint matching the
--            ON CONFLICT specification
--
-- This has already happened here. programme_id was added to the constraint and
-- no writer followed; all 22 raised 42P10 and FIVE nightly routines
-- (copo-attainment, hr-naac-evidence, sustainability-naac-evidence,
-- event-feedback-naac-evidence, coe-result-naac-snapshots) failed every night
-- for weeks before migration 20260809000000 (PR #2807) rebuilt all 22 to name
-- the five columns. That fix reached production on 2026-08-04.
--
-- Changing the key WITHOUT moving all 22 writers in the same transaction
-- re-creates that outage exactly. So this migration does both, atomically:
--   1. drops the five-column constraint (name resolved dynamically from
--      pg_constraint by conrelid/contype/conkey — a hardcoded guess plus
--      IF EXISTS is a silent no-op that leaves the old key in place)
--   2. adds the six-column UNIQUE NULLS NOT DISTINCT key
--   3. rebuilds all 22 writers onto the six-column target
--   4. asserts every one of those four facts before COMMIT
--
-- Eight of the 22 are LIVE TRIGGERS on user-facing tables (grievance_tickets,
-- events, alumni_outcomes, service_requests, health_sports_achievements,
-- institution_collaborations, ss_grants, anti_ragging_affidavits). A mismatch
-- is not a background failure — it is a user action that errors.
--
-- ----------------------------------------------------------------------------
-- WHY IT IS SAFE ON EXISTING DATA
--
-- Widening a unique key is strictly LESS restrictive: every set of rows that
-- satisfied UNIQUE(a,b,c,d,e) also satisfies UNIQUE(a,b,c,d,e,f). Confirmed
-- rather than assumed, on production 2026-08-04:
--   total rows                                    11,608
--   rows with institution_id IS NULL                   0
--   duplicates on the SIX columns                      0   <- the new key holds
--   sources already claimed by >1 institution           0
-- The ADD CONSTRAINT cannot fail on current data.
--
-- Twelve of the 35 conflict clauses carry "institution_id = EXCLUDED.
-- institution_id" in their DO UPDATE SET. Once institution_id is part of the
-- arbiter, the matched row necessarily already holds that value, so those
-- assignments become no-ops. They are left in place deliberately: removing them
-- would break the byte-identity guarantee below for zero behavioural gain.
--
-- ----------------------------------------------------------------------------
-- HOW THIS FILE WAS PRODUCED — read before editing
--
-- Each function below is its LIVE production definition (pg_get_functiondef),
-- captured 2026-08-04, with ONE mechanical change: ', institution_id' inserted
-- before the closing paren of each ON CONFLICT target. The generator asserted,
-- per function, that (a) removing exactly those insertions reproduces the live
-- source byte for byte, (b) the length delta equals n * len(', institution_id'),
-- and (c) no five-column target survives. Nothing was re-authored from a repo
-- file — a CREATE OR REPLACE from a stale source has silently reverted a live
-- gate in this codebase before.
--
-- 22 functions - 35 conflict clauses.
--
-- ----------------------------------------------------------------------------
-- CODE HALF — deployable in EITHER order, no window
--
-- Two PostgREST writers name the arbiter as a string:
--   app/api/cron/coe-result-naac-snapshots/route.ts
--   lib/services/admission/admission-accreditation-report-service.ts
-- Both already set institution_id on every row they write.
--
...
--     satisfies the anti-join and the corrected claim is never emitted.
--
--   · the 12 "institution_id = EXCLUDED.institution_id" assignments were the
--     institution CORRECTION path. With institution_id in the arbiter, a
--     re-stamp (a learner transferring college, an audit_cycle's institution
--     edited, a CO/PO row moved between the CAS Aided/Self twins) no longer
--     UPDATEs the row in place — it INSERTs a second one and orphans the first.
--     count(DISTINCT institution_id) then reads 2, and shared_count reports
--     drift as sharing. That is a FABRICATED value for the exact metric this
--     migration exists to produce.
--
-- Note also that no writer shipped today ever emits a legitimate second claim
-- (each resolves exactly one institution per source), so until a writer or a
...
-- ----------------------------------------------------------------------------
-- 0. Capture the privilege census BEFORE anything is replaced, so ASSERT 6 can
--    prove this migration widened nothing. oid survives CREATE OR REPLACE, so
...
BEGIN
  ...
  ON CONFLICT (source_table, source_id, body_code, metric_code, programme_id, institution_id) DO UPDATE
    ...
    ELSE
      ...
      VALUES
        ('cdc_placement_outcome_cycles', v_cycle_id, r.institution_id, 'NAAC', '8.2.1',
         ...
         now())
      ON CONFLICT (source_table, source_id, body_code, metric_code, programme_id, institution_id)
      DO UPDATE SET
        period_label = EXCLUDED.period_label,
        metadata     = EXCLUDED.metadata,
        mapped_at    = now()
...
-- fn_induction_emit_naac_evidence(uuid)
...
-- fn_sync_audit_cycle_evidence(uuid)
...
-- fn_sync_bos_meeting_evidence(uuid)
...
-- fn_sync_cdc_drive_evidence(uuid)
...
-- fn_sync_cdc_training_evidence(uuid)
...
-- fn_sync_procurement_po_evidence(uuid)
...
-- fn_sync_stakeholder_survey_evidence(uuid)
...
-- change who may call them. Verified on production 2026-08-04: anon holds
...
-- only anon and PUBLIC are revoked. Revokes only.
--
...
-- ----------------------------------------------------------------------------
-- 4. Assertions. Nothing below is theatre — each one has failed in rehearsal.
-- ----------------------------------------------------------------------------
DO $assert$
DECLARE
  v_key       int;
...
BEGIN
  ...
  SELECT count(*) INTO v_key
   ...
   WHERE c.conrelid = 'public.quality_evidence_mappings'::regclass
     ...
     AND (SELECT array_agg(a.attname::text ORDER BY k.ord)
         ...
         = ARRAY['source_table','source_id','body_code','metric_code','programme_id','institution_id'];
  IF v_key <> 1 THEN
    RAISE EXCEPTION 'expected exactly ONE six-column unique key on quality_evidence_mappings, found %. Every ON CONFLICT rewritten here would raise 42P10.', v_key;
  END IF;

  -- ASSERT 2 — the five-column arbiter is GONE. If it survived, the drop
  -- silently no-op'd and both keys are live.
  SELECT count(*) INTO v_old_key
   ...
   WHERE c.conrelid = 'public.quality_evidence_mappings'::regclass
     ...
     AND (SELECT array_agg(a.attname::text ORDER BY k.ord)
         ...
         = ARRAY['source_table','source_id','body_code','metric_code','programme_id'];
  IF v_old_key <> 0 THEN
    RAISE EXCEPTION 'the five-column evidence key still exists (% found) - the DROP did not take', v_old_key;
  END IF;

  -- ASSERT 3 — NOT scoped to the name list on purpose. A hand-written census
  -- can only prove itself right if it already was; scoping the staleness check
  -- to the 22 names would let a 23rd writer added after the 2026-08-04 capture
  -- keep the five-column target, pass every assertion, and raise 42P10 on its
  ...
  -- 2026-08-04 none of the 22 held EXECUTE via a bare PUBLIC ("=X/owner") ACL
  -- entry and none had a null proacl, so revoking PUBLIC is a no-op — but that
  -- is a measurement, and this turns it into an enforced invariant. If a
  -- function DID depend on the PUBLIC grant, ASSERT 5 would still pass (it only
  -- looks at anon) and the migration would commit green while the caller starts
  -- failing 42501.
  SELECT count(*) INTO v_drift
    FROM _evidence_acl_before b
   WHERE b.auth_x IS DISTINCT FROM has_function_privilege('authenticated', b.oid, 'EXECUTE')
      OR b.svc_x  IS DISTINCT FROM has_function_privilege('service_role',  b.oid, 'EXECUTE');
  IF v_drift <> 0 THEN
    RAISE EXCEPTION 'this migration changed EXECUTE for authenticated/service_role on % function(s) - it must only revoke anon and PUBLIC', v_drift;
  END IF;
END
$assert$;
