...
export const INDUCTION_AI_ROUTINES: AIRoutine[] = [
  {
    ...
    "configKnobs": "MODEL=claude-sonnet-4-6, BATCH_CAP=25 (max cohorts/run), programs read cap=5000 rows, existing-playbook read cap=5000 rows, AI max_tokens=1024, AI timeout=60000ms, maxDuration=300s",
  ...
  },
  {
    ...
    "configKnobs": "MODEL=claude-sonnet-4-6, THRESHOLD=3.5 (avg below = weak), MIN_RESPONSES=3, EVENT_CAP=50 (events scanned/run), CANDIDATE_CAP=40 (weak topics tipped/run), programs read cap=5000 rows, AI max_tokens=900, AI timeout=60000ms, maxDuration=300s",
