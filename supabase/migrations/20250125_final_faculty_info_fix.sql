-- Final fix for faculty information retrieval in attendance reports
-- This migration ensures faculty_name and faculty_email are properly retrieved from attendance_data
...
BEGIN
    ...
    SELECT 
        ...
        -- ENHANCED: Priority order for faculty name
        COALESCE(
            -- First: Use faculty_name from attendance_data if present and not default
            NULLIF(TRIM(period_info.faculty_name::TEXT), ''),
            NULLIF(period_info.faculty_name::TEXT, 'Unknown Faculty'),
            -- Second: Try to get from marked_by profile
            (SELECT COALESCE(st.first_name || ' ' || st.last_name, pr.full_name)
             FROM profiles pr
             LEFT JOIN staff st ON (st.email = pr.email OR st.institution_email = pr.email) AND st.is_active = true
             WHERE pr.id = attendance_record.marked_by::uuid),
            -- Third: Try staff assignment lookup (legacy fallback)
        ...
        -- ENHANCED: Priority order for faculty email
        COALESCE(
            -- First: Use faculty_email from attendance_data if present
            NULLIF(TRIM(period_info.faculty_email::TEXT), ''),
            NULLIF(period_info.faculty_email::TEXT, 'N/A'),
            -- Second: Get email from marked_by profile
            (SELECT COALESCE(st.email, st.institution_email, pr.email)
             FROM profiles pr
             LEFT JOIN staff st ON (st.email = pr.email OR st.institution_email = pr.email) AND st.is_active = true
             WHERE pr.id = attendance_record.marked_by::uuid),
            -- Third: Try staff assignment lookup (legacy fallback)
    ...
    FROM (
        SELECT 
            ...
            -- Get faculty information directly from attendance_data (new records will have this)
            (value->>'faculty_name')::TEXT as faculty_name,
            (value->>'faculty_email')::TEXT as faculty_email,
...
'Retrieves detailed attendance information with enhanced faculty information resolution. Prioritizes faculty_name/faculty_email from attendance_data, falls back to marked_by profile lookup, then legacy staff assignment lookup.';
