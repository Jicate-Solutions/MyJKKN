-- 20260628020000_fix_scf_measure_baseline_doublecount.sql
-- 2026-06-28 — FIX: fn_scf_measure_suggestion_outcomes baseline double-count.
-- The prior recomputed_baseline CTE divided sum(understood) by sum(s_responses)
-- where s_responses was repeated once per joined feedback row, inflating the
-- denominator (e.g. three 2s -> 6/9 = 0.67 instead of 2.0), so a no-change
-- session reported a fabricated lift of +1.33. Replaced with round(avg(understood),2)
-- over sessions with >=3 responses in the window — symmetric with the outcome estimator.
-- Found by a 2-cycle moat simulation; already applied to prod via Mgmt-API on 2026-06-28.
