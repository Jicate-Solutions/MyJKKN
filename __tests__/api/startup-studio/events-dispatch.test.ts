/**
 * Startup Studio event routes — dispatch table.
 *
 * 15 dynamic route files under app/api/startup-studio/events/[id]/** folded
 * into one optional catch-all to get 28 routes back under Vercel's 2048 cap.
 * The fold is only safe if every URL that worked before still reaches the same
 * handler with the same methods, so that is what this file pins down.
 *
 * The table is imported on its own — no handler module is loaded — so these
 * assertions need neither Supabase nor a Next.js request.
 */

import { existsSync } from 'fs';
import path from 'path';

import { NextRequest } from 'next/server';
import { describe, it, expect } from 'vitest';

import {
  DELETE as routeDELETE,
  GET as routeGET,
  OPTIONS as routeOPTIONS,
} from '@/app/api/startup-studio/events/[id]/[[...slug]]/route';
import {
  ROUTE_TABLE,
  allowHeader,
  matchRoute,
  type HttpMethod,
} from '@/lib/api/startup-studio/events/dispatch';

const EVENT_ID = '3f1c8a42-9b7d-4e15-a0c6-2d8e5f4b1a90';
const VENUE_ID = 'c47d2e18-6a35-4f90-b1e7-8c0d3a9f5b62';

/**
 * Every path the 15 original route files served, as the segments that follow
 * /api/startup-studio/events/<id>, with the methods each file exported.
 */
const ORIGINAL_ROUTES: Array<{
  file: string;
  slug: string[];
  key: string;
  methods: HttpMethod[];
  params?: Record<string, string>;
}> = [
  { file: '[id]/route.ts', slug: [], key: 'event', methods: ['GET', 'PATCH', 'DELETE'] },
  { file: '[id]/checklists/route.ts', slug: ['checklists'], key: 'checklists', methods: ['GET', 'POST'] },
  { file: '[id]/demo-day/route.ts', slug: ['demo-day'], key: 'demo-day', methods: ['GET', 'POST'] },
  {
    file: '[id]/export/verifications/route.ts',
    slug: ['export', 'verifications'],
    key: 'export-verifications',
    methods: ['GET'],
  },
  { file: '[id]/my-assignment/route.ts', slug: ['my-assignment'], key: 'my-assignment', methods: ['GET'] },
  { file: '[id]/my-team/route.ts', slug: ['my-team'], key: 'my-team', methods: ['GET'] },
  { file: '[id]/register/route.ts', slug: ['register'], key: 'register', methods: ['POST'] },
  { file: '[id]/registrations/route.ts', slug: ['registrations'], key: 'registrations', methods: ['GET', 'POST'] },
  { file: '[id]/submit/route.ts', slug: ['submit'], key: 'submit', methods: ['POST'] },
  { file: '[id]/venues/route.ts', slug: ['venues'], key: 'venues', methods: ['GET', 'POST'] },
  {
    file: '[id]/venues/auto-allocate/route.ts',
    slug: ['venues', 'auto-allocate'],
    key: 'venues-auto-allocate',
    methods: ['POST'],
  },
  { file: '[id]/venues/mentors/route.ts', slug: ['venues', 'mentors'], key: 'venues-mentors', methods: ['GET'] },
  {
    file: '[id]/venues/[venueId]/route.ts',
    slug: ['venues', VENUE_ID],
    key: 'venue',
    methods: ['PATCH', 'DELETE'],
    params: { venueId: VENUE_ID },
  },
  {
    file: '[id]/venues/[venueId]/staff/route.ts',
    slug: ['venues', VENUE_ID, 'staff'],
    key: 'venue-staff',
    methods: ['POST', 'DELETE'],
    params: { venueId: VENUE_ID },
  },
  {
    file: '[id]/venues/[venueId]/teams/route.ts',
    slug: ['venues', VENUE_ID, 'teams'],
    key: 'venue-teams',
    methods: ['POST', 'DELETE'],
    params: { venueId: VENUE_ID },
  },
];

const ALL_METHODS: HttpMethod[] = ['GET', 'POST', 'PATCH', 'DELETE'];

describe('startup studio event route table', () => {
  it('covers exactly the 15 routes that used to be separate files', () => {
    expect(ROUTE_TABLE).toHaveLength(ORIGINAL_ROUTES.length);
    expect([...ROUTE_TABLE].map((e) => e.key).sort()).toEqual(
      ORIGINAL_ROUTES.map((r) => r.key).sort(),
    );
  });

  it('uses a unique key per entry', () => {
    const keys = ROUTE_TABLE.map((e) => e.key);
    expect(new Set(keys).size).toBe(keys.length);
  });

  it('ships a handler module for every entry', () => {
    const handlerDir = path.join(
      process.cwd(),
      'lib/api/startup-studio/events/handlers',
    );
    for (const entry of ROUTE_TABLE) {
      expect(
        existsSync(path.join(handlerDir, `${entry.key}.ts`)),
        `missing handler module for ${entry.key}`,
      ).toBe(true);
    }
  });
});

describe('every original path reaches its original handler', () => {
  for (const route of ORIGINAL_ROUTES) {
    it(`${route.file} -> /${route.slug.join('/')}`, () => {
      const matched = matchRoute(route.slug);
      expect(matched, `no table entry matched ${route.file}`).not.toBeNull();
      expect(matched!.entry.key).toBe(route.key);
      expect(matched!.params).toEqual(route.params ?? {});
    });
  }
});

describe('every original path x method pair is allowed', () => {
  for (const route of ORIGINAL_ROUTES) {
    for (const method of route.methods) {
      it(`${method} /${route.slug.join('/')} -> ${route.key}`, () => {
        const matched = matchRoute(route.slug);
        expect(matched!.entry.key).toBe(route.key);
        expect(matched!.entry.methods).toContain(method);
      });
    }
  }
});

describe('methods the original file never exported are rejected', () => {
  for (const route of ORIGINAL_ROUTES) {
    const absent = ALL_METHODS.filter((m) => !route.methods.includes(m));
    for (const method of absent) {
      it(`${method} /${route.slug.join('/')} is a 405, not a silent 200`, () => {
        const matched = matchRoute(route.slug);
        expect(matched!.entry.methods).not.toContain(method);
        // The route turns this into a 405 and advertises what it does take.
        expect(allowHeader(matched!.entry)).toContain('OPTIONS');
        expect(allowHeader(matched!.entry)).toContain(route.methods[0]);
      });
    }
  }
});

describe('literal segments beat the :venueId parameter', () => {
  it('venues/auto-allocate is not treated as a venue id', () => {
    const matched = matchRoute(['venues', 'auto-allocate']);
    expect(matched!.entry.key).toBe('venues-auto-allocate');
    expect(matched!.entry.key).not.toBe('venue');
    expect(matched!.params).toEqual({});
  });

  it('venues/mentors is not treated as a venue id', () => {
    const matched = matchRoute(['venues', 'mentors']);
    expect(matched!.entry.key).toBe('venues-mentors');
    expect(matched!.entry.key).not.toBe('venue');
    expect(matched!.params).toEqual({});
  });

  it('a real venue id still lands on the venue handler', () => {
    const matched = matchRoute(['venues', VENUE_ID]);
    expect(matched!.entry.key).toBe('venue');
    expect(matched!.params).toEqual({ venueId: VENUE_ID });
  });

  it('orders every literal row above the parameter row of the same length', () => {
    for (let i = 0; i < ROUTE_TABLE.length; i += 1) {
      const outer = ROUTE_TABLE[i];
      const hasParam = outer.segments.some((s) => s.startsWith(':'));
      if (!hasParam) continue;

      // Anything below a parameterised row must not be a literal row that the
      // parameter would shadow.
      for (let j = i + 1; j < ROUTE_TABLE.length; j += 1) {
        const inner = ROUTE_TABLE[j];
        if (inner.segments.length !== outer.segments.length) continue;
        const shadowed = inner.segments.every(
          (seg, k) => outer.segments[k].startsWith(':') || outer.segments[k] === seg,
        );
        expect(
          shadowed,
          `${inner.key} sits below ${outer.key} and would never be reached`,
        ).toBe(false);
      }
    }
  });
});

describe('paths that never existed are rejected', () => {
  const unknown: string[][] = [
    ['nonsense'],
    ['export'],
    ['venues', VENUE_ID, 'nonsense'],
    ['venues', VENUE_ID, 'staff', 'extra'],
    ['checklists', 'extra'],
    ['my-team', VENUE_ID],
  ];

  for (const slug of unknown) {
    it(`/${slug.join('/')} has no handler`, () => {
      expect(matchRoute(slug)).toBeNull();
    });
  }

  it('an empty segment cannot stand in for a venue id', () => {
    expect(matchRoute(['venues', ''])).toBeNull();
  });
});

describe('the allow header advertises the real methods', () => {
  it('lists the event handler methods plus OPTIONS', () => {
    const matched = matchRoute([]);
    expect(allowHeader(matched!.entry)).toBe('GET, PATCH, DELETE, OPTIONS');
  });

  it('lists both venue team-member methods plus OPTIONS', () => {
    const matched = matchRoute(['venues', VENUE_ID, 'staff']);
    expect(allowHeader(matched!.entry)).toBe('POST, DELETE, OPTIONS');
  });
});

describe('OPTIONS provenance is recorded per entry', () => {
  it('marks export-verifications as the one without its own OPTIONS export', () => {
    const withoutOptions = ROUTE_TABLE.filter((e) => !e.hasOptions).map((e) => e.key);
    expect(withoutOptions).toEqual(['export-verifications']);
  });
});

describe('the folded folder holds exactly one route file', () => {
  it('has the catch-all and nothing else under [id]', () => {
    const base = path.join(process.cwd(), 'app/api/startup-studio/events/[id]');
    expect(existsSync(path.join(base, '[[...slug]]/route.ts'))).toBe(true);
    expect(existsSync(path.join(base, 'route.ts'))).toBe(false);
    expect(existsSync(path.join(base, 'venues/route.ts'))).toBe(false);
    expect(existsSync(path.join(base, 'venues/[venueId]/route.ts'))).toBe(false);
  });
});

/**
 * The reject paths run against the real exported handlers. They return before
 * any handler module is loaded, so this needs no Supabase and no environment.
 */
describe('the catch-all route rejects what it should', () => {
  const request = (slug: string[]) =>
    new NextRequest(
      new URL(
        `https://jkkn.test/api/startup-studio/events/${EVENT_ID}/${slug.join('/')}`,
      ),
    );
  const context = (slug: string[]) => ({
    params: Promise.resolve({ id: EVENT_ID, slug }),
  });

  it('answers an unknown path with a 404 envelope', async () => {
    const slug = ['nonsense'];
    const res = await routeGET(request(slug), context(slug));
    expect(res.status).toBe(404);
    expect(await res.json()).toEqual({ success: false, error: 'Not found' });
  });

  it('answers a known path with an unsupported method with a 405 and an Allow header', async () => {
    const slug = ['my-team'];
    const res = await routeDELETE(request(slug), context(slug));
    expect(res.status).toBe(405);
    expect(res.headers.get('Allow')).toBe('GET, OPTIONS');
  });

  it('answers OPTIONS on an unknown path with a 404 rather than a preflight', async () => {
    const slug = ['nonsense'];
    const res = await routeOPTIONS(request(slug), context(slug));
    expect(res.status).toBe(404);
  });

  it('synthesises the preflight Next.js used to generate for export/verifications', async () => {
    const slug = ['export', 'verifications'];
    const res = await routeOPTIONS(request(slug), context(slug));
    expect(res.status).toBe(204);
    expect(res.headers.get('Allow')).toBe('GET, OPTIONS');
  });
});
