-- Learner-scoped, block-scoped allocatable-rooms resolver for the manual
-- AllocateRoomDialog (/campus-living/residents). Mirrors fn_cl_admin_room_options'
-- proven physical+category predicates (student rooms, gender, institution-serving,
-- cohort eligibility, free beds) but scoped to one block and across ALL of the
-- learner's eligible room categories (fail-open on category when none configured,
-- like the dialog did before — the physical conditions still apply).
