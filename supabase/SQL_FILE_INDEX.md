...
- **2026-05-11** — Admission leads RLS asymmetry + primary-role strict counselor (BUG-003934, BUG-003933, BUG-003932, BUG-003928)
  - Migration: `20260511000000_admission_leads_close_rls_asymmetry_and_primary_role_strict_check.sql`. Applied via Supabase MCP. Verified post-state: `_user_is_strict_counselor` returns TRUE for primary-counselor users (Tamilselvi, primary=staff_counselor) and FALSE for multi-role executives (COO, primary=hr_admin + secondary admission_counselor). Backfill UPDATE filled `assigned_counselor_id` on ~3,945 leads from the legacy `counselor_id → admission_counselors.user_id` chain — post-state shows 0 counselor-only leads remain (was 3,945; "both" jumped from 7,793 to 11,738 ≈ matches).
  - **Root cause #1 (asymmetry):** `lib/api-helpers/admission-counselor-scope.ts:111-114` granted list visibility via `counselor_id.eq.<myAdmissionCounselorId> OR assigned_counselor_id.eq.<userId>`, but the detail-page RLS only checked `assigned_counselor_id`. Legacy bulk-assigns wrote `counselor_id` only and left `assigned_counselor_id` NULL — list shows the lead, detail page returns PGRST116 "0 rows" → "Lead not found." Closed via new STABLE SECURITY DEFINER helper `_user_owns_lead_via_counselor_id(uid, counselor_id)` that mirrors the API helper exactly, plus an OR-branch in `adm_leads_select` / `adm_leads_update`.
  - **Root cause #2 (strict-counselor over-classification):** the helper checked any-row counselor membership without `is_primary`. The COO holds `hr_admin` (primary), `coo`, and `admission_counselor` (legacy secondary) — was being flagged strict and locked out of his own institution's unassigned leads despite holding `admission.leads.view`. Rewritten to require the counselor key be `is_primary=true`. Override role-keys (`admission`, `administrator`) still suppress strict-counselor regardless of primary, preserving prior exclusion behavior.
  - `02_functions.sql`: appended 3 functions — `_user_owns_lead_via_counselor_id` (new), `_user_is_strict_counselor` (rewritten), `_user_can_view_lead_for_call` (rewritten to OR-both-columns).
  - `lib/api-helpers/admission-counselor-scope.ts`: TS mirror — query now selects `is_primary` from `user_roles` and only flags strict when a counselor row has `is_primary=true`. Keeps list-side and RLS-side classification in lockstep.
  - `lib/services/admission/lead-service.ts`: `getLead()` switched from `.single()` to `.maybeSingle()` so legitimate "0 rows" outcomes (RLS-denied or genuinely missing) no longer log `[ERROR]` — the calling page already handles the thrown `Lead not found` cleanly. Pure log-noise reduction; UX unchanged.
  - **No canonical `supabase/setup/03_policies.sql` entry needed** — `adm_leads_select` / `adm_leads_update` are migration-only (the setup file only carries `admission_lead_source_captures` policies). Migration body is the source of truth for those policies.
