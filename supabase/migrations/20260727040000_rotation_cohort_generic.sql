...
--   RAISEs a NOTICE and skips, and the engine still works via the identity
--   fallback described below. Section 2 is idempotent — if this somehow lands
--   first, re-running JUST that DO block afterwards attaches the FKs.
--
--   VERIFIED 2026-07-26 on prod kvizhngldtiuufknvehv: Lane A then Lane D applied
--   inside one BEGIN..ROLLBACK — 35/35 checks passed, zero residue. The FKs
--   attached on the first pass and rejected an unconfigured cohort_key.
--
...
--      learner_role_key}. Reads the Lane-A config table when present; otherwise
--      falls back to the IDENTITY map (key → key), which for 'mba_associate'
--      reproduces the old hardcoding exactly.
...
--     lost on rotate-out), for ALL previous members including leavers.
...
COMMENT ON COLUMN public.mba_teams.cohort_key IS
  'Which teaching-enterprise cohort this team belongs to (teaching_enterprise_cohorts.cohort_key). Defaults to ''mba_associate'' so pre-Phase-3 rows and any un-migrated writer keep the original behaviour. Drives which learner role fn_mba_rotation_apply requires of the team''s members.';

COMMENT ON COLUMN public.mba_rotation_cycles.cohort_key IS
  'Which teaching-enterprise cohort this rotation cycle serves. fn_mba_rotation_generate round-robins ONLY the active teams whose cohort_key matches — that is the cross-cohort isolation guarantee. Defaults to ''mba_associate''.';
...
--
--    Reads teaching_enterprise_cohorts when Lane A has landed (checked through
--    pg_attribute, which is privilege-independent, and read through dynamic SQL
--    so this function CREATEs and RUNs cleanly while the table does not exist).
--    Otherwise returns the identity map — which for 'mba_associate' is exactly
--    the literal the Part-3 engine hardcoded, so MBA behaviour is unchanged
--    whether or not Lane A is applied.
--
...
BEGIN
  ...
  -- Identity fallback (config table absent, or no row for this key).
  RETURN QUERY
  SELECT v_key,
         CASE WHEN v_key = 'mba_associate' THEN 'MBA Associate' ELSE v_key END,
         v_key;
END;
$fn$;

COMMENT ON FUNCTION public.fn_teaching_cohort_resolve(text) IS
  'Resolves a teaching-enterprise cohort_key to {display_name, learner_role_key}. Reads teaching_enterprise_cohorts when that config table is present; otherwise identity-maps the key (which reproduces the pre-Phase-3 ''mba_associate'' hardcoding exactly). Internal helper for the rotation engine and the cohort pickers.';
...
--    Always includes the MBA cohort and any key already in use on a team or a
--    cycle, so a picker can never drop an existing value; adds every active
--    configured cohort once Lane A has landed. Authentication-gated only (this
--    returns labels, not data) — matching the Part-3 rotation tables' own read
...
COMMENT ON FUNCTION public.fn_teaching_cohort_options() IS
  'Authenticated-only list of teaching-enterprise cohorts for UI pickers: {cohort_key, display_name}. Union of the active configured cohorts (once teaching_enterprise_cohorts exists) with the MBA cohort and any key already in use on a team or a rotation cycle.';
...
BEGIN
  ...
  SELECT r.learner_role_key INTO v_role
  FROM fn_teaching_cohort_resolve(p_cohort_key) r;

  IF v_role IS NULL THEN
    RETURN;                                    -- unknown / blank cohort
  END IF;
...
COMMENT ON FUNCTION public.fn_teaching_cohort_list_learners(text) IS
  'Manager-gated (improvement.board.manage) picker source: the learners holding a cohort''s learner role. Cohort-generic sibling of fn_mba_list_associates(); fn_teaching_cohort_list_learners(''mba_associate'') returns exactly what that function returns.';
...
--    ONLY change vs 20260725120000: the round-robin participant set is now the
--    cycle's OWN cohort (`t.cohort_key = v_cohort`) instead of every active
--    team, plus the COALESCE-hardened permission guard. Blackout jumping, the
--    ((team + period) mod departments) placement, the delete-then-insert
--    rebuild and the return shape are byte-for-byte unchanged.
-- ============================================================================
...
COMMENT ON FUNCTION public.fn_mba_rotation_generate(uuid, integer) IS
  'Manager-gated (improvement.board.manage) rebuild of a cycle''s rota grid. Round-robins the active mba_teams OF THE CYCLE''S OWN cohort across the cycle''s departments, skipping blackout windows. Returns {periods, teams, departments, slots_created}.';
...
--    ONLY change vs 20260725120000: the "still holds the learner role" test is
--    now per-cohort (each team's own cohort_key → learner_role_key) instead of
--    the 'mba_associate' literal. Everything else is preserved exactly:
...
--      · the previous period's postings are still expired (need-to-know)
--      · blackout gaps still auto-pause; past-the-end still completes the cycle
--      · still idempotent, still service_role ONLY
-- ============================================================================
...
BEGIN
  ...
  -- The _cohort_roles join cannot drop a row: it is built from the same cycle's
  -- slots and cohort_key is NOT NULL.
