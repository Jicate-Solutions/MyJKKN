...
import { Loader2, CalendarIcon, ClockIcon, MapPinIcon, BuildingIcon, Users2Icon, FileTextIcon, CheckIcon, XIcon } from 'lucide-react';
...
export default function ReservationDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchReservation = async () => {
      setLoading(true);
      try {
        const data = await ReservationService.getReservation(params.id);
        setReservation(data);
      } catch (error) {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const updatedReservation = await ReservationService.getReservation(params.id);
  ...
  const isUserApprover = () => {
    // Logic to determine if current user can approve/reject
  ...
  return (
    <ContentLayout title={loading ? 'Loading...' : `Reservation: ${reservation?.title || ''}`}>
      <Breadcrumb>
        <BreadcrumbList>
          <BreadcrumbItem>
          ...
          <BreadcrumbItem>
            <BreadcrumbPage>
              {loading ? 'Loading...' : reservation?.title || 'Reservation Details'}
            </BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      {loading ? (
              ...
              <CardContent className="space-y-6">
                <div>
                  ...
                  <p className="mt-1 text-lg">
                    {reservation.resource ? (
                      ...
                      >
                        {reservation.resource.resource_name} ({reservation.resource.resource_type})
                      </Link>
                    ) : (
                      'Unknown Resource'
                    )}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="mt-1">{formatDateTime(reservation.start_datetime)}</p>
                    </div>
                  </div>
                  <div className="flex items-start">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex items-start">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-gray-500">Requester Institution</h3>
                      <p className="mt-1">
                        {reservation.requester_institution?.name || 'Not specified'}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-start">
                    ...
                    <div>
                      <h3 className="text-sm font-medium text-gray-500">Requester Department</h3>
                      <p className="mt-1">
                        {reservation.requester_department?.department_name || 'Not specified'}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="mt-1">{reservation.user?.email || 'Unknown User'}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm">
                    {reservation.created_at ? formatDateTime(reservation.created_at) : 'Unknown'}
                  </p>
                </div>
                ...
                {reservation.updated_at && (
                  <div>
                    ...
                    <p className="mt-1 text-sm">{formatDateTime(reservation.updated_at)}</p>
                  </div>
                )}
              </CardContent>
              <CardFooter>
        ...
        <div className="py-12 text-center">
          ...
          <p className="mt-2 text-gray-500">The reservation you are looking for doesn&apos;t exist or you don&apos;t have permission to view it.</p>
          <Button 
            className="mt-4" 
            onClick={() => router.push('/resources/reservations')}
          >
