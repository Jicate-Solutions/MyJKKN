...
// Letters track the sample-data key order. staff_id was removed on 2026-08-28
// (the database generates it), so everything from profile_picture shifted one
// column left — the letters are positional, not names, so they must move too.
const COLUMN_WIDTHS = {
  A: 20, // first_name
  B: 20, // last_name
  C: 15, // gender
  ...
  O: 25, // designation
  P: 30, // institution_email
  Q: 40, // category_id or category_name
...
const ID_BASED_INSTRUCTIONS = [
  ...
  [''],
  ['Optional Fields:'],
  ['1. Staff ID - Do NOT include. Generated automatically on save (e.g. DCH001 teaching, NOTDCH001 non-teaching); any value in this sheet is ignored.'],
  ...
  ['- Staff ID is generated by the system and cannot be set from this sheet'],
...
export default function DownloadStaffTemplateButton() {
  ...
  const createExampleData = (templateType: 'id' | 'name') => {
    const exampleData = [
      ...
      ['staff_id', '(auto)', 'Generated on save — omit this column'],
