...
-- Critical for audit trail queries — every "show history for this metric" lookup needs this.
...
> 3. **SELECT-only enforcement:** The engine MUST validate that `query_template` starts with `SELECT` (after trimming whitespace/comments). The blocklist check MUST scan the ENTIRE query text (not just the leading keyword) using a full-text regex: `/\b(INSERT|UPDATE|DELETE|DROP|ALTER|CREATE|TRUNCATE|GRANT|REVOKE|EXECUTE)\b/i`. This catches DML-in-subquery attacks like `SELECT * FROM (DELETE FROM x RETURNING *) y`.
