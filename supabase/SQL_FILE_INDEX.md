...
- `supabase/migrations/20260731190000_clarification_two_sided_close.sql` — **NOT applied yet — apply was permission-gated in the build session; rehearsal GREEN.** Full `BEGIN..ROLLBACK` rehearsal of [migration + battery] as one Mgmt-API batch passed (HTTP 201, 12/12 assert groups), and a SEPARATE call confirmed zero residue (no table, no fns, CHECK unchanged, no config row, 0 BAT-rows). Spec: `specs/clarification-act-two-sided-close-2026-07-30.md` (8 Director-locked decisions).
