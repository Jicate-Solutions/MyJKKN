...
/**
 * Color-coded framework pill so Fink's vs Bloom's vs custom are scannable at a
 * glance across the taxonomy screens. Renders the human label ("Fink's") rather
 * than the raw code ("finks").
 */
export function TaxonomyBadge({ type, className }: { type: string; className?: string }) {
  ...
  const label = t === 'finks' ? "Fink's" : t === 'blooms' ? "Bloom's" : type;
