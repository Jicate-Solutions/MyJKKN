-- Preview function gains p_strict (default false) so the candidate table mirrors the
-- strict toggle. The three physical checks pass p_strict to fn_learner_strictly_eligible_for_room.
-- See 20260615160000 for the engine-side change.
DROP FUNCTION IF EXISTS public.fn_auto_allocate_candidates(uuid);
CREATE OR REPLACE FUNCTION public.fn_auto_allocate_candidates(p_block_id uuid, p_strict boolean DEFAULT false)
...
REVOKE EXECUTE ON FUNCTION public.fn_auto_allocate_candidates(uuid, boolean) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_auto_allocate_candidates(uuid, boolean) TO authenticated, service_role;
