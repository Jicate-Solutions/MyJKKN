...
export function useHostelAlertRules(institutionId: string | undefined, filters?: Record<string, unknown>) {
...
export function useHostelRiskAlerts(institutionId: string | undefined, filters?: Record<string, unknown>) {
  const { isSuperAdmin } = usePermissions();
  return useQuery({
