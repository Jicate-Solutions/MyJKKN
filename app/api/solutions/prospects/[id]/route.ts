...
export const GET = withAuth(async (request, auth, context) => {
  ...
  const result = await ProspectsService.getProspectById(id)
  if (!result) return errorResponse('Prospect not found', 404)
...
export const PATCH = withAuth(async (request, auth, context) => {
  ...
  const result = await ProspectsService.updateProspect(id, safeBody)
  return successApiResponse(result)
})
