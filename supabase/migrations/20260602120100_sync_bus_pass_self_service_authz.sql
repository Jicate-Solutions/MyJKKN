-- Add a self-service authorization path to the bus-pass sync RPC so the requester
-- can finalize their OWN request when the type has NO approval steps (instant
-- self-service). The no-steps gate prevents bypassing approval on review-required
-- types; auth.uid()=requester prevents acting on someone else's request. The
-- privileged approver path (approved/fulfilled only) is unchanged.
