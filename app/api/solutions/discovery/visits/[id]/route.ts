...
export const GET = withAuth(async (request, auth, context) => {
  ...
  const result = await DiscoveryService.getDiscoveryVisitById(id)
  if (!result) return errorResponse('Discovery visit not found', 404)
...
export const PATCH = withAuth(async (request, auth, context) => {
  ...
  const result = await DiscoveryService.updateDiscoveryVisit(id, safeBody)
  return successApiResponse(result)
})

export const DELETE = withAuth(async (request, auth, context) => {
  const { id } = await context!.params!
  await DiscoveryService.deleteDiscoveryVisit(id)
