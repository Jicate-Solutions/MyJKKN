...
/** Shape of vote row for counting */
interface VoteRow {
  nomination_id: string;
  position_id?: string;
}

/** Shape of nomination row for results */
interface NominationForResults {
  nominee_id: string;
  position_id: string | null;
}

/** Shape of YUVA history row for progression */
interface YUVAHistoryRow {
  role: string;
...
/** Shape of eligible user row with profile join */
interface UserWithProfile {
  user_id: string;
  user?: { id: string; full_name: string; email: string; avatar_url: string | null } | null;
}

/** Shape of profile row for eligible nominees */
...
/** Shape of nomination with nominee_id for duplicate check */
interface NominationWithNomineeId {
  nominee_id: string;
}
...
export class LCSelectionService {
  ...
  static async reviewNomination(
    ...
    try {
      const nomination = data as unknown as { nominee_id?: string };
  ...
  static async getInterviews(nominationId: string): Promise<LCInterview[]> {
    ...
    return (data || []) as unknown as LCInterview[];
  }
  ...
  static async getElectionResults(electionId: string): Promise<{
    ...
    ((votes || []) as unknown as VoteRow[]).forEach((vote) => {
    ...
    const results = nominations.map(nom => ({
      ...
      nominee_avatar: (nom.nominee as { avatar_url?: string | null })?.avatar_url || null,
  ...
  static async getProgressionHistory(userId: string): Promise<{
    ...
    const yuvaRoles = ((yuvaHistory || []) as unknown as YUVAHistoryRow[]).map((r) => ({
      role: r.role,
  ...
  static async getEligibleNominees(electionId: string): Promise<{
    ...
    if (election.type === 'yuva_selection') {
      ...
      const { data: existingNominations } = await this.supabase
        .from('lc_nominations')
        .select('nominee_id')
        .eq('election_id', electionId);

      const nominatedIds = new Set(((existingNominations || []) as unknown as NominationWithNomineeId[]).map((n) => n.nominee_id));

      eligibleUsers = ((profiles || []) as unknown as ProfileRow[])
        .filter((p) => !nominatedIds.has(p.id))
        .map((p) => ({
          ...p,
      ...
      const { data: pastChairs } = await this.supabase
        .from('yuva_vertical_members')
        .select(`
          user_id,
      ...
      eligibleUsers = ((pastChairs || []) as unknown as UserWithProfile[])
        .filter((c) => c.user)
        .map((c) => ({
          id: c.user!.id,
      ...
      const { data: lcMembers } = await this.supabase
        .from('lc_members')
        .select(`
          user_id,
          user:profiles!user_id(id, full_name, email, avatar_url)
      ...
      eligibleUsers = ((lcMembers || []) as unknown as UserWithProfile[])
        .filter((m) => m.user)
        .map((m) => ({
          id: m.user!.id,
  ...
  static async declareResults(electionId: string): Promise<{
    ...
    for (const winner of winners) {
      const { data: nomination, error: updateError } = await this.supabase
        .from('lc_nominations')
