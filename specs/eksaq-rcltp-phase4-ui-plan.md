# EKSAQ RCLTP — Phase 4 UI Master Plan
...
1. **Every AI-drafted artifact is born `status='draft'`, `source='ai_generated'`,** with an `ai_meta` provenance stamp: `{ provenance: "AI-drafted — NOT validated; requires educator/EKSAQ review before use with students", model, drafted_at }`.
...
3. **Provisional numbers** (band cutoffs, NIPUN wpm, score weights) live in the CRUDable `platform_policies` rows, seeded `is_active=false` + labelled "provisional — pending EKSAQ validation". Real published standards (e.g. NIPUN Bharat fluency targets) are used where they exist and **cited**, never silently invented. EKSAQ's validated values overwrite the rows — no rebuild.
4. **Every screen showing AI-drafted or provisional content shows a persistent banner:** "Provisional — AI-drafted, pending EKSAQ validation."
...
### Phase 4a — Foundation + content unlock  ·  BUILDABLE NOW (zero EKSAQ dependency)
...
- **Sample content** loaded as a reviewable DRAFT seed (see `eksaq-rcltp-sample-content-DRAFT.md`).
...
- **Question-review console**: approve/reject AI-draft questions (the gate from 4a, teacher-side). "Generate questions with AI" button → calls gated `/questions/generate` → shows honest "awaiting EKSAQ prompt" until content lands.
...
- The 3 named reports (student/parent score report, teacher analytics, school-head dashboard) as shells; the **24-report catalog + the score/band data are EKSAQ-gated** → shells show "awaiting EKSAQ scoring + report catalog". No fabricated numbers.
...
- **Real reports** (24-report catalog + validated scoring) — slot into 4e shells when EKSAQ delivers.
