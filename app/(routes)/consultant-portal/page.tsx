...
//   - Fix 1+2: consultant lookup uses direct user_id query, not scan-and-find.
//   - Fix 6: "leads to next tier" calculation reads from the config-driven
//     getTierForConversions() reader instead of the hardcoded dashboard stats.
//     Badge LABEL still uses dashboard.stats.current_tier for parity with the
//     legacy display.
...
export default function ConsultantPortalDashboardPage() {
  ...
  const [resolvedTier, setResolvedTier] = useState<ResolvedTier | null>(null);
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      // Fix 1+2: direct user_id lookup, not limit:1 scan + .find().
      ...
      if (lookupError) {
        throw new Error(lookupError.message || 'Failed to load consultant');
      }
      ...
      const consultantRow = myConsultant as EducationConsultant;
      setConsultant(consultantRow);

      // Load portal dashboard
      const dashboardData = await ConsultantService.getConsultantPortalDashboard(
        consultantRow.id,
      );
      setDashboard(dashboardData);

      // Fix 6: source of truth for "leads to next tier" is the config reader,
      // not the hardcoded threshold map baked into the dashboard service.
      const tier = await getTierForConversions(
        consultantRow.total_conversions ?? 0,
        consultantRow.institution_id ?? profile.institution_id ?? null,
      );
  ...
  // Fix 6: leads_to_next + progress now come from the config reader. Falls back
  // to dashboard stats if the reader call returned null (cache cold / Supabase
  // unreachable was caught inside the service).
  const leadsToNext = resolvedTier?.leads_to_next ?? stats.leads_to_next_tier ?? null;
  const totalConversions = consultant?.total_conversions ?? stats.total_conversions ?? 0;
  const nextTierMin = resolvedTier?.next_tier
    ? (resolvedTier.max_conversions ?? 0) + 1
    : null;
  const progressToNextTier =
    nextTierMin && nextTierMin > 0
      ? Math.min(100, (totalConversions / nextTierMin) * 100)
      : 100;
            ...
            {leadsToNext !== null && leadsToNext > 0 ? (
              <>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {leadsToNext} more conversions to next tier
                </p>
              </>
            ) : (
              <p className="text-xs text-muted-foreground">
