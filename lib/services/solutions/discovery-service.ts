...
export class DiscoveryService extends BaseService {
  ...
  static async getDiscoveryVisits(
    ...
    let query = this.supabase.from('sh_discovery_visits')
      .select('*, client:sh_clients(id, name), department:sh_departments(id, name)', { count: 'exact' })
  ...
  static async createDiscoveryVisit(input: CreateDiscoveryVisitInput): Promise<DiscoveryVisit> {
    const { data, error } = await this.supabase.from('sh_discovery_visits')
      .insert({
        ...
        client_attendees: input.client_attendees || [],
        observations: input.observations || null,
        current_systems: input.current_systems || null,
        pain_points: input.pain_points || null,
        opportunities: input.opportunities || null,
        competitor_info: input.competitor_info || null,
        budget_indication: input.budget_indication || null,
        timeline_indication: input.timeline_indication || null,
