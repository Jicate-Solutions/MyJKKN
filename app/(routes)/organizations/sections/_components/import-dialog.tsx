...
/**
 * Import Dialog Component for Sections
 *
 * Features:
 ...
 * - Supports 5-level cascading validation (Institution → Degree → Department → Program → Semester)
 */
...
export function ImportDialog({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/organizations/sections/import', {
        method: 'POST',
        body: formData
      });
      ...
      if (data.success) {
        toast.success(
          `Successfully imported ${data.successCount} section${data.successCount !== 1 ? 's' : ''}!`,
          {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an Excel file to import multiple sections at once. The file must match
            the template format with 5-level cascading hierarchy (Institution → Degree → Department → Program → Semester).
          </DialogDescription>
        </DialogHeader>
                ...
                <AlertDescription>
                  <div className="space-y-2">
                    <p className="font-medium">
                      {result.success
                        ? `Successfully imported ${result.successCount} section${result.successCount !== 1 ? 's' : ''}`
