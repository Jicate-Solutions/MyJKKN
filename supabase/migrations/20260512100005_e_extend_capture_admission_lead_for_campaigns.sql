-- =====================================================
-- 2026-05-12 — Task 5: Extend capture_admission_lead RPC for campaign_link_id
--
-- Extends the canonical capture entry point to accept an optional
-- `campaign_link_id` in p_capture. The link id is soft-validated against
-- admission_campaign_links.is_active; an invalid/inactive link drops
-- attribution but keeps the lead (lead is the commercially valuable
-- artifact, attribution is metadata).
--
-- The Task 3 trigger trg_sync_lead_campaign_attribution handles the
-- first/last denormalization onto admission_leads and the capture_count
-- bump on admission_campaign_links.
--
-- Returns: jsonb { lead_id, capture_id, action, reactivated, attributed_link }
-- =====================================================
...
BEGIN
  ...
  ELSE
    ...
    -- Trigger only fires on UPDATE; CREATE path needs explicit null->new log.
