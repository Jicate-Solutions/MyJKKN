-- =====================================================================
-- 2026-05-11  Fully-qualify extensions.digest() in bulk routing functions
--
-- After migration 20260513180000_* baked `#variable_conflict use_column`
-- into both bulk_route_unassigned_leads and bulk_round_robin_assign,
-- runtime now fails with:
--
--   ERROR  42883  function digest(text, unknown) does not exist
--
-- Both functions set `SET search_path = public`, but the pgcrypto
-- extension (which provides digest()) is installed in the `extensions`
-- schema in this Supabase project. With search_path locked to public,
-- bare `digest(...)` resolves nothing.
--
-- Fix: fully-qualify the call as `extensions.digest(...)`. Safer than
-- widening search_path under SECURITY DEFINER — leaving search_path
-- locked to a single schema is best-practice to prevent search-path
-- injection in privilege-elevated functions.
-- =====================================================================
...
BEGIN
  ...
  v_hash := encode(extensions.digest(v_plan, 'sha256'), 'hex');
