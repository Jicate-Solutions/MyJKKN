...
/**
 * Hook to export OD requests data (returns data for CSV generation)
 */
export function useExportODRequests() {
  return useMutation({
    mutationFn: async (filters: {
      ...
      const result = await LCODService.getODRequests({ ...filters, page: 1, limit: 10000 });
      return result.data;
    },
    onError: (error: Error) => {
      toast.error(error.message || 'Failed to export OD requests');
    },
  });
}
