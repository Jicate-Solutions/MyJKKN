-- 20260610170000_candidates_institution_name.sql
-- Auto-allocate preview: add institution_name to fn_auto_allocate_candidates so the
-- per-student table can show + filter by the learner's institution (the block serves
-- one or more institutions via hostel_block_institutions). Enables the hierarchical
-- Institution → Program → Semester filter. Additive OUT column ⇒ DROP+CREATE (42P13).
-- Rebuilt from 20260610160000 (live).
