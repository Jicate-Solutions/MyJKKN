...
/**
 * The link THIS reader should see for an entry — or nothing.
 *
 * WHY A SECOND RULE EXISTS. entryHref() above answers "does this link point at
 * a real screen"; it cannot answer "may the person reading this row open it",
 * because it runs once at sync time over ~7,000 commits with no reader in the
 * room. The two questions have different answers for the same row: verified in
 * a browser on production, 2026-09-14, a super admin opened
 * /cdc/admin/exam-topic-map from a Foundation entry, while a learner following
 * the SAME link got "Access Denied — Required Permission: cdc.training.edit",
 * and a faculty member following an Admission entry's link got "None of your
 * roles include the permission admission.consultants.commissions.view".
 *
 * Both denials render as explicit cards, so nothing is silently bouncing — but
 * the Director's ruling of 2026-09-13 (edge case 3) is that a link to a page
 * someone cannot open is HIDDEN for them: "no dead ends". A wall you can read
 * is still a wall, and it is one this page sent the reader into.
 *
 * WHY ENTRY VISIBILITY DOES NOT ALREADY COVER IT. Which entries a reader
 * receives is decided per MODULE (fn_changelog_visible_modules → canSeeModule),
 * by permission NAMESPACE. The href is derived from the files a commit touched,
 * and a commit filed under one module routinely edits a screen gated by another
 * module's key. Module-level visibility therefore says nothing at all about the
 * one deep path a given row advertises.
 *
 * STILL PURE, AND DELIBERATELY SO. The access rule is not implemented here — it
 * is passed in. This file is imported by scripts/generate-changelog.mjs, which
 * runs in plain Node with no React, no session and no `@/` alias, so a direct
 * dependency on the app's permission stack would break the generator. The
 * caller (components/changelog/whats-new-view.tsx) supplies `canOpen`, wired to
 * this repository's canonical route gate — routeMatcher + isPageAccessible, the
 * same pair RoutePermissionGuard and the sidebar already use. There is no second
 * permission system here and there must never be one.
 *
 * @param {string | null | undefined} entryHref   the entry's own derived screen
 * @param {string | null | undefined} moduleHref  the module's landing page
 * @param {string} moduleLabel                    wording for the module link
 * @param {(href: string) => boolean} canOpen     may this reader open that path
 * @returns {{ href: string, label: string } | null}
 */
export function chooseEntryLink(entryHref, moduleHref, moduleLabel, canOpen) {
  // The exact screen, when the reader may actually open it. "Open this page"
  // promises precision, so it is only ever offered for a path that resolves.
  if (entryHref && canOpen(entryHref)) {
    return { href: entryHref, label: 'Open this page' };
  }
  // Otherwise the area — a weaker but honest promise, and re-tested rather than
  // assumed: a module a reader receives NEWS about is not automatically a module
  // whose landing page their role opens.
  if (moduleHref && canOpen(moduleHref)) {
    return { href: moduleHref, label: `Open ${moduleLabel}` };
  }
  // And otherwise nothing. Plain text beats an anchor that takes a tap, takes
  // focus, and spends both on a refusal.
  return null;
}
