...
-- Fix get_seat_analytics RPC:
--   1. Switch from academic_years to admission_years (per-institution-per-program cohorts).
--   2. Use admission_years.sanctioned_intake for total seats.
--   3. Count lifecycle_status IN ('admitted','active','graduated') as filled.
--   4. Dual-join: admission_year_id FK (primary) OR integer year match (fallback for NULL FKs).
--   5. Drop old 2-arg signature to avoid silent routing to stale function.

-- Drop old 2-argument function first (signature changed from (uuid,uuid) to (uuid))
