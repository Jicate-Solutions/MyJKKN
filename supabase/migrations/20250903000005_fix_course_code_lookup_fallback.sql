-- Fix course code lookup by adding fallback to lookup by course name when course_id is null
-- This fixes the issue where course_code shows as N/A in attendance quick view modal
...
BEGIN
    ...
    -- ENHANCED: Handle course lookup with fallback to course name search
    BEGIN
        course_uuid := NULL;
        IF period_data->>'course_id' IS NOT NULL AND period_data->>'course_id' != '' AND period_data->>'course_id' != 'null' THEN
    ...
    -- FIXED: Get additional details from related tables with proper JOINs and course lookup fallback
    SELECT 
    ...
    -- FIXED: If course_code is still null, try to lookup by course name from attendance_data
    IF (result_record.course_code IS NULL OR result_record.course_code = '') AND period_data->>'course_name' IS NOT NULL THEN
        SELECT 
            c.course_code::TEXT
        INTO course_lookup_record
        FROM courses c
        WHERE LOWER(TRIM(c.course_name)) = LOWER(TRIM(period_data->>'course_name'))
        LIMIT 1;
        
        IF FOUND AND course_lookup_record.course_code IS NOT NULL THEN
            result_record.course_code := course_lookup_record.course_code;
        END IF;
    END IF;
    ...
    RETURN QUERY SELECT
        ...
        COALESCE(result_record.course_code, 'N/A')::TEXT,  -- FIXED: Should now show actual course code
        ...
        COALESCE(result_record.degree_name, 'N/A')::TEXT;
