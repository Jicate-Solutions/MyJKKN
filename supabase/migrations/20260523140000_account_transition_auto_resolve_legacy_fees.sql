-- ============================================================================
-- 20260523140000 — Auto-resolve fees for legacy-mode learners on account transition
-- ============================================================================
-- Bug: admission officers clicking "Move to Account" on learners with
-- legacy_fee_mode=true and empty fee_items get 'legacy_fee_items_empty'.
-- The workaround (edit → Finance → save → retry) triggers the post-save
-- adoptStructureForLead path, which flips legacy_fee_mode and resolves.
--
-- Fix: In the legacy branch of the transition RPC, when fee_items is
-- NULL/empty, auto-flip legacy_fee_mode=false and resolve from the fee
-- structure matrix — same as the non-legacy path. If the matrix also
-- yields nothing, raise a descriptive error.
-- ============================================================================

DROP FUNCTION IF EXISTS public.admission_account_transition_with_bills(uuid, jsonb, jsonb, uuid, text);
