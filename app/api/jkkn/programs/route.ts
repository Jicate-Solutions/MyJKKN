...
import type { JkknProgramUpstreamResponse } from '@/types/jkkn-api/programs';
 ...
 *
 * Secure server-side proxy for the JKKN programs endpoint.
 * JKKN_API_KEY is a server-only env var — never sent to the browser.
 * Accepts ?page, ?limit, ?search, ?institution_id, ?degree_id, ?department_id, ?isActive.
 *
...
export async function GET(request: NextRequest) {
  ...
  if (degreeId) upstreamParams.set('degree_id', degreeId);
  const departmentId = searchParams.get('department_id');
  if (departmentId) upstreamParams.set('department_id', departmentId);
  const isActive = searchParams.get('isActive');
  if (isActive !== null) upstreamParams.set('isActive', isActive);

  try {
    const res = await fetch(
      `${JKKN_API_BASE_URL}/api-management/organizations/programs?${upstreamParams}`,
      {
        headers: {
