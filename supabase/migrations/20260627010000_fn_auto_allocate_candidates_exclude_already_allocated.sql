-- Exclude already-allocated students from fn_auto_allocate_candidates so the
-- preview table only shows the unallocated hostelites (those who actually need
-- a bed). Previously, allocated students were included and shown with
-- verdict='out' / exclusion_reason='Already allocated', which cluttered the
-- preview and slowed the query by running two expensive LATERAL eligibility
-- joins for students who can never be placed.
--
-- Fix: add NOT EXISTS to the cohort CTE before any lateral joins fire.
-- Mirrors the not_allocated check already in the base CTE (a.learner_id=p.id
-- via profiles join), applied one step earlier for efficiency.
...
RETURNS TABLE(
  ...
  gender text, has_profile boolean, gender_ok boolean,
  not_allocated boolean, physical_rule_ok boolean, bed_available boolean,
