...
-- Auto-allocate preview: add semester_name to fn_auto_allocate_candidates so the
-- per-student validation table can show which semester/year each learner is in
-- (the cohort dimension that physical-room rules reserve by). Additive: one new
-- column after program_name; logic unchanged. Rebuilt from 20260610140000 (live).
-- Adding an OUT column changes the return row type, so the old signature must be
-- dropped first (CREATE OR REPLACE alone errors 42P13).
