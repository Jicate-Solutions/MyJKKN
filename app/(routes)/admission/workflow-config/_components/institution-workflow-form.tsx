...
export function InstitutionWorkflowForm({
  ...
  const [hasGdPi, setHasGdPi] = useState(config?.has_gd_pi ?? false);
  const [hasMeritList, setHasMeritList] = useState(config?.has_merit_list ?? false);
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex items-center justify-between">
            <Label htmlFor="gdpi">GD/PI</Label>
            <Switch id="gdpi" checked={hasGdPi} onCheckedChange={setHasGdPi} />
          </div>
          <div className="flex items-center justify-between">
            <Label htmlFor="merit">Merit List</Label>
            <Switch
              id="merit"
              checked={hasMeritList}
              onCheckedChange={setHasMeritList}
            />
          </div>
        </CardContent>
      </Card>
