-- Fix get_attendance_record_details function to properly handle student names and marked_by field
-- Issues fixed:
-- 1. Student names showing as "Unknown Student" due to improper NULL handling in concatenation
-- 2. marked_by field returning UUID instead of name
...
BEGIN
        ...
        SELECT 
            ...
            CASE 
                WHEN st.first_name IS NOT NULL AND TRIM(st.first_name) != '' THEN
                    TRIM(st.first_name) || COALESCE(' ' || NULLIF(TRIM(st.last_name), ''), '')
                ELSE 
                    'Unknown Student'
            END as student_name,
    ...
    RETURN QUERY SELECT
        ...
        CASE 
            ...
            ELSE 0
        END,
        marked_by_name::TEXT,  -- FIXED: Return resolved name instead of UUID
