...
/**
 * Import Dialog Component for Enquiries
 *
 * Features:
 ...
 * - Download template shortcut with cascading dropdowns
 */
...
export function ImportDialog({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/learners/enquiries/import', {
        method: 'POST',
        body: formData
      });
      ...
      if (data.success) {
        toast.success(
          `Successfully imported ${data.successCount} enquir${data.successCount !== 1 ? 'ies' : 'y'}!`
        );
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an Excel file to import multiple enquiry records at once.
            The file must include cascading fields (Institution → Department → Program → Semester → Section).
          </DialogDescription>
        </DialogHeader>
          ...
          <Alert>
            ...
            <AlertDescription className="flex items-center justify-between">
              <span className="text-sm">
                Don&apos;t have a template? Download it with cascading dropdowns and validation.
              </span>
              <Button
                variant="outline"
                size="sm"
          ...
          {uploading && (
            <div className="space-y-2">
              ...
              <p className="text-sm text-center text-gray-600 dark:text-gray-400">
                ...
                {progress >= 30 && progress < 70 && 'Validating data and hierarchy...'}
                {progress >= 70 && 'Creating enquiry records...'}
              </p>
            </div>
          )}
            ...
            <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-green-700 dark:text-green-300">
                          Successfully imported {result.successCount} enquir
                          {result.successCount !== 1 ? 'ies' : 'y'} out of {result.totalRows}{' '}
              ...
              {result.duplicateEmails && result.duplicateEmails.length > 0 && (
                <Alert variant="destructive">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    <p className="font-medium mb-2">Duplicate Student Emails Found:</p>
                    <ul className="list-disc list-inside text-sm space-y-1">
                      {result.duplicateEmails.map((msg, index) => (
                        <li key={index}>{msg}</li>
                      ))}
                    </ul>
                  </AlertDescription>
              ...
              {/* Error Details */}
              {result.errors && result.errors.length > 0 && !result.duplicateEmails && (
