-- Year-of-study aware academic fee resolution.
--
-- Makes admission_resolve_fee_items_for_lead() filter the matched fee
-- structure's ITEMS by the learner's current year of study, using the
-- new admission_fee_structure_items.applies_to / applies_year_of_study
-- columns + the fn_learner_year_of_study() helper.
--
-- TWO surgical additions versus the prior version (everything else is
-- preserved byte-for-byte):
--   1. DECLARE: v_year int := COALESCE(public.fn_learner_year_of_study(p_learner_id), 1);
--      (undetermined year -> treat as first-year, never NULL)
--   2. The admission_fee_structure_items (alias fsi) SELECT WHERE clause
--      gains a year-of-study predicate AND-ed with the existing condition.
