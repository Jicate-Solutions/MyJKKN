...
import { useJkknDepartments } from '@/hooks/use-jkkn-departments';
import type { JkknDepartment } from '@/types/jkkn-api/departments';
...
function DepartmentRow({ department }: { department: JkknDepartment }) {
  return (
    <TableRow className="hover:bg-muted/40 transition-colors">
      <TableCell className="font-medium max-w-[240px] truncate" title={department.department_name}>
      ...
      <TableCell className="hidden sm:table-cell text-muted-foreground text-sm">
        {format(new Date(department.created_at), 'dd MMM yyyy')}
      </TableCell>
    </TableRow>
  );
}
...
export function DepartmentsTable() {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } =
    useJkknDepartments({ page, limit: PAGE_SIZE, search });
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50 hover:bg-muted/50">
                    <TableHead className="w-[240px]">Department Name</TableHead>
                      ...
                      <TableCell colSpan={6} className="h-48 text-center text-muted-foreground">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          {search
                            ...
                            : 'No departments returned from the API.'}
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
      ...
      {!isError && !isLoading && totalPages > 1 && (
        <div className="flex items-center justify-between gap-4">
          <p className="text-sm text-muted-foreground">
            ...
            <span className="font-medium">{total}</span> total departments
          </p>
