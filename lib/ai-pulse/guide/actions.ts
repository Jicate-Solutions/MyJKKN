...
/**
 * AI Pulse Smart Guide — server actions for progress + instrumentation.
 *
 * Mirrors the proven campus-living adoption layer (PR #1379): a "use server"
 * file may export ONLY async functions (no types/consts) or the Vercel build
 * breaks. All three are scoped to the signed-in user, and RLS enforces row
 * ownership too (migration 20260613234500_guide_progress_events.sql — the same
 * generic guide_progress / guide_events tables every module guide shares).
 * Reads/writes FAIL SOFT — a progress hiccup must never block the guide or
 * throw into the UI.
 */
