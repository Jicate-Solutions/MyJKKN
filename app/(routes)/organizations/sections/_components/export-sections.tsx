  ...
  const handleExport = async () => {
    try {
      ...
      const response = await fetch(
        `/api/organizations/sections/export?${params.toString()}`
      );
      ...
      if (contentDisposition) {
        ...
        filename = `sections-export-${dateStr}.${exportFormat}`;
      }
      ...
      toast.error(
        error instanceof Error ? error.message : 'Failed to export sections'
      );
    } finally {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='sm:max-w-[425px]'>
        <DialogHeader>
          <DialogTitle>Export Sections</DialogTitle>
          <DialogDescription>
