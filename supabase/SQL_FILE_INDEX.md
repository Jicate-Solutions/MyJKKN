...
- `supabase/migrations/20260805150000_ai_pulse_starter_topics_from_timetables.sql` — **NOT applied to any database — Director-gated apply.** Closes the reason 6 of 8 AI Pulse cycles produced zero starter prompts: `fn_ai_pulse_domain_starter_candidates` derived its topics from `ai_pulse_live_attendance` for the SAME cycle, but the generation cron fires at 09:00 UTC and on cycle `53a9eb29` (2026-07-30) the first check-in was 13:10:06 UTC — measured on prod, exactly **0** of that cycle's 285 check-ins existed before 09:00, so the `>= min_learners` floor yielded nothing and the run enqueued nothing. Replaced from the LIVE definition (`pg_get_functiondef` md5 `790099e503eba1e5ddd3e93f333c600a`), not from a repo file; the whole `prior_context` CASE — including the auto-revert branch and its `domain_starter_revert_margin` read — is carried over byte-for-byte, and the RETURNS TABLE shape is unchanged so the cron route needs no reshaping.
- **Grain = ACTIVE PROGRAMME, every institution** (Director decision #4). Measured on prod: **120** active programmes across **11** institutions, versus 3,833 active courses (64× over capacity) and 44 programmes inside the 5 rotation colleges. Deliberately NOT restricted to the rotation (decision #7) — the ~2,049 learners in Arts & Science Self (760), Matric (552), Arts & Science Aided (511) and CBSE (225) now receive a prompt despite having no session, team or leaderboard place.
...
- **Carry-over is EXCLUSION, not sort order** (decision #5). A topic already holding a starter for this cycle is dropped from the result, so each capped run continues exactly where the last stopped and the route's new `remaining` field is literally "how many are still waiting". Sorting-last was rejected because the Max lane's dedupe key is an IN-FLIGHT guard only (`lib/services/platform/ai-jobs-lane.ts` — queued/claimed/running), NOT an "already done" guard: a third run over a sorted-but-included list would re-enqueue served topics and `fn_ai_pulse_record_domain_starter` would UPSERT over prompts learners may already have copied.
