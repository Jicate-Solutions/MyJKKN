...
export const MISC_AI_ROUTINES: AIRoutine[] = [
  ...
  },
  {
    ...
    "configKnobs": "cdc_placement_loop.master_enabled=false (DARK — config row), cdc_placement_loop.min_cohort_size=10 (config row; smaller cohorts skipped as noise), delta deadband=±2.0pp on progression rate (fixed in fn), June AY cutoff",
    "sideEffects": "WRITES cdc_placement_outcome_cycles (one row per cohort per month, change-only) and upserts quality_evidence_mappings (NAAC 8.2.1, is_auto=true). No notifications, no human messages. Full no-op while dark.",
    "safeToManualTrigger": true,
