-- Updated: 2026-07-25 - Add CARRE-C5 evidence line: open event-date requests (Lane B).
-- Based EXACTLY on 20260725110000_fix_carre_evidence_academic_block.sql (the live
-- definition, verified byte-identical via pg_get_functiondef before this edit).
-- ONE new block added after the OD/A3 block: for every module, a C5 line with the
-- count of OPEN event_date_requests + oldest waiting days (cheap aggregate over the
-- new table from 20260725150000_event_date_requests.sql; partial index keeps it
-- index-only). Every other byte of the function is identical to the live definition.
-- Timing: the C5 aggregate was timed as plain SQL in a rolled-back mgmt-API probe
-- before this CREATE OR REPLACE (ms recorded in the PR body).
...
BEGIN
  ...
  -- ── Event-date requests → C5 (asks that wait), all three modules ─────────
  SELECT count(*)::int,
         COALESCE(max(now()::date - r.requested_at::date), 0)
    INTO v_edr_open, v_edr_oldest_days
  FROM public.event_date_requests r
  WHERE r.decided_at IS NULL;

  RETURN QUERY
  SELECT 'CARRE-C5'::text,
         format('Event-date loop: %s open requests for a confirmed event date (oldest waiting %s days). Each open row is a timestamped ask that nobody has answered yet — measured waiting, not anecdote.',
                v_edr_open, v_edr_oldest_days)::text,
         NULL::smallint,
         jsonb_build_object('edr_open', v_edr_open, 'edr_oldest_days', v_edr_oldest_days);
