...
//
// NOTE (merge of #2584 + #2496): this PR also adds a server-side `lanes[]` view
// carrying `oldest_mins` per lane — a cleaner source for the same signal than the
// client-side age computed from `by_type[].oldest` below. Both are kept here so
// neither PR regresses; folding the banner onto `lanes[]` is a follow-up, not a
// change smuggled into this PR.
