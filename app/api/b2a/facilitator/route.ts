/**
 * B2A — Facilitators (faculty / teachers / mentors).
 * Backed by `staff` filtered to facilitator role_keys.
 */
...
const MODULE_KEY = 'facilitator' as const;
const ENDPOINT = '/api/b2a/facilitator';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    let query = context.supabase
      .from('staff')
      .select('id, first_name, last_name, designation, role_key, department_id, institution_id, is_active, date_of_joining, created_at', { count: 'exact' })
      .or('role_key.ilike.%faculty%,role_key.ilike.%teacher%,role_key.ilike.%facilitator%,role_key.ilike.%mentor%,role_key.ilike.%professor%')
      .order('created_at', { ascending: false })
      .range(from, to);

    if (institutionId) query = query.eq('institution_id', institutionId);
    if (url.searchParams.get('is_active') === 'true') query = query.eq('is_active', true);
    ...
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Failed to list facilitators', detail: message } },
