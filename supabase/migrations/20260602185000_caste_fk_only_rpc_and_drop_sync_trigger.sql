-- Caste FK-only DB prep: ai_rpc_admissions derives caste from a castes join
-- (cst.name AS caste); drop the community/caste FK→text sync trigger + function
-- (now a no-op — community branch removed in the community phase, caste branch
-- removed here as the caste TEXT column is retired). The full ai_rpc_admissions
-- body is reapplied with only the caste join/select change vs 20260602182000.
...
DROP FUNCTION IF EXISTS public.sync_learner_community_caste_text();
