-- ============================================================================
-- THE ADMIN BALANCE GRID LEARNS ABOUT ACCRUAL (2026-09-05)
--
-- v_hr_leave_balance_src has computed `accrued` and `pending` since
-- 2026-09-02, and the STAFF-facing side already reads both -- leave-service.ts
-- selects them and the apply drawer tells the staff member "X of Y days have
-- accrued so far this year". hr_leave_balance_staff_detail, which feeds
-- /hr/admin/leave-balances, builds its own cell JSON and emitted neither.
-- So HR saw entitled-only figures while the staff member saw accrual-aware
-- ones, and the two screens disagreed about the same person.
--
-- Now that Casual Leave is actually on monthly accrual (20260905120000) that
-- disagreement is no longer cosmetic: entitled is 12 all year while accrued
-- climbs one a month, so the admin grid would overstate every CL balance by
-- up to 11 days.
--
...
-- ---------------------------------------------------------------------------
-- 2. The admin detail RPC learns about accrual
--
-- CREATE OR REPLACE, never DROP + CREATE: a DROP takes the ACL with it, and the
-- anon REVOKE applied in 20260925000000 would silently come undone -- a new
-- function is EXECUTE-able by PUBLIC, which includes anon.
--
-- Body is byte-for-byte 20260828191000 plus two additions: the cells CTE now
-- carries v.accrued / v.pending out of v_hr_leave_balance_src (which has
-- computed both since 2026-09-02), and the day cell exposes them.
-- ---------------------------------------------------------------------------
