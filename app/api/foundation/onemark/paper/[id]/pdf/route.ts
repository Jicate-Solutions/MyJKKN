...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // A character no embedded font can print is NOT an error: the renderer
    // prints a visible "[?]" in its place and logs the item id and code point
    // (render.ts). This route used to answer 422 for it, which blocked a whole
    // answer key over one ᵣ (Wave 3 try-out, 2026-09-11).
