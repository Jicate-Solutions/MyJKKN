-- Auto-allocation: optional FLOOR scope. Operators can restrict an auto-allocation run
-- (and its preview/candidate validation) to a single floor of a block, or leave it across
-- all floors. p_floor int DEFAULT NULL → NULL means "all floors". The filter is applied to
-- every room/bed query so preview, candidate table, and the engine agree.
--
-- Adding a parameter changes each function's signature, so the old versions are DROPped
-- first (CREATE OR REPLACE would leave an ambiguous overload that PostgREST 300-errors on).
-- Grants (EXECUTE to authenticated + service_role; not anon/PUBLIC) are re-applied.

-- ── Preview (aggregate counts) ──────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.fn_auto_allocate_preview(uuid);
CREATE FUNCTION public.fn_auto_allocate_preview(p_block_id uuid, p_floor int DEFAULT NULL)
...
$function$;
REVOKE EXECUTE ON FUNCTION public.fn_auto_allocate_preview(uuid, int) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_auto_allocate_preview(uuid, int) TO authenticated, service_role;

-- ── Candidate validation table ──────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.fn_auto_allocate_candidates(uuid, boolean);
CREATE FUNCTION public.fn_auto_allocate_candidates(p_block_id uuid, p_strict boolean DEFAULT false, p_floor int DEFAULT NULL)
...
$function$;
REVOKE EXECUTE ON FUNCTION public.fn_auto_allocate_candidates(uuid, boolean, int) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_auto_allocate_candidates(uuid, boolean, int) TO authenticated, service_role;

-- ── Engine (generate proposed batch) ────────────────────────────────────────
DROP FUNCTION IF EXISTS public.fn_auto_allocate_classic(uuid, uuid, boolean);
CREATE FUNCTION public.fn_auto_allocate_classic(p_block_id uuid, p_hostel_year_id uuid, p_strict boolean DEFAULT false, p_floor int DEFAULT NULL)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  RETURN v_batch;
END $function$;
REVOKE EXECUTE ON FUNCTION public.fn_auto_allocate_classic(uuid, uuid, boolean, int) FROM anon, PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_auto_allocate_classic(uuid, uuid, boolean, int) TO authenticated, service_role;
