/**
 * Shared Advanced Search Component for Learners Modules
 *
 * Used across: Profiles, Enquiries, Alumni, Change Requests
 *
 * Features:
...
interface LearnerAdvancedSearchSharedProps {
  onSearch: (filters: LearnerSearchFilters) => void;
  onClear: () => void;
  placeholder?: string;
  showApplicationId?: boolean; // For enquiries module
}
...
export function LearnerAdvancedSearchShared({
  onSearch,
  onClear,
  placeholder = "Search students...",
  showApplicationId = false
}: LearnerAdvancedSearchSharedProps) {
  // Separate query states for each search field
  const [nameQuery, setNameQuery] = useState('');
  const [rollNumberQuery, setRollNumberQuery] = useState('');
  const [emailQuery, setEmailQuery] = useState('');
  const [searchOptions, setSearchOptions] = useState<LearnerSearchOptions>({
    ...defaultSearchOptions,
    searchFields: {
      ...defaultSearchOptions.searchFields,
      applicationId: showApplicationId
    }
  });
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 gap-2">
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="name"
                        checked={searchOptions.searchFields.name}
                        onCheckedChange={(checked) =>
                          handleFieldChange('name', checked as boolean)
                        }
                      />
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="rollNumber"
                        checked={searchOptions.searchFields.rollNumber}
                        onCheckedChange={(checked) =>
                          handleFieldChange('rollNumber', checked as boolean)
                        }
                      />
                      <Label htmlFor="rollNumber" className="text-sm font-normal cursor-pointer">
                        Roll Number
                      </Label>
                    </div>
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        id="collegeEmail"
                        checked={searchOptions.searchFields.collegeEmail}
                        onCheckedChange={(checked) =>
                          handleFieldChange('collegeEmail', checked as boolean)
                        }
                      />
                      <Label htmlFor="collegeEmail" className="text-sm font-normal cursor-pointer">
                        College Email
                      </Label>
                    </div>
                    {showApplicationId && (
                      <div className="flex items-center space-x-2">
                        <Checkbox
                          id="applicationId"
                          checked={searchOptions.searchFields.applicationId}
                          onCheckedChange={(checked) =>
                            handleFieldChange('applicationId', checked as boolean)
                          }
                        />
                        <Label htmlFor="applicationId" className="text-sm font-normal cursor-pointer">
                          Application ID
                        </Label>
                      </div>
                    )}
