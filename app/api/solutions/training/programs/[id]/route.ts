...
export const GET = withAuth(async (request, auth, context) => {
  ...
  const result = await TrainingService.getProgramById(id)
  if (!result) return errorResponse('Training program not found', 404)
...
export const PATCH = withAuth(async (request, auth, context) => {
  ...
  const result = await TrainingService.updateProgram(id, safeBody)
  return successApiResponse(result)
})
