...
import { Loader2, MoreHorizontal, Plus, Search, Filter, Download, MoreHorizontal as DotsIcon } from 'lucide-react';
import { useDigitalResources } from '@/hooks/resource/use-digital-resources';
import { DigitalResource, DigitalResourceType, AccessMethod } from '@/types/digital-resources';
...
const PaginationEllipsis = ({ className }: { className?: string }) => (
  <div className={`flex h-9 w-9 items-center justify-center ${className}`} aria-hidden="true">
...
export function DigitalResourcesTable() {
  ...
  const [selectedAccessMethod, setSelectedAccessMethod] = useState<string>("all-methods");
  ...
  const handleAccessMethodChange = (value: string) => {
    ...
    fetchDigitalResources({
      search: searchQuery,
      access_method: value === "all-methods" ? undefined : value as AccessMethod,
      page: 1
    });
  };
  ...
  const handleEdit = (id: string) => {
    router.push(`/digital-resources/edit/${id}`);
  };
  ...
  const getStatusBadge = (resource: DigitalResource) => {
    ...
    if (resource.license_information?.expiration_date) {
      const expirationDate = parseISO(resource.license_information.expiration_date);
    ...
    <Card className="w-full">
      ...
      <CardContent>
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <div className="flex gap-2 flex-wrap sm:flex-nowrap">
            ...
            <Select value={selectedAccessMethod} onValueChange={handleAccessMethodChange}>
              <SelectTrigger className="w-full sm:w-[180px]">
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            No digital resources found. Create your first resource to get started.
          </div>
        ) : (
                  ...
                  <TableRow key={resource.id}>
                    ...
                    <TableCell>
                      {resource.license_information?.expiration_date
                        ? (() => {
                            const date = parseISO(resource.license_information.expiration_date);
      ...
      <CardFooter className="flex justify-between">
        <div className="text-sm text-muted-foreground">
          Showing {resources.length} of {(metadata as ResourceMetadata).total} resources
        </div>
        {renderPagination()}
      </CardFooter>
