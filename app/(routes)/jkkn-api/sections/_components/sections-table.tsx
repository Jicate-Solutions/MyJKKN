...
import { useJkknSections } from '@/hooks/use-jkkn-sections';
import type { JkknSection } from '@/types/jkkn-api/sections';
...
function SectionRow({ section }: { section: JkknSection }) {
  return (
    <TableRow className="hover:bg-muted/40 transition-colors">
      <TableCell className="font-medium max-w-[180px] truncate" title={section.section_name}>
...
export function SectionsTable() {
  ...
  const { data, isLoading, isFetching, isError, error, refetch } =
    useJkknSections({ page, limit: PAGE_SIZE, search });
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        <Button
          ...
          disabled={isFetching}
          className="ml-auto"
          aria-label="Refresh sections list"
        >
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/50 hover:bg-muted/50">
                    ...
                    <TableHead className="hidden xl:table-cell">Semester</TableHead>
                      ...
                      <TableCell colSpan={7} className="h-48 text-center text-muted-foreground">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          {search
                            ...
                            : 'No sections returned from the API.'}
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
      ...
      {!isError && !isLoading && totalPages > 1 && (
        <div className="flex items-center justify-between gap-4">
          <p className="text-sm text-muted-foreground">
            ...
            <span className="font-medium">{total}</span> total sections
          </p>
