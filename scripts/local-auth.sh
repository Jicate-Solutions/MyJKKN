...
# Callback path must be an EXACT match in Supabase's allow-list.
# A `?next=...` suffix breaks the match — Supabase silently rejects it and
# defaults to the project Site URL (undocumented; caught 2026-04-24).
# Workaround: use bare /auth/callback, role-redirect to /, then user navigates
# to TARGET_PATH. Deep-link via `?next=` will work once the allow-list has
# `http://localhost:3000/**` wildcard (see scripts/README.md setup note).
TARGET_PATH="${REDIRECT_PATH}"
LOCAL_REDIRECT="http://localhost:3000/auth/callback"
...
echo "$ACTION_LINK"
echo ""
echo "After the link auto-opens and auth completes, navigate to:"
echo "  http://localhost:3000${TARGET_PATH}"
echo ""

# Optional: auto-open in default browser if available
if command -v open >/dev/null 2>&1; then
  open "$ACTION_LINK" 2>/dev/null && echo "✅ Opened in default browser" || echo "⚠️  Auto-open failed; paste the link manually"
fi
