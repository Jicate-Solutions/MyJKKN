...
### Classroom Practice (L3) — teacher-level CARRE catalog (2026-07-29, NOT YET APPLIED)
- Location: `supabase/migrations/20260729183000_classroom_practice_catalog_l3.sql`
...
- Functions NEW: `fn_carre_create_classroom_audit(text,uuid,date,boolean)` (self-open by any team member; opening on another person needs audit leadership; freezes `catalog='CLASSROOM_PRACTICE'` + `teacher_profile_id` into the snapshot and sets `module_key='classroom-practice'`) · `fn_carre_set_participant_window(uuid,boolean)` (owner/leadership open-close of `participant_scoring_open` — no RPC previously existed) · `fn_carre_search_teachers(text)` (institution-scoped picker returning `profiles.id`, min 2 chars, LIKE wildcards escaped) · `fn_carre_learner_in_owner_roster(uuid,uuid)` (boolean-only roster gate; joins `profiles.email` = `session_feedback.faculty_email` because `faculty_id` is a `staff`-table id, not a `profiles.id`)
- Functions REPLACED (anon lock re-asserted on each): `fn_carre_participant_score` (parameter validation moved from catalog prefix to FROZEN-SNAPSHOT membership; adds the roster gate for `CLASSROOM_PRACTICE` cycles) · `fn_carre_participant_context` (returns `catalog` + `teacher_name`; same roster gate, denial reason `not_in_teacher_roster`) · `fn_carre_participant_rollup` (leadership path and k≥3 floor unchanged; ADDS an owner path admitted only when the window is CLOSED and the owner has self-scored every item) · `fn_carre_get_audit` (one added key: `cycle.participant_scoring_open`)
- Invariants enforced in the database, not the UI: batch reveal (no medians while the window is open), self-score-before-compare, k≥3, roster scope.
- Verification: executed against a stub substrate locally — 27/27 functional assertions passed (roster gate incl. case-insensitive email, batch reveal, 12-of-13 vs 13-of-13 self-score boundary, k-floor, cross-owner isolation, plain CARRE cycles unaffected). Rolled-back Mgmt-API rehearsal battery: `.claude/battery-classroom-practice-l3.sql`.
