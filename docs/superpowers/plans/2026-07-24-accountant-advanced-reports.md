...
- **`collected` = `SUM(billing_receipt_items.amount_paid)`** joined receipt→bill (allocation-based), never raw `billing_receipts.payment_amount`, so it is attributable per course/year.
