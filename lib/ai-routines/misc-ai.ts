...
export const MISC_AI_ROUTINES: AIRoutine[] = [
  ...
  },
  {
    ...
    "whatItDoes": "Tells the head of a learner's own department which of THEIR learners newly entered — or worsened within — high/critical risk, with the evidence attached: tier, composite score, 14-day attendance and its change, overdue bill count, and the engine's suggested actions. The risk engine ran exactly ONCE, on 2026-07-30 (4,342 rows; 59 critical, 403 high) — that was the operator's verification run while repairing it, and nothing was ever scheduled to call it again, so this routine has had only one stale day of assessments to read ever since. Migration 20260918033000 gives it a nightly pg_cron clock at 00:35 UTC. Recipients are TEAM MEMBERS ONLY — no learner and no family is notified by any path here.",
