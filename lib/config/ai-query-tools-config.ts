...
export const AI_QUERY_TOOLS_REGISTRY: AIQueryToolConfig[] = [
  ...
  },
  {
    ...
    description: 'Comprehensive location-based learner search - searches across permanent address district, street address, and taluk fields. Returns learners from specific areas with location statistics, distribution data, and indicates which field matched.',
    category: 'learners',
    permission: 'learners.view',
    tier: 1,
    rpcFunction: 'ai_rpc_learners_by_location',
    parameters: [
      { name: 'district', type: 'string', description: 'Filter by district name (e.g., Erode, Salem, Coimbatore) - searches in district, street, and taluk fields' },
      { name: 'state', type: 'string', description: 'Filter by state name (e.g., Tamil Nadu, Kerala)' },
      { name: 'taluk', type: 'string', description: 'Filter by taluk/sub-district name' },
      { name: 'city', type: 'string', description: 'Filter by city name - searches in district and street fields' },
  ...
  },
  {
    ...
    parameters: [
      { name: 'search', type: 'string', description: 'Free text search across name, roll number, email, mobile, parent names, district, school' },
      { name: 'status', type: 'string', description: 'Learner status', enum: ['active', 'inactive', 'graduated', 'exited', 'pending'] },
      ...
      { name: 'bus_required', type: 'boolean', description: 'Whether a day-scholar needs the college bus' },
      { name: 'institution_id', type: 'string', description: 'Filter by institution UUID' },
      { name: 'department_id', type: 'string', description: 'Filter by department UUID' },
      { name: 'program_id', type: 'string', description: 'Filter by program UUID' },
  ...
  // Admissions Tools (JKKN: Admission Applications merged with learners)
  {
    ...
    parameters: [
      ...
      { name: 'department_id', type: 'string', description: 'Filter by department UUID' },
      { name: 'program_id', type: 'string', description: 'Filter by program UUID' },
      { name: 'status', type: 'string', description: 'Admission status (enquiry, admitted, registered)' },
      { name: 'entry_type', type: 'string', description: 'Entry type (FIRST YEAR, LATERAL)' },
      ...
      { name: 'community', type: 'string', description: 'Filter by community (BC, MBC, SC, ST, OC)' },
      { name: 'counseling_applied', type: 'boolean', description: 'Applied through counseling' },
      { name: 'first_graduate', type: 'boolean', description: 'First-generation graduate' },
      { name: 'quota', type: 'string', description: 'Admission quota (GOVERNMENT, MANAGEMENT)' },
      { name: 'accommodation_type', type: 'string', description: 'Accommodation type (hostel, dayscholar)' },
    ...
    examples: [
      'Show all admissions from Erode district',
      'List government quota admissions',
      'How many enquiries did we receive this month?',
      'Show admissions who applied through counseling',
      'List first-generation graduates who got admitted',
    ],
  },
  {
    name: 'get_admission_details',
    description: 'Get complete details of a specific admission including all 71 fields',
    category: 'learners',
    permission: 'learners.admissions.view',
    tier: 1,
    ...
    examples: [
      'Show details for admission with application ID APP123',
      'Get complete profile of admission',
    ],
  },
  {
    name: 'get_admissions_by_location',
    description: 'Search admissions by geographic location (district, state, taluk, city)',
    category: 'learners',
    permission: 'learners.admissions.view',
    tier: 1,
    ...
    parameters: [
      ...
      { name: 'status', type: 'string', description: 'Admission status filter' },
      { name: 'include_stats', type: 'boolean', description: 'Include location statistics (default: true)' },
    ],
    examples: [
      'Show admissions from Karur district',
      'List admissions from Tamil Nadu',
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'date_from', type: 'string', description: 'Start date (YYYY-MM-DD)' },
      { name: 'date_to', type: 'string', description: 'End date (YYYY-MM-DD)' },
      { name: 'group_by', type: 'string', description: 'Group by field (status, department, etc.)' },
    ],
    examples: [
  ...
  },
  {
    ...
    description: 'Advanced admission analytics - conversion rates, processing times, trends, monthly breakdown',
    category: 'learners',
    permission: 'learners.admissions.dashboard',
    tier: 1,
    ...
    parameters: [
      ...
      { name: 'include_trends', type: 'boolean', description: 'Include monthly trends (default: true)' },
    ],
    examples: [
      'What is our admission conversion rate?',
      'Show monthly admission trends',
      'How long does it take to process admissions?',
    ],
  },
  {
    name: 'get_admission_referrers',
    description: 'Get top consultants/referrers with program and location breakdown - who brings admissions from where',
    category: 'learners',
    permission: 'learners.admissions.dashboard',
    tier: 1,
    rpcFunction: 'ai_rpc_admission_referrers',
    parameters: [
      { name: 'reference_type', type: 'string', description: 'Referrer type (consultant, staff, etc.)' },
      { name: 'reference_name', type: 'string', description: 'Referrer name search' },
      { name: 'institution_id', type: 'string', description: 'Filter by institution UUID' },
      { name: 'program_id', type: 'string', description: 'Filter by program UUID' },
      { name: 'top_n', type: 'number', description: 'Number of top referrers to return (default: 10)' },
      { name: 'include_details', type: 'boolean', description: 'Include detailed breakdown (default: true)' },
    ],
    examples: [
      'Who are our top 5 consultants?',
      'Show admissions brought by each consultant',
      'Which programs do our consultants refer from which places?',
