...
### SCF pre-session materials — record of already-applied migration (2026-07-30)
...
- Location: `supabase/migrations/20260801100000_scf_session_resources.sql` — **ALREADY LIVE on prod** (objects verified 2026-07-30: 4 resources posted 07-25→07-29). This commit only lands the file on `main` for the record: it previously existed solely on `feat/session-feedback-pre-session-setup`, which caused a false "table missing" alarm during the Classroom Practice L1 data-availability check (stale Mgmt-API replica listing compounded it). Re-running the file is safe (IF NOT EXISTS / OR REPLACE throughout).
