...
const timetableFormSchema = z
  .object({
    ...
    attendance_mode: z
      ...
      .default('period_wise'),
    // class_incharge_id is only mandatory for session_wise (school) timetables;
    // period_wise (college) timetables keep it NULL, matching the DB column
    // (see supabase/migrations/20260610_add_attendance_mode_and_class_incharge.sql).
...
export default function EditTimetablePage() {
  ...
  // Staff list for the Class Incharge picker (session_wise timetables).
  ...
  // attendance_mode locks once attendance exists (the service treats it as an
  // unsafe structural change); class_incharge_id remains editable.
  const attendanceModeLocked = hasAttendance && !isSuperAdmin;
    ...
    const fetchTimetable = async () => {
      ...
      try {
        ...
        form.reset({
          ...
          class_incharge_id: (timetableData as any).class_incharge_id || ''
        });
          ...
          <CardContent className='p-6'>
            <Form {...form}>
              ...
              >
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                              <SelectTrigger>
                                <SelectValue
                                  placeholder={
                                    !watchDegreeId
                                      ? `First select a ${adapt('degree')}`
                                      : `Choose ${adapt('department')}`
                                  }
                                />
                              </SelectTrigger>
                            </FormControl>
                          ...
                          <FormDescription>
                            The {adapt('department')} within the {adapt('degree')}
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                            ...
                            <FormControl>
                              <SelectTrigger>
                                <SelectValue
                                  placeholder={
                                    !watchDepartmentId
                                      ? `First select a ${adapt('department')}`
                                      : `Choose ${adapt('program')}`
                                  }
                                />
                              </SelectTrigger>
                            </FormControl>
                          ...
                          <FormDescription>
                            The specific {adapt('program')} within the {adapt('department')}
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                      )}
                            ...
                            <FormControl>
                              <SelectTrigger>
                                <SelectValue
                                  placeholder={
                                    !watchProgramId
                                      ? `First select a ${adapt('program')}`
                                      : `Choose ${adapt('semester')}`
                                  }
                                />
                              </SelectTrigger>
                            </FormControl>
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                              ...
                              <FormControl>
                                <SelectTrigger>
                                  <SelectValue
                                    placeholder={
                                      !effectiveSemesterId
                                        ? `First select a ${adapt('semester')}`
                                        : `Choose ${adapt('section')}`
                                    }
                                  />
                                </SelectTrigger>
                              </FormControl>
                            ...
                            <FormDescription>
                              The {adapt('section')} for this timetable
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                        )}
                ...
                <div className='border-t pt-4 grid grid-cols-1 gap-6 md:grid-cols-2'>
                  <FormField
                    control={form.control}
                      ...
                      <FormItem>
                        ...
                        <FormDescription>
                          {attendanceModeLocked ? (
                            <span className='text-amber-600 dark:text-amber-400'>
                              Locked — attendance has already been recorded for
                              this timetable.
                            </span>
                          ) : watchAttendanceMode === 'session_wise' ? (
                            <span>
                              Collected only at the first forenoon and first
                              afternoon period; both present = full day, one =
                              half day. Marked by the class incharge.
                            </span>
                          ) : (
                            <span>Attendance is marked for every period.</span>
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                  ...
                  {/* Class Incharge — always shown; required only for session_wise (day-wise) attendance */}
                  {(
                    <FormField
                      control={form.control}
