...
export const GUIDE_PERSONAS: readonly GuidePersona[] = [
  ...
  // Admission CRM has no self-service applicant audience inside the platform,
  // so there is no ungated "learner" baseline lane. The counsellor lane is
  // gated by the front-line operator key (admission.leads.view); the admin
  // lane by the module-configuration key (admission.settings.view).
  "admin",
  "counsellor",
] as const;
