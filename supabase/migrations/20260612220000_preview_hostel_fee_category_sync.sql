-- =============================================================================
-- Preview (dry-run) for the fee-condition category sync
--
-- The Program Eligibility "Sync to Learner Profiles" button now shows a
-- per-learner preview BEFORE the operator commits. fn_preview_hostel_fee_categories
-- mirrors fn_apply_hostel_fee_categories / _bulk EXACTLY but writes nothing:
--   * scope: ACTIVE hostel learners (accommodation code 'hostel'), optional
--     institution filter — including the no-academic-bill learners the bulk
--     sync SKIPS, so the operator sees the full validation picture
--   * per learner: resolved fee, matched condition (reason), proposed room +
--     mess category after gender-name translation, Classic fallback,
--     allocation-wins (room) and overwrite-never-wipe — and whether the row
--     would actually change
--
-- reason values:
--   band_match                  — a fee-band condition matched
--   classic_default_fee_unknown — has academic bill(s) but none tagged to the
--                                 learner's current academic year → Classic
--   classic_default_no_band     — fee known but no active band covers it → Classic
--   no_academic_bill            — sync skips this learner entirely
-- =============================================================================
