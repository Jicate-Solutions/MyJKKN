...
2. Dashboard shows: 65/72 metrics auto-populated, 7 need manual entry
...
- [ ] 40+ data connectors producing metric values from existing tables
...
CREATE TABLE regulatory_frameworks (
  ...
  total_max_score numeric,                           -- e.g., 1000 for NAAC, 100 for NIRF
  description text,
...
CREATE INDEX idx_reg_metric_values_lookup ON regulatory_metric_values(metric_id, institution_id, academic_year);
...
CREATE INDEX idx_reg_submissions_lookup ON regulatory_submissions(framework_id, institution_id, academic_year);
...
├── Day 5: Seed NAAC 2022 Revised framework (7 criteria, ~72 key metrics)
