...
export const GET = withAuth(async (request, auth) => {
  ...
  const phase_id = getStringParam(url, 'phase_id')
  const communication_type = getStringParam(url, 'communication_type')
  const direction = getStringParam(url, 'direction')

  const result = await DiscoveryService.getCommunications({
    page,
    limit,
    client_id,
    solution_id,
    phase_id,
    communication_type: communication_type as any,
...
export const POST = withAuth(async (request, auth) => {
  ...
  const result = await DiscoveryService.createCommunication({
    ...body,
    recorded_by: body.recorded_by ?? auth.user.id,
  })
