...
const ACTION = 'certificate' as const;
...
export default function CertificatePage() {
  ...
  const filtered = useMemo(() => {
    return (registrations ?? []).filter((r: any) => {
      ...
      if (filterStatus === 'issued' && !r.certificate_issued) return false;
      if (filterStatus === 'not_issued' && r.certificate_issued) return false;
      return true;
    });
  ...
  const issuedPct = stats ? Math.round(((stats.certificate_issued ?? 0) / Math.max(stats.total, 1)) * 100) : 0;

  // Block non-admin users
  if (!access.isLoading && !access.canAccessOps) {
    return <MarathonAccessDenied title="Certificate Issuance" eventId={eventId} />;
  }

  return (
    <ContentLayout title={`${eventName} - Certificate Issuance`}>
      <PageBreadcrumb
        items={[
          { label: 'Home', href: '/' },
      ...
      <div className="space-y-5 mt-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold tracking-tight">Certificate Issuance</h1>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-2 rounded-full border bg-card px-4 py-2 shadow-sm">
              ...
              <span className="text-sm font-semibold tabular-nums">
                {stats?.certificate_issued ?? 0}
              </span>
              <span className="text-xs text-muted-foreground">
                / {stats?.total ?? 0}
              </span>
            </div>
            <div className="rounded-full border bg-amber-50 px-3 py-2 shadow-sm">
              <span className="text-sm font-bold text-amber-700 tabular-nums">{issuedPct}%</span>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Card>
                <CardContent className="p-3 text-center">
                  <FileCheck className="h-4 w-4 mx-auto text-amber-500 mb-1" />
                  <p className="text-2xl font-bold tabular-nums text-amber-600">{stats?.certificate_issued ?? 0}</p>
              ...
              <Card>
                <CardContent className="p-3 text-center">
                  ...
                  <p className="text-2xl font-bold tabular-nums">{(stats?.total ?? 0) - (stats?.certificate_issued ?? 0)}</p>
                          ...
                          {paginatedData.length === 0 ? (
                            ...
                            paginatedData.map((r: any) => (
                              <TableRow
                                ...
                                className={cn(
                                  'transition-colors',
                                  r.certificate_issued && 'bg-amber-50/50 dark:bg-amber-950/10'
                                )}
                              >
