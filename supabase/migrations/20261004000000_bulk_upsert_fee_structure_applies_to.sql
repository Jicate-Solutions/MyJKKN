-- 20261004000000_bulk_upsert_fee_structure_applies_to.sql
--
-- The bulk sheet can now carry "Applies To" / "Year of Study" per fee. The RPC
-- already read applies_to out of the payload, but its year fallback was
-- independent of it:
--
--     applies_to           := COALESCE(item, prev, 'every_year')
--     applies_year_of_study := COALESCE(item, prev)            -- <-- unrelated
--
-- That was harmless only because the sheet never sent applies_to: item and prev
-- always moved together. The moment a sheet can change a fee from
-- 'specific_year' to 'every_year', the previous year survives underneath it and
-- afsi_applies_year_chk -- a BICONDITIONAL, `(applies_to = 'specific_year') =
-- (applies_year_of_study IS NOT NULL)` -- rejects the insert. The operator would
-- see a raw Postgres constraint name for an edit that is perfectly legal.
--
-- The year is now DERIVED from the resolved applies_to, the same way due_anchor
-- self-heals when the date it pointed at is gone, and 'specific_year' with no
-- year anywhere fails with a sentence instead of a constraint name.
--
