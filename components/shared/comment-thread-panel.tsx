...
import {
  MentionComposer as Composer,
  MentionText as MentionSpans,
} from '@/components/shared/mention-composer';

// The composer, its "@query" parser and the tagged-name renderer were extracted
// to components/shared/mention-composer.tsx (2026-09-24) when the recruitment
// candidate discussion needed the same tagging. Re-exported here because
// search-taggable-staff.ts and both consumer cards import the type from this
// module; moving that import too would be churn for no gain.
export type { TaggablePerson } from '@/components/shared/mention-composer';
import type { TaggablePerson } from '@/components/shared/mention-composer';
...
function MentionText({ body, mentions }: { body: string; mentions?: ThreadComment['mentions'] }) {
  return <MentionSpans body={body} names={(mentions ?? []).map((m) => m.name)} />;
}
