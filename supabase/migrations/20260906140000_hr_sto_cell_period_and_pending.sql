-- ============================================================================
-- THE PERMISSION COLUMN NAMES ITS MONTH, AND SPLITS OUT PENDING (2026-09-06)
--
-- TWO THINGS WERE TRUE BUT INVISIBLE.
--
-- 1. NO PERIOD ON SCREEN. Short Time Off is budgeted per period -- every live
--    Permission type is total_duration, 120 minutes, limit_period='month' -- and
--    it does NOT carry forward. "45m left of 2h" is therefore unreadable
--    without saying of WHICH month. The window was computed and put in the cell
--    tooltip only, so it was invisible unless hovered.
--
-- 2. PENDING WAS COUNTED BUT NOT SHOWN. hr_trig_sto_enforce_limits,
--    hr_sto_usage and this RPC all count 'pending' and 'escalated' beside
--    'approved' -- an undecided request is budget already committed, which is
--    correct. But the grid showed one merged "used" figure, and for September
--    2026 that figure is 29 pending requests against 7 approved. An admin
--    reading it as approved time is reading it almost entirely wrong.
--
-- Both are now returned: sto_types carries the TYPE-level window for the column
-- header, and each cell carries requests_pending / minutes_pending alongside the
-- committed totals. Per-cell period_start/period_end stay, because an
-- hr_leave_type_assignments row can override limit_period for one person and
-- the header window would then be wrong for them.
--
-- Nothing about ENFORCEMENT changes. The budget is spent by exactly the same
-- requests as before; only what the screen admits changes.
--
-- Body is 20260905120100 plus those additions.
--
