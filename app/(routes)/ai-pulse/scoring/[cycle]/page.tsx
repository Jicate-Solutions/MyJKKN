...
/**
 * AI Pulse — Scoring (thin redirect to existing event evaluate surface).
 *
 ...
 * before forwarding to the existing `/startup-studio/events/[id]/evaluate`
 * page so all auth, RLS, evaluator-role gates and UI continue to work
 * unchanged. If no matching cycle exists, render 404.
 *
 * Spec: specs/myjkkn-ai-pulse-spec.md (events-extension v3).
 */
export default async function AiPulseScoringRedirect({ params }: Props) {
  ...
  redirect(`/startup-studio/events/${event.id}/evaluate`);
}
