...
export const LOOP_GOVERNANCE_ROUTINES: AIRoutine[] = [
  ...
  },
  {
    ...
    whatItDoes:
      "Gives the consultants loop the scheduled run it never had. The measurer has existed since 2026-08-26 (fn_consultants_measure_conversion, migration 20261003010000) and a weekly known-delta regress proves it, but nothing ever called it — consultant_conversion_measurements has stayed empty, so the loop produced no reading of its own. Each Monday this route makes one RPC, which reads the consultant attribution ledger once and, per consultant, rates the 30-day WINDOW against that consultant's OWN pre-window baseline with the SAME estimator (conversion = current_stage IN ('enrolled','confirmed'); rate = conversions/attributions*100, 2 dp), NULLing either side that sits below the de-noise floor. It upserts one row per (consultant, window) into consultant_conversion_measurements and returns them. The route then averages the window conversion rates of the consultants that cleared the floor into the run's headline number and records it against the loop's bar.",
    configKnobs:
