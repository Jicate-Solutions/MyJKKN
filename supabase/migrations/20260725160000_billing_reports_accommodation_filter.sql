-- ============================================================================
-- 20260725160000 — accommodation filter + cohort institution-scope fix
-- ============================================================================
-- Supersedes 20260725103000. Two changes:
--
-- 1. NEW FILTER: p_accommodation_codes text[] on the cohort helper and all six
--    RPCs. Multi-select over accommodation_types.code
--    (hostel | dayscholar | pg | not_applicable). NULL accommodation_type_id is
--    EXCLUDED whenever any code is selected -- 501 learners have no
--    accommodation recorded, and "not recorded" is deliberately NOT treated as
--    "not applicable" (which is an explicit answer, 1 learner). This is a
--    DELIBERATE asymmetry with the scheme filter's 'other' bucket, which DOES
--    sweep up NULL scholarship_type. Do not "harmonise" them.
--
-- 2. FIX (final review, I3): billing_report_student_cohort no longer takes or
--    filters on p_institution_ids. Each RPC already scopes the FACT table
--    (b/r/inv.institution_id = ANY(v_inst)), which is the authoritative scope.
--    Filtering the LEARNER's institution as well DROPPED rows outright wherever
--    a learner's profile institution differs from their bill's institution --
--    measured at 2 students today. Symptom was a vanishing row, not merely a
--    mislabelled one, and it bit single-institution views too.
--
-- WHY THE DROPS BELOW ARE MANDATORY:
--   CREATE OR REPLACE with a CHANGED parameter list does not replace anything --
--   it creates a SECOND OVERLOAD. Every parameter here has a DEFAULT, so
--   PostgREST could not then disambiguate and RPC calls would fail with
--   "function is not unique" (42725), breaking the already-live functions.
--   Safe to drop: the frontend is unreleased, nothing shipped calls them.
--   POST-APPLY MUST VERIFY: exactly 7 such functions exist, NOT 14.
-- ============================================================================

DROP FUNCTION IF EXISTS public.billing_report_student_cohort(uuid[], uuid, uuid, uuid, uuid, uuid, text[]);
DROP FUNCTION IF EXISTS public.get_billing_reports_outstanding(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], uuid, date, date, int, int);
DROP FUNCTION IF EXISTS public.get_billing_reports_collection(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], uuid, date, date, int, int);
DROP FUNCTION IF EXISTS public.get_billing_reports_invoices(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], uuid, date, date, int, int);
DROP FUNCTION IF EXISTS public.get_billing_reports_discounts(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], uuid, date, date, int, int);
...
AS $$
  ...
  LEFT JOIN public.accommodation_types at ON at.id = lp.accommodation_type_id
  WHERE (p_degree_id     IS NULL OR lp.degree_id     = p_degree_id)
    ...
    AND (
      p_accommodation_codes IS NULL OR cardinality(p_accommodation_codes) = 0
      OR COALESCE(at.code, '') = ANY(p_accommodation_codes)
    );
$$;

REVOKE EXECUTE ON FUNCTION public.billing_report_student_cohort(uuid, uuid, uuid, uuid, uuid, text[], text[]) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.billing_report_student_cohort(uuid, uuid, uuid, uuid, uuid, text[], text[]) TO authenticated, service_role;
...
REVOKE EXECUTE ON FUNCTION public.get_billing_reports_outstanding(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.get_billing_reports_outstanding(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) TO authenticated, service_role;
...
REVOKE EXECUTE ON FUNCTION public.get_billing_reports_collection(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.get_billing_reports_collection(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) TO authenticated, service_role;
...
BEGIN
  ...
  JOIN public.billing_report_student_cohort(
         p_degree_id, p_department_id, p_program_id,
         p_semester_id, p_section_id, p_schemes, p_accommodation_codes) c ON c.student_id = inv.student_id
...
REVOKE EXECUTE ON FUNCTION public.get_billing_reports_invoices(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.get_billing_reports_invoices(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) TO authenticated, service_role;
...
REVOKE EXECUTE ON FUNCTION public.get_billing_reports_discounts(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.get_billing_reports_discounts(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) TO authenticated, service_role;
...
REVOKE EXECUTE ON FUNCTION public.get_billing_reports_refunds(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.get_billing_reports_refunds(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date, int, int) TO authenticated, service_role;
...
BEGIN
  ...
  FROM public.billing_report_student_cohort(
    p_degree_id, p_department_id, p_program_id,
    p_semester_id, p_section_id, p_schemes, p_accommodation_codes);
...
REVOKE EXECUTE ON FUNCTION public.get_billing_reports_dashboard(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.get_billing_reports_dashboard(uuid[], uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, text[], text[], uuid, date, date) TO authenticated, service_role;
