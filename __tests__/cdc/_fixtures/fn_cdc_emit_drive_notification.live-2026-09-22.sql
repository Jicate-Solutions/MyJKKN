-- main's CURRENT definition of fn_cdc_emit_drive_notification, renamed so it can
-- be installed beside the reworked version as a CONTROL. Do not edit; it exists
-- to prove the test can tell main's behaviour from this PR's.
--
-- PROVENANCE, stated exactly: this is the body committed on jicate/main at
-- f7a67e7a089a365946db90db03ad12a759aa5aea
-- in supabase/migrations/20260919100000_cdc_drive_notification_emitter_restore.sql,
-- read out of git on 2026-09-22. It was NOT read from the production catalogue
-- in this session - this lane has no database access - so it is what main
-- installs on apply, not a reading of what the live catalogue holds right now.
--
-- It REPLACES _fixtures/fn_cdc_emit_drive_notification.live-2026-09-18.sql,
-- which recorded the body 20260915100000 had left running (no attendance_day
-- branch, learner links pointing at the coordinator page, one combined
-- cancelled row). That file is kept, renamed, as
-- _fixtures/fn_cdc_emit_drive_notification.regressed-2026-09-15.sql - it is now
-- the control that proves the regression cases in the suite can actually fail.
--
-- The ACL block from the migration is deliberately omitted: this is a control
-- under another name, and the grant assertions in the suite are about the real
-- function.
CREATE OR REPLACE FUNCTION public.fn_ctl_main_emit_2026_09_22(p_drive_id uuid, p_from_state text, p_to_state text, p_actor uuid)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
