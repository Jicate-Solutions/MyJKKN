...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const unreachable = ownersWithoutConfig(allOwners, configs);
    ...
    if (configs.length === 0) {
      return NextResponse.json({
        ...
        owners_without_config: unreachable,
        total_owner_rows: allOwners.length,
        message:
          unreachable.length > 0
            ? `No digests to send — but ${unreachable.length} owner(s) have no digest config row, so they could not be reached even if this route were armed.`
