...
- `supabase/migrations/20260808110000_health_tournament_permissions_two_step_approval.sql` — **NOT applied to any database — Director-gated apply.** Validated only inside a `BEGIN .. ROLLBACK` batch on prod via the Mgmt-API `database/query` endpoint (20 assertions green, then a separate call confirmed prod unchanged: 0 rows, column absent, widened CHECKs absent, new policies absent, role keys back to their prior values). Unblocks a flow that has been **dead since 2026-04-13**: `HealthSportsService.approvePermissionStep()` shipped with ZERO callers, so `health_tournament_permissions` had never held a single row and off-campus permission was still circulated on paper. Director-locked path is **two parties** — the Physical Director FILES for the whole squad, the PRINCIPAL approves; `step3_principal_*` is THE approval step. Steps 1/2/4 get the new status **`'not_required'`** (CHECK widened on those three only — step3 is deliberately left at pending|approved|rejected so 'not_required' can never be written to the step a human decides), because the pre-existing CHECKs allowed only pending|approved|rejected, making the sole representable choices a permanent false "pending" or a fabricated "approved". Steps 1 and 4 could not stay in the path at all: the **only** `sports_coordinator` holder is the Physical Director who files, so he would approve his own request twice, and `pe_director`/`physical_director` do not exist as roles (0 rows in `custom_roles`). 🪤 **`health_tournament_permissions_self` was never self-scoped** — its `JOIN profiles p ON ((lp.id = p.id) OR (p.id = auth.uid())) WHERE p.id = auth.uid()` pins `p` to the caller in the WHERE, making the OR unconditionally true so every `learners_profiles` row joins: measured on prod with a real profile id substituted for `auth.uid()`, the subquery returned **7156 of 7156** learners, and the policy is `FOR ALL`, so it granted UPDATE and DELETE on every row too. Not an exposure only because the table is empty — the UI landing in this PR is what fills it, so the fix ships here as `learner_id = get_my_learner_id()` (an existing STABLE SECURITY DEFINER helper, referenced not redefined, so no new anon grant is created). Approver policy replaces a `get_current_user_role() = ANY(ARRAY[...])` list that both **omitted `principal` entirely** (the one approver could not read or write any row — an inbox alone renders empty) and read the LEGACY single-role field, so a principal arriving through `user_roles` was invisible; now `COALESCE(is_super_admin(),false) OR COALESCE(is_admin(),false) OR COALESCE(user_has_permission('health.sports.approve'),false)`, every predicate COALESCEd because a SECDEF guard returning NULL falls through. A **separate filer door** (`_filer_insert` + `_filer_select`, keyed on the new `health.sports.file_request`, INSERT/SELECT only — no UPDATE, no DELETE, and `filed_by_profile_id = auth.uid()`) exists because neither other policy admits the filer: the approver policy needs the key the filer must NOT hold (D1 — that is the whole point of two parties) and the self policy needs a learner row a team member does not have. **Section 6 is the only part that changes DATA and it is what decides whether the feature works at all** — widening a permission takes four layers, and RLS alone reaches none of the others: `principal` is granted the new `health.sports.approve`, and `sports_coordinator` is granted `health.sports.file_request` **plus `learners.profiles.view`**, because that role has `learners.view`, `learners.profiles.view` AND `learners.admissions.view` all **= false** (VALUES read on prod — note `permissions ? 'key'` tests EXISTENCE and reports true for all three, which is why the values were read) and `learners_profiles`' SELECT policy requires one of them, so without it the squad picker returns zero learners for the very person meant to file. Deliberately NOT granted: `health.sports.approve` to `sports_coordinator` (the D1 boundary) and `health.sports.file_request` to `principal`. Written as `jsonb_set` on the FLAT key because that is what `user_has_permission()` reads (`(cr.permissions->>permission_name)::boolean`). No explicit `BEGIN`/`COMMIT` in the file, deliberately — repo convention, and an inner COMMIT would turn any future `BEGIN .. ROLLBACK` dry-run of this file into a live apply.
