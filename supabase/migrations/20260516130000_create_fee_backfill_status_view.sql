-- ============================================================================
-- 20260516130000 — vw_learners_profile_fee_backfill_status
-- ============================================================================
-- Classifies every admitted+legacy learners_profiles row into a resolution
-- bucket so the new "Fees Setup Pending" tab can surface what's blocking auto-
-- fetch of a fee structure, and so the bulk-adopt migration can target only
-- the rows it can safely resolve.
--
-- Match logic mirrors admission_resolve_fee_items_for_lead() byte-for-byte:
--   strict   = all 8 dims + active status + community via junction
--   relaxed  = same minus quota_id (Tier 2 in the backfill plan)
--
-- security_invoker=true → RLS on learners_profiles flows through, so a
-- counselor sees only their institution's rows; a super_admin sees all.
-- ============================================================================
...
WITH base AS (
    SELECT
        ...
        lp.created_at,
        lp.updated_at,
        -- Ordered list of missing FK columns (most-impactful first)
        ARRAY_REMOVE(ARRAY[
...
relaxed_matches AS (
    -- Tier 2: drop quota_id from join, keep community gate
    SELECT
        b.learner_id,
        COUNT(*)                                                 AS cnt,
        ARRAY_AGG(afs.id ORDER BY afs.updated_at DESC NULLS LAST) AS all_ids
...
SELECT
    ...
    -- Resolution status — single, authoritative classification per row.
    -- Order of evaluation matters: missing FKs win over match-count buckets.
    CASE
    ...
    -- Single matched structure id (tier1 or tier2 unambiguous).
    -- Use array_agg[1] because Postgres has no native MIN(uuid) aggregate
    -- and we want deterministic ordering (newest updated wins on ties).
    CASE
    ...
    -- Candidate list when ambiguous (so triage UI can present a picker)
    CASE
...
COMMENT ON VIEW public.vw_learners_profile_fee_backfill_status IS
    'Classification of admitted+legacy learners_profiles by their fee-structure resolvability. Powers the "Fees Setup Pending" tab and the bulk-adopt migration. RLS-aware (security_invoker).';

-- View permissions follow base-table grants thanks to security_invoker;
-- authenticated needs explicit SELECT on the view object itself.
