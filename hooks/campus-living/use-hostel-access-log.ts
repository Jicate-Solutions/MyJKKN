...
export function useHostelAccessLogs(institutionId: string | undefined, filters?: Record<string, unknown>) {
  const { isSuperAdmin } = usePermissions();
  return useQuery({
    queryKey: hostelAccessLogKeys.list({ institutionId, ...filters }),
    queryFn: () => CampusLivingAccessLog.getAccessLogs(isSuperAdmin ? undefined : institutionId, filters),
