...
export function PaymentAccountsManager() {
  ...
  const [rotateFor, setRotateFor] = useState<{ institutionId: string; label: string | null } | null>(null);
  ...
  async function runTest(account: RazorpayAccountSummary) {
    ...
    try {
      const result = await test.mutateAsync(account.institutionId);
      if (result.success) {
  ...
  async function confirmDeactivate() {
    ...
    try {
      await deactivate.mutateAsync(deactivateTarget.institutionId);
      toast.success('Account deactivated — institution falls back to the common account');
    } catch (err) {
                        ...
                        {canManage && a.isActive && (
                          <>
                            <Button variant='ghost' size='sm' className='h-7 gap-1 px-2 text-xs'
                              onClick={() => { setRotateFor({ institutionId: a.institutionId, label: a.accountLabel }); setAddOpen(true); }}>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {deactivateTarget && (
                <>New payments for <strong>{instName(deactivateTarget.institutionId)}</strong> will use the common (env) account.
                Existing transactions remain verifiable (they pin their original account). You can add a new account anytime.</>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
