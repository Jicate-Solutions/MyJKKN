...
- Re-keys four April-seed `sh_accreditation_metrics` rows (metric_type='NAAC') whose codes don't match the Binary Accreditation Framework 2024 (names verified against the seed before re-keying): `1.3.1`→`1.6.1` (cross-cutting issues → IKS/cross-cutting family; runs FIRST to free 1.3.x), `1.2.1`→`1.3.1` (flexibility content; Binary 1.2 = stakeholder feedback), `2.3.1`→`7.10.1` (faculty retention → Attribute 7 Governance, category updated), `6.2.1`→`6.5.1` (sports → Binary 6.5; cricket-exclusion note preserved in calculation_method).
