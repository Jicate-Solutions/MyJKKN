...
-- Updated: 2025-02-14 - Added role check constraint for all application roles
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    -- Added: 2025-01-27 - Required by create_preregistered_profile() RPC and profiles_insert_preregistered RLS policy
...
CREATE TABLE IF NOT EXISTS public.learners_profiles (
    ...
    original_student_id UUID UNIQUE, -- Source student.id if migrated from students (UNIQUE required for upsert ON CONFLICT)
...
-- =====================================================
-- SECTION: SESSION TRACKING & ACTIVITY LOGS
-- Added: 2026-02-17 - Resolves auth callback errors after login
-- =====================================================

-- user_sessions: Tracks user login sessions for analytics and security
-- Note: institution_id has no FK — public.institutions table not in IMS schema
...
-- Updated: 2026-02-17 - Removed duplicate table definition (wrong column names: activity_type, activity_description).
-- Correct definition with action_type, description, resource_name, institution_id, request_url, request_method
-- is in SECTION 13 above. Migration 20260217_fix_activity_log_column_schema renamed the columns in the DB.
