-- Calendar Phase 4: room/resource reservations aggregation               2026-06-23
-- Adds 1 read-only UNION branch (Source 10) for resource_reservations.
-- Institution via resource_id -> resources.institution_id join (resource_reservations
-- has NO direct institution_id column). status in ('approved','completed').
-- Non-sensitive/institutional → no people-leave gate. ADDITIVE: 9 prior branches unchanged.
-- Exams deferred (external COE, no local table); class timetables excluded
-- (recurring weekday+period JSONB, ~280K expanded sessions). Mirror deferred.
...
BEGIN
  ...
  UNION ALL
  -- Source 6: general events (Phase 3) -------------------------------------
  SELECT
  ...
  UNION ALL
  -- Source 7: Learners Council events (Phase 3) ----------------------------
  SELECT
  ...
  UNION ALL
  -- Source 8: Startup Studio events (Phase 3) ------------------------------
  SELECT
  ...
  UNION ALL
  -- Source 9: BOS board meetings (Phase 3; institutions_id col) -------------
  SELECT
  ...
  UNION ALL
  -- Source 10 (NEW): room/resource reservations (institution via resources join)
  SELECT
