// app/api/organizations/sections/import/route.ts
...
} from '@/lib/utils/mappings/section-excel-mappings';

/**
 * POST /api/organizations/sections/import
 *
 * Bulk import sections from Excel file with 5-level cascading validation
 *
 * Features:
...
/**
 * Parse a single Excel row into a section object
 */
function parseExcelRow(
  row: ExcelJS.Row,
  rowNumber: number
): { data: ParsedSectionRow | null; errors: ImportError[] } {
  const errors: ImportError[] = [];

  // Extract cell values (columns A-G) - Using NAMES instead of codes/IDs
  ...
  const sectionName = getCellValue(row.getCell(6));
  ...
  // Construct section object - using NAMES (keep original case, just trim)
  const sectionData: ParsedSectionRow = {
  ...
  // Zod validation
  try {
    sectionRowSchema.parse(sectionData);
  } catch (err) {
  ...
  return { data: sectionData, errors: [] };
}
  ...
  try {
    ...
    if (userError || !user) {
      return NextResponse.json(
        { error: 'Unauthorized', message: 'You must be logged in to import sections' },
        { status: 401 }
      );
    }
    ...
    const parsedRows: ParsedSectionRow[] = [];
    ...
    parsedRows.forEach((row, index) => {
      const key = `${row.semester_name}|${row.section_name}`;
      if (!nameMap.has(key)) {
        nameMap.set(key, []);
      }
      nameMap.get(key)!.push(index + 2); // +2 for header row
    });

    const duplicatesInFile = Array.from(nameMap.entries())
      .filter(([_, indices]) => indices.length > 1)
      .map(
        ([key, indices]) =>
          `Section "${key.split('|')[1]}" in semester "${key.split('|')[0]}" appears in rows: ${indices.join(', ')}`
      );
    ...
    if (duplicatesInFile.length > 0) {
      const result: ImportResult = {
        ...
        duplicateNames: duplicatesInFile
      };
      return NextResponse.json(result, { status: 400 });
    }
    ...
    if (instError) {
      console.error('[sections/import] Institution lookup error:', instError);
      return NextResponse.json(
        {
    ...
    // Create name-to-UUID mappings for institutions
    ...
    if (degreeError) {
      console.error('[sections/import] Degrees lookup error:', degreeError);
      return NextResponse.json(
        {
          error: 'Database error',
    ...
    if (deptError) {
      console.error('[sections/import] Departments lookup error:', deptError);
      return NextResponse.json(
        {
          error: 'Database error',
    ...
    if (progError) {
      console.error('[sections/import] Programs lookup error:', progError);
      return NextResponse.json(
        {
          error: 'Database error',
    ...
    if (semesterError) {
      console.error('[sections/import] Semesters lookup error:', semesterError);
      return NextResponse.json(
        {
          error: 'Database error',
    ...
    const semesterKeyToIdMap = new Map<string, string>();
    semesters?.forEach((sem) => {
      const key = `${sem.program_id}|${sem.semester_name}`;
      semesterKeyToIdMap.set(key, sem.id);
    });

    // Fetch existing sections for duplicate checking
    const semesterIds = semesters?.map((s) => s.id) || [];
    const { data: existingSections, error: sectionError } = await supabase
      .from('sections')
      .select('semester_id, section_name')
      .in('semester_id', semesterIds);

    if (sectionError) {
      console.error('[sections/import] Sections lookup error:', sectionError);
      return NextResponse.json(
        {
          error: 'Database error',
          message: 'Failed to check existing sections'
        },
        { status: 500 }
      );
    }
    ...
    const existingSectionKeys = new Set(
      existingSections?.map((s) => `${s.semester_id}|${s.section_name}`) || []
    );
    ...
    parsedRows.forEach((row, index) => {
      ...
      const semesterKey = `${progUuid}|${row.semester_name}`;
      const semesterUuid = semesterKeyToIdMap.get(semesterKey);
      ...
      // Check section doesn't already exist in database
      const sectionKey = `${semesterUuid}|${row.section_name}`;
      if (existingSectionKeys.has(sectionKey)) {
        validationErrors.push({
          row: rowNumber,
          field: 'section_name',
          message: `Section "${row.section_name}" already exists in semester "${row.semester_name}"`
        });
        return;
      }
      ...
      row.semester_uuid = semesterUuid;
    });
    ...
    const sectionsToInsert = parsedRows.map((row) => ({
    ...
    if (insertError) {
      console.error('[sections/import] Insert error:', insertError);
      return NextResponse.json(
        {
          error: 'Database error',
          message: `Failed to insert sections: ${insertError.message}`
        },
        { status: 500 }
      );
    }
    ...
    const result: ImportResult = {
      success: true,
      successCount: insertedSections?.length || 0,
      errorCount: 0,
      totalRows: parsedRows.length,
      errors: []
    };
    ...
    console.error('[sections/import] Unexpected error:', error);
