...
export async function GET(request: NextRequest) {
  try {
    ...
    if (searchTerm) {
      query = query.or(
        `semester_code.ilike.%${searchTerm}%,semester_name.ilike.%${searchTerm}%`
      );
    }
    ...
    if (!semesters || semesters.length === 0) {
      return NextResponse.json(
        { error: 'No semesters found matching your criteria' },
        { status: 404 }
      );
    }
    ...
    const formattedData = semesters.map((semester) => {
      return {
        ...
        institutionName: semester.institutions?.name || '',
        ...
        updatedAt: semester.updated_at
          ? new Date(semester.updated_at).toLocaleString()
          : ''
      };
    });
    ...
    const filename = `semesters-export-${dateStr}.${
    ...
    if (exportFormat === 'json') {
      ...
      // Add title row
      worksheet.addRow(['Semesters Export']).font = {
        bold: true,
        size: 16
      };
      ...
      worksheet.columns = [
        ...
        { header: 'Semester Name', key: 'semesterName', width: 30 },
        { header: 'Semester Type', key: 'semesterType', width: 12 },
