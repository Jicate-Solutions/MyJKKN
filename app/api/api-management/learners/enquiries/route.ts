...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Execute query
    const { data: enquiriesRaw, error, count } = await query;
    ...
    const enquiries = (enquiriesRaw ?? []).map((row: any) => {
      ...
      const { admission_year_obj: _ay, quota_obj: _q, community_obj: _c, caste_obj: _cs, ...rest } = row;
      return { ...rest, admission_year: ayObj?.program_start_year ?? null, quota: quotaObj?.name ?? null, community: communityObj?.code ?? null, caste: casteObj?.name ?? null };
    });
