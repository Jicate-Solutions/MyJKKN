...
export class TrainingService extends BaseService {
  ...
  static async getPrograms(
    ...
    let query = this.supabase.from('sh_training_programs')
      .select(
        `
        *,
  ...
  static async getProgramById(id: string): Promise<TrainingProgramWithDetails | null> {
    const { data, error } = await this.supabase.from('sh_training_programs')
      .select(
        `
        *,
        solution:sh_solutions(
  ...
  static async getSessions(
    ...
    let query = this.supabase.from('sh_training_sessions')
      .select(
        `
        *,
  ...
  static async getSessionById(id: string): Promise<TrainingSessionWithDetails | null> {
    const { data, error } = await this.supabase.from('sh_training_sessions')
      .select(
        `
        *,
        program:sh_training_programs(
  ...
  static async createSession(input: CreateTrainingSessionInput): Promise<TrainingSession> {
    ...
    if (!sessionNumber) {
      const { data: existing } = await this.supabase.from('sh_training_sessions')
  ...
  static async claimSession(
    ...
    const { data: existing } = await this.supabase.from('sh_cohort_assignments')
      .select('id')
  ...
  static async removeAssignment(sessionId: string, cohortMemberId: string): Promise<void> {
    const { error } = await this.supabase.from('sh_cohort_assignments')
      .delete()
      .eq('session_id', sessionId)
      .eq('cohort_member_id', cohortMemberId);
  ...
  static async completeSession(
    ...
    const { data: session, error: sessionError } = await this.supabase.from('sh_training_sessions')
      .update({
    ...
    const { data: assignments } = await this.supabase.from('sh_cohort_assignments')
    ...
    if (assignments && assignments.length > 0) {
      for (const assignment of assignments) {
        ...
        if (member) {
          ...
          await this.supabase.from('sh_cohort_members')
            .update({
        ...
        await this.supabase.from('sh_cohort_assignments')
          .update({
