...
//   - Fix 1+2: consultant lookup via direct user_id query (no scan-and-find).
//   - Fix 3: transaction.final_amount -> transaction.gross_amount (the
//     `final_amount` column does not exist in supabase/migrations/admission/
//     003_consultant_tables.sql; the type def at types/education-consultants.ts:286
//     was fiction).
//   - Fix 4: transaction.paid_at -> transaction.payment_date (date column,
//     not timestamp, so we drop time formatting).
...
// Type augmentation for the actual DB shape — the central
// ConsultantCommissionTransaction type still lies about column names. Engine PR
// keeps the legacy type in place and narrows at the row site so we don't break
// other consumers.
type CommissionRow = ConsultantCommissionTransaction & {
...
export default function ConsultantCommissionsPage() {
  ...
  // Load consultant data — Fix 1+2: direct user_id lookup.
  useEffect(() => {
    async function loadConsultant() {
      ...
      try {
        ...
        const { data: myConsultant } = await supabase
        ...
        setConsultant((myConsultant as EducationConsultant) || null);
      } catch (err) {
  ...
  // Calculate from transactions if available — Fix 3 applied in reducer.
  const transactionList = (transactions?.data || []) as CommissionRow[];
  const transactionStats = transactionList.reduce(
    (acc: { pending: number; earned: number; approved: number; paid: number }, t: CommissionRow) => {
      const amount = t.net_amount || t.gross_amount || 0;
            ...
            <>
              <div className="rounded-md border overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {transactionList.map((transaction: CommissionRow) => {
                      ...
                      return (
                        <TableRow key={transaction.id}>
                          ...
                          <TableCell className="text-right">
                            <div>{formatCurrency(transaction.gross_amount ?? 0)}</div>
                          ...
                          <TableCell>
                            ...
                            {transaction.status === 'paid' && transaction.payment_date && (
                              <p className="text-xs text-muted-foreground mt-1">
                                Paid: {new Date(transaction.payment_date).toLocaleDateString('en-IN', {
                                  day: 'numeric',
                                  month: 'short',
                                  year: 'numeric',
                                })}
