...
export function MembersClient({
  ...
  const handleExport = () => {
    exportMutation.mutate(
      ...
      {
        onSuccess: (data) => {
          const headers = ['Name', 'Position', 'Institution', 'Term', 'Status', 'Appointed Date', 'Notes'];
          const rows = data.map((m: any) => [
            m.user?.full_name || 'Unknown',
            m.position?.title || '-',
            institutions.find((i) => i.id === m.institution_id)?.name || m.institution_id || '-',
            m.term?.name || '-',
            m.status,
            m.appointed_at ? new Date(m.appointed_at).toLocaleDateString('en-IN') : '-',
            m.appointment_notes || '',
          ]);
          ...
          a.href = url;
          a.download = `lc-members-${new Date().toISOString().split('T')[0]}.csv`;
          a.click();
