/**
 * B2A — Maturity assessment (audit cycles) read-only list.
 * GET /api/b2a/maturity-assessment
 *
 * NOTE: audit_cycles has institution_ids (uuid[]) — array of institutions.
 * When institutionId is bound on the API key, filter to rows that contain it.
 */
...
const MODULE_KEY = 'maturity-assessment' as const;
const TABLE_NAME = 'audit_cycles';
const ENDPOINT = '/api/b2a/maturity-assessment';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    let query = context.supabase
      ...
      .range(from, to);
    // institution_ids is a uuid[] — filter using `contains` semantics.
    if (institutionId) query = query.contains('institution_ids', [institutionId]);
