-- Fix attendance reports data consistency between table and details pages
-- Issue: Table shows multiple periods but doesn't pass period_id consistently to details
-- Solution: Add period_id to return type and ensure consistent data display

-- First, update get_attendance_report_list function to include period_id
...
RETURNS TABLE(
    id TEXT,
    period_id TEXT,  -- ADDED: Include period_id for consistent navigation
...
BEGIN
    ...
    -- Return paginated results with period_id included
    RETURN QUERY
    WITH main_results AS (
        SELECT 
            sa.id::TEXT as id,
            period_data.period_id::TEXT as period_id,  -- ADDED: Return period_id
            sa.attendance_date,
            ...
            -- FIXED: Use course info from attendance_data first, then fallback to courses table
            COALESCE(
                NULLIF(period_data.course_name::TEXT, ''), 
                NULLIF(c.course_name::TEXT, ''), 
                'Unknown Course'
            ) as course_name,
            COALESCE(
                NULLIF(c.course_code::TEXT, ''), 
                'N/A'
            ) as course_code,
            ...
            -- FIXED: Show assigned faculty from attendance_data, not marked_by
            COALESCE(
                NULLIF(period_data.faculty_name::TEXT, ''), 
                'Unknown Faculty'
            ) as faculty_name,
