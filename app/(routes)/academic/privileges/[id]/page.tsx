...
export default function PrivilegeGroupDetailPage() {
  ...
  // Committee tab state
  const { data: reviewers, isLoading: reviewersLoading } = useGroupReviewers(groupId);
  const addReviewer = useAddGroupReviewer(groupId);
  const removeReviewer = useRemoveGroupReviewer(groupId);
  const [reviewerSearch, setReviewerSearch] = useState('');
  const { data: searchResults, isLoading: searchLoading } = useSearchReviewerCandidates(
    groupId,
    reviewerSearch
  );
  ...
  const reviewersList: any[] = Array.isArray(reviewers) ? reviewers : [];
  ...
  const handleAddReviewer = (profileId: string) => {
    addReviewer.mutate(profileId, {
      onSuccess: () => {
        setReviewerSearch('');
      },
    });
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="members">
          <TabsList>
            ...
            <TabsTrigger value="committee" className="gap-1.5">
          ...
          <TabsContent value="committee" className="mt-4">
            <div className="space-y-4">
              {/* Review Renewals Button */}
              <div className="flex justify-end">
                <Button
                  onClick={() => router.push(`/academic/privileges/${groupId}/renewals`)}
                >
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">
                    Assigned Reviewers ({reviewersList.length})
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {reviewersLoading ? (
                    ...
                    <div className="text-center py-8">
                      <UserCog className="h-8 w-8 text-muted-foreground/40 mx-auto mb-3" />
                      <p className="text-sm text-muted-foreground">
                        No reviewers assigned yet. Add reviewers below.
                      </p>
                    </div>
                  ) : (
                    <div className="rounded-md border">
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">Add Reviewer</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    ...
                    {searchLoading && reviewerSearch.length >= 2 && (
                    ...
                    {reviewerSearch.length >= 2 && !searchLoading && searchResults && searchResults.length === 0 && (
                        ...
                        {searchResults.map((profile: any) => (
                          ...
                          >
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              onClick={() => handleAddReviewer(profile.id)}
