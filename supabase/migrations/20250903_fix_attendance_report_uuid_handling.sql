-- Fix get_attendance_report_list function to handle empty course_id values properly
-- Issue: RPC fails with "invalid input syntax for type uuid: ''" when course_id is empty string
...
BEGIN
    ...
    -- Fixed base query with proper UUID handling for empty/null course_id values
    base_query := '
        SELECT DISTINCT
            sa.id::TEXT as id,
            sa.attendance_date,
        ...
        CROSS JOIN LATERAL (
            SELECT 
                ...
                -- FIXED: Safe UUID casting with proper NULL/empty string handling
                CASE 
