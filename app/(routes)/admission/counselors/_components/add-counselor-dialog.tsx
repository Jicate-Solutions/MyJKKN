  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      if (userType === 'learner') {
        ...
        // Reference: is_pre_registered=true means the profile was stubbed ahead
        // of the user signing in; it flips to false on first OAuth/email login.
        let query = supabase
          .from('profiles')
          .select('id, full_name, email, phone_number, role')
          .eq('institution_id', existInstitutionId)
          .in('role', ['faculty', 'hod', 'staff', 'digital_coordinator'])
          // "NOT is_pre_registered IS TRUE" -> matches both false and NULL,
          // which is what we want (activated users only, defensive on nullable column).
