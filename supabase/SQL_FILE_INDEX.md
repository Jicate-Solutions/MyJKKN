...
### Accreditation — loop→AQAR evidence rollup, NAAC 6.5 IQAC bridge (PR 1/2) — 2026-07-09
- Seeds 3 NAAC Criterion 6.5 metric rows in `sh_accreditation_metrics` (6.5.1 reserved / 6.5.2 teaching loops / 6.5.3 improvement initiatives; idempotent on (metric_type, metric_code); existing 6.1.1/6.2.1 "Attribute" taxonomy untouched).
- `fn_accreditation_rollup_loop_evidence()` (VOLATILE SECURITY DEFINER, service_role ONLY — GRANT-level gate exactly like the SCF loop RPCs, 20260630160000): idempotently upserts one `quality_evidence_mappings` row per MEASURED loop cycle — `scf_ai_suggestions` domain=session_feedback → 6.5.2 loop_key `scf_teaching` (incl. Better/Same/Worse vote counts from `scf_note_resolution_votes`), `induction_session_effectiveness` → 6.5.2 `induction_session`, `scf_ai_suggestions` domain=induction → 6.5.3 `induction_playbook`, `mess_menu_recommendations` → 6.5.3 `mess_menu` (loop dark, 0 rows fine). ON CONFLICT (source_table, source_id, body_code, metric_code) refreshes metadata+mapped_at; never clobbers is_auto=false manual mappings. Returns per-loop jsonb counts (+ `count` total for the dispatcher summary).
