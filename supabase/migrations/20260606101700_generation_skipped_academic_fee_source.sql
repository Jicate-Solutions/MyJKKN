-- Corrective migration for campus_living_generate_hostel_year_bills:
--   Stamp fee_source='academic' on SKIPPED academic items (data-consistency fix).
--   The academic loop's PROPOSED path adds fee_source, but the SKIPPED path pushed
--   the raw item with no fee_source, so the per-learner generation UI crashed when
--   rendering a skipped academic line (feeSourceLabel on undefined). Make skipped
--   academic items carry fee_source='academic', uniform with proposed.
--   The hostel loop's skipped path is unchanged — those items already carry
--   fee_source from campus_living_resolve_hostel_fee.
