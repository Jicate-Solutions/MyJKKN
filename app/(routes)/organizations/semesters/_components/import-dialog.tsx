...
export function ImportDialog({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `Successfully imported ${data.successCount} ${adapt('semester')}${data.successCount !== 1 ? 's' : ''}!`,
          {
      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Import {adapt('Semesters')}
          </DialogTitle>
          <DialogDescription>
            Upload an Excel file to import multiple {adapt('semesters')} at once. The file must match
            the template format with 4-level cascading hierarchy (Institution → {adapt('Degree')} → {adapt('Department')} → {adapt('Program')}).
          </DialogDescription>
        </DialogHeader>
                ...
                <AlertDescription>
                  <div className="space-y-2">
                    <p className="font-medium">
                      {result.success
                        ? `Successfully imported ${result.successCount} ${adapt('semester')}${result.successCount !== 1 ? 's' : ''}`
              ...
              {result.duplicateCodes && result.duplicateCodes.length > 0 && (
                <div className="border rounded-lg p-4">
                  <h4 className="font-medium text-sm mb-2">
                    Duplicate {adapt('Semester')} Codes
                  </h4>
