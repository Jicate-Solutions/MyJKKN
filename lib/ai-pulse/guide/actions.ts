 ...
 * enforces row ownership too. Reads/writes FAIL SOFT — a progress hiccup must
 * never block the guide or throw into the UI.
 *
 * Mirrors lib/campus-living/guide/actions.ts (the proven, instrumented install)
 * so all three module guides feed the SAME public.guide_progress / guide_events
 * tables. Installed to close AI Pulse's instrumentation gap (its FAB was live
 * but had no event sink → 0 events), 2026-06-14.
 */
