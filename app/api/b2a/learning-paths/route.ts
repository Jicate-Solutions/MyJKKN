/**
 * B2A — Learning Paths (PDE quests) read-only list.
 * GET /api/b2a/learning-paths
 *
 * NOTE: pde_quests has no institution_id column. We gate solely by API key
 * validity and module permission, but still log institutionId in the audit
...
const MODULE_KEY = 'learning-paths' as const;
const TABLE_NAME = 'pde_quests';
const ENDPOINT = '/api/b2a/learning-paths';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    // pde_quests has no institution_id column — list is global per key validity.
