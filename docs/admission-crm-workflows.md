...
[Counselor logs in] --> Dashboard shows "My Day"
    ...
    |
    |--> [Follow-up Queue]
    |      |-- Leads with next_followup_at = today
...
- `rescheduleFollowup(leadId, newDate)` - Move follow-up date
...
[Save to admission_lead_scores]
    ...
    |
    v
[Update lead.lead_score on admission_leads]
...
[Submits new lead referral]
    |-- full_name, phone, email, program_interest
    |-- referral_code
    |
    v
...
| Table | Service | Purpose |
|-------|---------|---------|
| `education_consultants` | ConsultantService | Consultant profiles |
| `consultant_commissions` | ConsultantService | Commission records |
