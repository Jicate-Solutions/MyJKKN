...
export default function AbsenteesPage() {
  ...
  // Defensive: service returns {data: HostelAttendance[], count: number} — unwrap to array.
  // Also accepts raw array form if the shape changes later.
  const raw = rawAbsentees as any;
  const absentees: any[] = Array.isArray(raw)
    ? raw
  ...
  const filteredAbsentees = absentees.filter((a: any) => {
    const name = a?.name ?? '';
    const roll = a?.roll ?? '';
    const matchesSearch =
  ...
  const criticalCount = absentees.filter((a: any) => a?.status === 'critical').length;
  const warningCount = absentees.filter((a: any) => a?.status === 'warning').length;
