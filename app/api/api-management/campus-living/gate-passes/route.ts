...
/**
 * GET /api/api-management/campus-living/gate-passes
 * Query params: page, limit, status, pass_type, block_id, learner_id, date_from, date_to
 */
export const GET = withApiKeyAuth(async (request, auth) => {
  ...
  const passType = getStringParam(url, 'pass_type');
  ...
  if (passType) query = query.eq('pass_type', passType);
...
/**
 * POST /api/api-management/campus-living/gate-passes
 */
