-- Add period_id to attendance reports for consistent navigation between reports and details
-- This fixes the data inconsistency issue where reports show different data than details

-- Drop and recreate the function with period_id support
...
RETURNS TABLE(
    id TEXT,
    period_id TEXT,  -- Added period_id for consistent navigation
...
BEGIN
    ...
    -- Enhanced base query with period_id for consistent navigation
    base_query := '
        SELECT DISTINCT
            sa.id::TEXT as id,
            period_data.period_id::TEXT as period_id,  -- Include period_id for navigation consistency
            sa.attendance_date,
        ...
        CROSS JOIN LATERAL (
            SELECT 
                key as period_id,  -- This is the key that will be used for navigation
                ...
                -- Safe UUID casting with proper NULL/empty string handling
                CASE 
    ...
    IF p_staff_id IS NOT NULL THEN
        base_query := base_query || ' AND EXISTS (
            SELECT 1 FROM staff_plan_courses spc 
            JOIN staff_plans sp ON spc.staff_plan_id = sp.id 
            WHERE spc.staff_id = ' || quote_literal(p_staff_id) || 
            ' AND spc.course_id = period_data.course_id
        )';
    END IF;
...
-- Add helpful comment
COMMENT ON FUNCTION get_attendance_report_list IS 'Enhanced attendance reports function with period_id for consistent navigation between reports table and details page. Fixes data inconsistency issue.';
