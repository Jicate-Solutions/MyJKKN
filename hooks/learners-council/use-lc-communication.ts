...
/**
 * Hook to export announcements data (returns data for CSV generation)
 */
export function useExportAnnouncements() {
  return useMutation({
    mutationFn: async (filters: {
      scope?: AnnouncementScope;
      status?: AnnouncementStatus;
    }) => {
      // Fetch all matching data without pagination limit
      const result = await LCCommunicationService.getAnnouncements({ ...filters, page: 1, limit: 10000 });
      return result.data;
    },
    onError: (error: Error) => {
      toast.error(error.message || 'Failed to export announcements');
    }
  });
}
