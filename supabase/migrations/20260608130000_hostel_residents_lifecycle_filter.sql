-- Hostel Residents surface: restrict v_learner_hostelites to actual residents.
--
-- Before: the view's only filter was `acc.code = 'hostel'`, so EVERY learner
-- flagged hostel showed up regardless of lifecycle — including enquiry_submitted
-- (46), graduated (16), inactive (2), rejected (1) — on the Residents list AND
-- in the Generate-bills surface (both read this view).
--
-- After: residents = hostel accommodation AND lifecycle_status IN
-- (active, reserved, admitted). Stakeholder-chosen set (2026-06-08): enrolled +
-- firmly-incoming students. NOTE this intentionally EXCLUDES 'account' (12) —
-- a narrower set than billing's BILLABLE_LIFECYCLE_STATUSES
-- (lib/services/billing/schedule/student-search-service.ts), which also includes
-- 'account'. Account-stage hostellers therefore won't appear here or in
-- Generate-bills until they advance to admitted/reserved/active.
--
-- CREATE OR REPLACE (columns unchanged) preserves existing grants. Mirrored into
