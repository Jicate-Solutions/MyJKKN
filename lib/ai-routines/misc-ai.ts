...
export const MISC_AI_ROUTINES: AIRoutine[] = [
  ...
  },
  {
    ...
    "callsClaude": false,
    "whatItDoes": "Each night it computes per-institution NAAC evidence snapshots from live module data: lesson-plan / pedagogy-tagging coverage from the curriculum lesson spine (Metric 5.1.1), facilities-in-daily-use from the resource registry plus hostel/mess/health/sports activity counts (Metric 3.1.1), and IT infrastructure with the learner:computer ratio from the resource registry's 'IT & Digital Resources' category (Metric 3.4.1). Each snapshot row fans out to quality_evidence_mappings so the accreditation narrative is backed by live counts instead of nothing.",
    ...
    "sideEffects": "DB writes only: upserts facility_teaching_naac_evidence snapshot rows (one per institution × metric × AY) and is_auto=true quality_evidence_mappings rows (NAAC 5.1.1 / 3.1.1 / 3.4.1), refreshing metadata + mapped_at on re-run. NEVER touches manually-curated (is_auto=false) mappings. Counts only — no faculty or learner identities. No notifications, no external messages.",
    "safeToManualTrigger": true,
