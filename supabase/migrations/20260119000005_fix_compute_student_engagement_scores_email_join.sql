-- Fix compute_student_engagement_scores - join by email instead of ID
-- Date: 2026-01-19
-- Issue: profiles.id ≠ learners_profiles.id (different UUIDs)
-- Solution: Join via email matching (profiles.email = lp.student_email OR lp.college_email)
...
BEGIN
    ...
    -- ✅ FIXED: Join by email (profiles.email matches learners_profiles email columns)
    FOR v_student IN
        SELECT DISTINCT
            p.id as user_id,  -- Use profiles.id (auth user ID)
            lp.institution_id,
            lp.department_id,
            lp.program_id,
            lp.semester_id,
    ...
    -- Refresh the materialized view (non-concurrent)
...
COMMENT ON FUNCTION compute_student_engagement_scores IS 'Computes individual student engagement scores, percentile ranks, and risk assessments for active students. Joins profiles and learners_profiles by email matching.';
