...
- 5 SECURITY DEFINER RPCs (all `SET search_path = public`): `fn_max_chat_request(text,uuid)→jsonb` (allowlist-gated; refuses without insert when the `maxlane:poller-heartbeat` pulse is >5 min stale so the route can fall back instantly; one live request per requester; 4000-char cap) + `fn_max_chat_status(uuid)→jsonb` + `fn_max_chat_cancel(uuid)` (requester-scoped; cancel marks pending/claimed rows error so a late runner completion — guarded on status='claimed' — can't overwrite it) — all three `REVOKE anon,PUBLIC; GRANT authenticated`. `fn_max_chat_claim_pending()→setof` (≤3 via FOR UPDATE SKIP LOCKED; 10-min expiry for BOTH unclaimed and claimed-but-silent rows — chat is time-sensitive) + `fn_max_chat_complete(uuid,text,text,text)` — both service-role-restricted by GRANT and runtime JWT check, mirroring the fn_max_lane_* siblings.
- Consumer: app/api/ai-query/route.ts (enqueue → 2.5s long-poll → 150s unclaimed / 240s total deadlines → cancel + cached-API fallback). Runner: ai-query-chat.mjs on the Windows box (claims, answers via headless claude -p, records ai_model_usage provider=claude_code model=max-subscription cost 0).
- Location: `supabase/migrations/20260711013000_max_lane_chat_requests.sql` (additive; apply via Mgmt API at/after merge — until applied, the route's RPC call errors and every question just uses the API path).
