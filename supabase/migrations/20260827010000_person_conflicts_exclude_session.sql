-- ============================================================================
-- The Availability Spine — Limb 2: PEOPLE — self-clash fix (exclude one session)
-- File: 20260827010000_person_conflicts_exclude_session.sql | Date: 2026-08-27
--
-- BUG: re-opening an ALREADY SAVED induction session that has a resource person
-- reported that person as "Double-booked — can't be assigned at this time",
-- naming the very session being edited:
--     SAVITHIRAN M: Speaking: Fresher Induction 2026 - Engineering
--                   — Registration (9:15 AM–10:30 AM)
-- Source 2 below overlap-tests EVERY speaking row in the window, including the
-- row this edit already owns. Callers tier an 'event' clash as a HARD block, so
-- the session became un-saveable — the speaker could only be removed, never kept.
--
-- FIX: an optional p_exclude_session_id. During an EDIT, "is this person free?"
-- always means "free apart from the booking I am currently editing", so the
-- caller passes the session id it is saving and Source 2 skips that one row.
-- NULL (create-new, and every existing caller) behaves exactly as before — no
-- other source, scope guard, or dedupe rule changes.
--
-- DROP-then-CREATE, not a bare CREATE OR REPLACE: adding a DEFAULT parameter
-- makes a NEW signature, and leaving the 3-arg original beside it would make
-- every 3-arg call ambiguous ("function is not unique"). Dropping first keeps
-- exactly one resolvable signature; 3-arg callers land on the new default.
-- Bodies below are the LIVE definitions with ONLY the marked lines added.
--
-- ---------------------------------------------------------------------------
-- Original header (20260629120000_person_availability_brain.sql) follows.
--
...
DROP FUNCTION IF EXISTS public.fn_people_conflicts(uuid[], timestamptz, timestamptz);
DROP FUNCTION IF EXISTS public.fn_person_conflicts(uuid, timestamptz, timestamptz);

CREATE OR REPLACE FUNCTION public.fn_person_conflicts(
  p_profile_id         uuid,
  p_start              timestamptz,
  p_end                timestamptz,
  p_exclude_session_id uuid DEFAULT NULL   -- ADDED: the session being edited
)
RETURNS TABLE (
...
BEGIN
  ...
  WHERE sp.profile_id = p_profile_id
    ...
    AND (p_exclude_session_id IS NULL OR es.id <> p_exclude_session_id);  -- ADDED: never clash with yourself
...
REVOKE EXECUTE ON FUNCTION public.fn_person_conflicts(uuid, timestamptz, timestamptz, uuid) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.fn_person_conflicts(uuid, timestamptz, timestamptz, uuid) TO authenticated;
...
CREATE OR REPLACE FUNCTION public.fn_people_conflicts(
  ...
  p_start              timestamptz,
  p_end                timestamptz,
  p_exclude_session_id uuid DEFAULT NULL   -- ADDED: passed straight through
)
RETURNS TABLE (
  profile_id uuid,
  source     text,
...
AS $$
  ...
  CROSS JOIN LATERAL public.fn_person_conflicts(pid, p_start, p_end, p_exclude_session_id) c
$$;

REVOKE EXECUTE ON FUNCTION public.fn_people_conflicts(uuid[], timestamptz, timestamptz, uuid) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.fn_people_conflicts(uuid[], timestamptz, timestamptz, uuid) TO authenticated;
