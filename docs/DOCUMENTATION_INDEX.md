...
- [FreeRADIUS VPS playbook — radius.jkkn.ai](guides/network/2026-09-06-GUIDE-freeradius-vps-playbook.md) — DRAFT (Director ruling 2026-09-06, Part 5 of the campus Wi-Fi SSO spec): Hetzner CCX13 provisioning, Debian + FreeRADIUS 3.2, RADSEC on TCP 2083 with a Let's Encrypt cert and the SHA-256 fingerprint for `network_radius_servers.tls_cert_fingerprint`, one client per router keyed by NAS-Identifier, `rlm_rest` → `/api/network/radius-auth` with a Vault bearer and a 300 ms budget, CoA on UDP 3799, accounting Option A (direct `rlm_sql`) vs Option B (HTTPS receiver) left to the Director (Q3), hardening, health/alert, backup/restore, and the decision-17 failover runbook for 6 am.
- [MikroTik CCR2116 hotspot handoff — JKKN-RADIUS-Test SSID](guides/network/2026-09-06-GUIDE-mikrotik-hotspot-handoff.md) — DRAFT (Director ruling 2026-09-06): the exact RouterOS 7.20 commands for the router half of the captive-portal SSO — `/radius add … protocol=radsec`, `/radius incoming` on 3799, hotspot profile with `use-radius=yes login-by=https,mac-cookie ssl-certificate=<router cert>`, hotspot on the trial bridge only, the `login.html` redirect to `wifi.jkkn.ai/api/network/sso` with the `$(mac)/$(ip)/$(link-login-only)/$(link-orig)` variables, walled-garden entries for MyJKKN/Supabase/Google, the decision-17 auto-open/auto-close script, back-out, and the 50-user parallel-week plan. Every command cites its help.mikrotik.com page; unconfirmed lines are marked `[UNVERIFIED — check on the router]`.
