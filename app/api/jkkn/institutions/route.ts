...
import type { JkknInstitutionUpstreamResponse } from '@/types/jkkn-api/institutions';
 ...
 *
 * Secure server-side proxy for the JKKN institutions endpoint.
 * The API key is stored in JKKN_API_KEY (server-only env var) and is
 * never sent to the browser. Accepts ?page, ?limit, ?search, ?isActive params
 * that are forwarded to the upstream API.
 *
...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(
      `${JKKN_API_BASE_URL}/api-management/organizations/institutions?${upstreamParams}`,
      {
        headers: {
