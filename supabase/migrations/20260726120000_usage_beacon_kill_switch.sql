...
SELECT * FROM (VALUES
  ('analytics.usage_beacon.enabled','global','false'::jsonb,'boolean',
   'Master switch for the in-app Usage Beacon. When true, each page view inside the authenticated shell is posted to /api/analytics/usage/events and recorded in usage_events (event_type=page_visit, source=explicit), which feeds module_usage_daily, feature_usage_summary and institution_health_scores — the substrate behind per-module adoption reporting. When false the endpoint writes nothing and instructs the client to stand down for the session. Dark by default: enabling it starts write traffic proportional to platform-wide navigation, so flip it deliberately and watch usage_events volume. Read-only telemetry — it never changes what any user can see or do.',
