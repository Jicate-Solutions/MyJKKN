...
GATE_B="$ROOT/supabase/migrations/20261227090100_notifications_must_answer.sql"
...
if [ -f "$GATE_B" ]; then psql -d "$DB" -v ON_ERROR_STOP=1 -q -f "$GATE_B"; else
  ...
  git -C "$ROOT" show jicate/feat/blocking-feedback-gate:supabase/migrations/20261227090100_notifications_must_answer.sql | psql -d "$DB" -v ON_ERROR_STOP=1 -q; fi
