...
import { XCircle, RefreshCw, Home, AlertTriangle, AlertCircle, Info, ArrowRight, HelpCircle } from 'lucide-react';
...
export default function PaymentFailedPage() {
  ...
  const { data: paymentStatus, isLoading, error } = usePaymentStatus(transactionId, !!transactionId);

  useEffect(() => {
    if (paymentStatus && ['failed', 'cancelled', 'expired'].includes(paymentStatus.status)) {
        ...
        >
          <Card className="max-w-md w-full shadow-lg">
            <CardHeader className="text-center">
              ...
              <CardTitle className="text-destructive">Error Loading Payment Status</CardTitle>
              <CardDescription>
  ...
  const getStatusInfo = (status: string) => {
    const statusInfo: Record<string, { title: string; description: string; color: string; bgGradient: string }> = {
      failed: {
          ...
          >
            ...
            >
              <Card className="shadow-xl border-2 border-red-100 dark:border-red-900 overflow-hidden">
                <div className={`bg-gradient-to-r ${statusInfo.bgGradient} p-6 text-white`}>
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm opacity-90 mb-1">Transaction ID</p>
                      <p className="font-mono text-lg font-semibold">
                        {paymentStatus.transaction_id.slice(0, 8)}...{paymentStatus.transaction_id.slice(-8)}
                      </p>
                    </div>
                ...
                <CardContent className="p-6 space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <span className="text-sm text-gray-600 dark:text-gray-400">Bills Selected</span>
                        <span className="font-semibold text-gray-900 dark:text-white">{paymentStatus.bills_paid}</span>
                      </div>

                      {paymentStatus.payment_method && (
                        <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                          <span className="text-sm text-gray-600 dark:text-gray-400">Payment Method</span>
                          <span className="font-semibold text-gray-900 dark:text-white">{paymentStatus.payment_method.toUpperCase()}</span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                    <div className="flex items-center justify-between bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
                      <span className="text-lg font-semibold text-gray-700 dark:text-gray-300">Amount</span>
                      <span className="text-3xl font-bold text-gray-900 dark:text-white">
                        ₹{paymentStatus.amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </span>
                    </div>
                  </div>
            ...
            >
              <p>
                Having trouble? Contact our support team for assistance with your payment.
              </p>
            </motion.div>
          </motion.div>
        )}
