-- ============================================================================
-- 20260724091000 — Accountant reports: collected = billing_receipts.payment_amount
-- ============================================================================
-- CORRECTION to 20260724090000. That migration computed `collected` from
-- billing_receipt_items.amount_paid (allocation-based). Production data shows
-- allocations total only ~25% of billing_receipts.payment_amount (line-items
-- record partial/component allocations, not the full payment), so allocation-
-- based collected massively understated collections and diverged from the
-- shipped /billing/analytics RPCs (which use billing_receipts.payment_amount).
--
-- This migration CREATE OR REPLACEs the 3 affected functions to compute
-- `collected` from billing_receipts.payment_amount, matching the design spec
-- (§6.2) and the analytics dashboard. Because receipts are not academic-year-
-- stamped, the academic-year filter attributes a receipt via its student's
-- learners_profiles.academic_year_id. Bill-based metrics (outstanding, cleared,
-- billed, counts) are unchanged and still use the bill's own academic_year_id.
-- get_billing_report_outstanding_by_year is unchanged (bill-based only).
-- ============================================================================
...
-- 2) SCHEMES (collected from receipts; billed keeps the IST date FILTER) ------
...
-- 3) HEADLINE KPIs (collected from receipts) ---------------------------------
