...
/** Shape of ticket with category join for analytics */
interface TicketWithCategory {
  category?: { name: string } | null;
}

/** Shape of ticket with priority field for analytics */
interface TicketWithPriority {
  priority: string | null;
}

/** Shape of ticket with notification fields */
interface TicketWithNotifyFields {
  subject?: string;
  raised_by_id?: string;
  assigned_to?: string;
...
export class LCIssueService {
  ...
  static async assignIssue(issueId: string, assigneeId: string): Promise<GrievanceTicket> {
    ...
    try {
      const ticket = data as unknown as TicketWithNotifyFields;
      if (assigneeId) {
  ...
  static async getIssueStats(institutionId?: string): Promise<LCIssueStats> {
    ...
    let resolvedMonthQuery = this.supabase
      .from('grievance_tickets')
      .select('*', { count: 'exact', head: true })
      .eq('status', 'resolved')
      .gte('resolved_at', startOfMonth.toISOString());
    if (institutionId) {
      resolvedMonthQuery = resolvedMonthQuery.eq('institution_id', institutionId);
    }
    ...
    if (institutionId) {
      categoriesQuery = categoriesQuery.eq('institution_id', institutionId);
    }

    // Get priority breakdown
    let prioritiesQuery = this.supabase
      .from('grievance_tickets')
      .select('priority')
      .in('status', ['open', 'in_progress']);
    if (institutionId) {
      prioritiesQuery = prioritiesQuery.eq('institution_id', institutionId);
    }
    ...
    ((categoriesResult.data || []) as unknown as TicketWithCategory[]).forEach((t) => {
    ...
    ((prioritiesResult.data || []) as unknown as TicketWithPriority[]).forEach((t) => {
  ...
  static async escalateIssue(
    ...
    try {
      const updatedTicket = data as unknown as TicketWithNotifyFields;
