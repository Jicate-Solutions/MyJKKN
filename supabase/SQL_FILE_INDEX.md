...
- `fn_scf_notify_session_pending(date,uuid,text)` — PR-B faculty-triggered per-session nudge; reuses the `fn_scf_nudge_pending_learners` two-write (`notifications` + `user_notifications`) delivery path, assigned-faculty (or super/admin) only, identity-scoped, idempotent per learner/session/day. REVOKE anon,PUBLIC / GRANT authenticated.
- `fn_scf_set_gate_mode(text,uuid)` — PR-C super-admin BREAK-GLASS kill-switch; upserts the `session_feedback.gate_mode` platform_policies row (global or per-institution). Revert to `visibility`/`off` halts enforcement in one call, no deploy. super_admin ONLY. REVOKE anon,PUBLIC / GRANT authenticated.
