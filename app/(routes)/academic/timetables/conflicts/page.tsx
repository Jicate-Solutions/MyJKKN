...
import { AlertTriangle, CheckCircle, RefreshCcw, X, Check, Lock, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function TimetableConflictsPage() {
  ...
  const [currentPage, setCurrentPage] = useState({ 'staff-mismatch': 1, 'missing-plans': 1 });
  ...
  useEffect(() => {
    if (!permissionsLoading) {
      if (!isSuperAdmin) {
        toast.error('Access Denied: Only super administrators can access timetable conflicts.');
  ...
  // Pagination logic
  const getPaginatedConflicts = (type: 'STAFF_MISMATCH' | 'NO_STAFF_PLAN', page: number) => {
  ...
  const getCurrentPageConflicts = (tabKey: string) => {
    if (tabKey === 'staff-mismatch') {
      return getPaginatedConflicts('STAFF_MISMATCH', currentPage['staff-mismatch']);
    } else {
      return getPaginatedConflicts('NO_STAFF_PLAN', currentPage['missing-plans']);
    }
  };
  ...
  const handleSelectAll = (tabKey: string) => {
    ...
    const syncableCurrentConflicts = currentConflicts.filter(c => c.conflict_type === 'STAFF_MISMATCH' && c.planned_staff_id);
    const currentKeys = syncableCurrentConflicts.map((c, index) => `${c.timetable_id}-${c.course_id}-${index}`);
    
    const allCurrentSelected = currentKeys.every(key => selectedConflicts.has(key));
  ...
  const PaginationControls = ({ tabKey, totalPages }: { tabKey: string, totalPages: number }) => {
        ...
        {conflicts.length > 0 ? (
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="staff-mismatch" className="flex items-center gap-2">
                <AlertTriangle className="h-4 w-4" />
                Staff Mismatches ({conflictsByType.STAFF_MISMATCH.length})
              </TabsTrigger>
              <TabsTrigger value="missing-plans" className="flex items-center gap-2">
                <AlertTriangle className="h-4 w-4" />
              ...
              <Card>
                <CardHeader>
                  ...
                  <div className='flex gap-2 flex-wrap'>
                    ...
                    {conflictsByType.STAFF_MISMATCH.some(c => c.planned_staff_id) && (
                      <>
                        <Button
                ...
                <CardContent>
                  <div className='grid gap-4'>
                    {getCurrentPageConflicts('staff-mismatch').map((conflict, index) => {
                            ...
                            <div className='flex items-start gap-3'>
                              {canSync && (
                                <Checkbox
                                  ...
                                  onCheckedChange={(checked) =>
                                    handleSelectConflict(conflictKey, !!checked)
                                  }
                                  disabled={bulkSyncing}
                                  className='mt-1'
                                />
                              ...
                              <div className='space-y-2'>
                                ...
                                <div>
                                  ...
                                  <div className='text-sm text-muted-foreground'>
                                    Current: <span className="text-red-600 font-medium">{conflict.timetable_staff_name}</span>{' '}
                                    → Planned: <span className="text-green-600 font-medium">{conflict.planned_staff_name}</span>
                                  </div>
                                </div>
                                <div>
                            ...
                            <div>
                              {canSync && (
                                <Button
                                  size='sm'
                                  variant='outline'
                                  onClick={() => handleSyncTimetable(conflict)}
                                  disabled={syncingTimetable === conflict.timetable_id}
                                  className='gap-1'
                                >
            ...
            <TabsContent value="missing-plans">
              <Card>
                <CardHeader>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Courses in timetables that don't have staff planning configured
                  </p>
                  <div className='flex gap-2 flex-wrap'>
                    <Button
                      onClick={loadConflicts}
                ...
                <CardContent>
                  <div className='grid gap-4'>
                    {getCurrentPageConflicts('missing-plans').map((conflict, index) => {
                      const conflictKey = `${conflict.timetable_id}-${conflict.course_id}-${index}`;

                      return (
                        <div key={conflictKey} className="border rounded-lg p-4">
                          <div className='flex items-start justify-between'>
                            <div className='space-y-2'>
                              <div>
                                <div className='font-medium'>
                              ...
                              <div>
                                ...
                                <div className='text-sm text-muted-foreground'>
                                  Currently assigned: {conflict.timetable_staff_name}
                                </div>
                                <div className='text-xs text-amber-600 mt-1'>
                                  💡 This course appears in the timetable but has no staff planning configured. Please add staff planning for this course to resolve the conflict.
                                </div>
                              </div>
                              <div>
                                {getConflictBadge(conflict.conflict_type)}
