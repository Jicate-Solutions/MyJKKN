...
/** Shape of a vote row for demographic breakdown */
...
/** Shape of a member row with institution join for demographics */
...
/** Shape of a channel row with members for enrichment */
interface ChannelRow {
  id: string;
  [key: string]: unknown;
}

/** Shape of a chat message row for last-message map */
...
export class LCCommunicationService {
  ...
  static async getAnnouncementReadStats(announcementId: string): Promise<{
    ...
    const { data: reads, error } = await this.supabase
      .from('lc_announcement_reads')
      .select(
        `
        *,
  ...
  static async createPoll(
    ...
    if (data.options && data.options.length > 0) {
      ...
      if (optError) {
        ...
        await this.supabase.from('lc_polls').delete().eq('id', (poll as { id: string }).id);
        throw new Error(`Failed to create poll options: ${optError.message}`);
      }
    }

    // Return poll with options
    return this.getPollById((poll as { id: string }).id);
  }
  ...
  static async votePoll(
    ...
    const { data: poll, error: pollFetchError } = await this.supabase
      .from('lc_polls')
    ...
    const { data: optionCheck, error: optionError } = await this.supabase
      .from('lc_poll_options')
      .select('id')
      .eq('id', optionId)
      .eq('poll_id', pollId)
    ...
    if (!poll.allows_multiple) {
      ...
      const { data: existingOptionVote } = await this.supabase
        .from('lc_poll_votes')
        .select('id')
        .eq('poll_id', pollId)
  ...
  static async getPollResultsWithDemographics(pollId: string): Promise<{
    ...
    const allVotes = (votes || []) as unknown as PollVoteRow[];
    ...
    const voterIds = [...new Set(allVotes.map((v) => v.user_id))] as string[];
    ...
    for (const m of ((members || []) as unknown as MemberWithInstitution[])) {
  ...
  static async getTopicPosts(
    ...
    const { data: parentPosts, count, error } = await this.supabase
      .from('lc_forum_posts')
      .select(
        `
        *,
    ...
    const { data: allReplies } = await this.supabase
      .from('lc_forum_posts')
      .select(
        `
        *,
  ...
  static async getFlaggedPosts(filters: {
    ...
    const posts = (data || []) as unknown as LCForumPost[];
    for (const post of posts) {
      const reactions = (post.reactions || []) as LCForumReaction[];
      post.reaction_counts = {
        like: reactions.filter((r) => r.type === 'like').length,
  ...
  static async getChannels(
    ...
    const channelIds2 = ((channels || []) as unknown as ChannelRow[]).map((c) => c.id);
    let lastMessageMap: Record<string, unknown> = {};
    ...
    if (channelIds2.length > 0) {
      ...
      for (const msg of ((allMessages || []) as unknown as ChatMessageRow[])) {
    ...
    const enrichedChannels: LCChatChannel[] = ((channels || []) as unknown as ChannelRow[]).map((channel) => ({
      ...channel,
  ...
  static async createChannel(
    ...
    if (memberError) {
      console.error('[lc/communication] Error adding channel members:', memberError);
