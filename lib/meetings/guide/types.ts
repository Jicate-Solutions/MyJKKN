 ...
 *
 * Byte-identical to the other modules' per-module guide types (admission /
 * campus-living / PDE / …): the ONLY thing that differs per module is the
 * `GuidePersona` union below. The central composition layer
 * (lib/guide/types.ts + lib/guide/registry.ts) re-keys these module personas
 * onto the closed canonical persona union at merge time.
 *
 * ── HOW TO ADAPT ──────────────────────────────────────────────────────────
...
/* ────────────────────────────────────────────────────────────────────────
 * 1. PERSONAS — Meetings (Universal Booking) roles.
 * ──────────────────────────────────────────────────────────────────────── */
...
export const GUIDE_PERSONAS: readonly GuidePersona[] = [
  ...
  // the Meetings module has no self-service learner audience inside the platform
  // (people book a host's page from the public internet, off a guide surface),
  // so there is no ungated "learner" baseline lane. The `host` lane is gated by
  // the module-access key (meetings.view) — every staff member who can open the
  // module is a potential booking-page owner; the `admin` lane by the analytics
  // key (meetings.analytics.view) — the module operator who runs routing forms,
  // webhooks and watches the adoption scoreboard.
  "admin",
  "host",
] as const;
