...
--   Fix A: align EXISTS dedup with the partial unique indexes
--          (status NOT IN ('cancelled','superseded')) so a superseded prior
--          bill no longer wrongly blocks a replacement.
--   Fix B: revoke EXECUTE from anon (function self-gates, but anon should not run it).
