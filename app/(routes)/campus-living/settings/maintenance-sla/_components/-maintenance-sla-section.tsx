...
/**
 * MaintenanceSlaSection — chrome-less editor body for the Maintenance SLA settings.
 *
 * Extracted verbatim from the standalone page
 * (/campus-living/settings/maintenance-sla) so the same editor can be embedded
 * inside the unified Campus Living config sections without nesting a second
 * ContentLayout. Renders ONLY the working body (the h1 + Save header row and the
 * SLA-grid / escalation cards). No ContentLayout / PermissionGuard / page chrome.
 *
 * Persists to `hostel_maintenance_sla_config` (one row per category x priority)
 * via batch upsert. Permission gate: `campus_living.settings.edit` (or
 * super-admin) controls the Save button + input enablement.
 *
 * NOTE: The "Escalation Rules" card still has a deferred note because a
 * *percent-of-SLA* trigger + multi-role notify list require a computed column or
 * a new mapping table — deferred to the migrations team. The per-row SLA hours
 * ARE saved.
 */
...
export function MaintenanceSlaSection() {
