...
- Verification: executed against a stub substrate locally — **21/21 assertions pass, 0 skips**, including the decisive per-learner numbers (3 answers from 1 learner → `voices=1`, median NULL; +2 distinct learners → `voices=3`, median **0** where impression-weighting would have given 4; latest-answer-per-learner wins; pre-creation-instant excluded; cross-tenant permission holder forbidden while the same-tenant holder is admitted). Battery: `.claude/battery-classroom-practice-hardening.sql`.
