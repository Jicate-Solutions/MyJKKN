// =====================================================================
// PDE max-pde lane heartbeat — prove the coaching worker is ALIVE
// =====================================================================
...
//   {"path":"/api/cron/pde-lane-heartbeat?secret=$CRON_SECRET","schedule":"23 * * * *"}
// Minute 23 deliberately, not 0: the top of the hour is already crowded and a
// heartbeat that queues behind a stampede reports latency that is not the
// lane's fault.
//
...
export async function GET(req: NextRequest) {
  ...
  const { data: enq, error: enqErr } = await (supabase as any).rpc('fn_ai_enqueue_system', {
    ...
    p_dedupe_key: `pde-lane-heartbeat:${new Date().toISOString().slice(0, 13)}`,
  });
  if (enqErr) {
    return NextResponse.json(
