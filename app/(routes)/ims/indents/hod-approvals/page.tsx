...
/**
 * Phase D — HOD approval queue.
 *
 * Lists indents in 'pending_local_approval' for every department the current
 * user heads (departments.head_of_department_id). Approve forwards the indent
 * to the store admin's pending queue; Reject ends it with a mandatory reason.
 *
 * Scoping is inherent in the query (headship, not store), so a non-HOD who
 * lands here simply sees the empty state — no other department's data leaks.
 */
export default function HodApprovalsPage() {
  return (
    <ImsPageGuard module="ims.indents" action="view">
      <HodApprovalsPageInner />
    </ImsPageGuard>
  );
}

function HodApprovalsPageInner() {
  ...
  const { data: pendingList = [], isLoading } = useImsHodPendingIndents(
    profile?.id
  );
  ...
  return (
    <ContentLayout title="HOD Approvals">
      <div className="space-y-6">
        {/* Header */}
        <div>
          <h2 className="text-2xl font-bold tracking-tight">HOD Approvals</h2>
          <p className="text-muted-foreground">
            Indent requests from your department&apos;s lab assistants awaiting
            your approval
          </p>
        </div>
        ...
        <Dialog open={!!rejectId} onOpenChange={() => { setRejectId(null); setRejectReason(''); }}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Please provide a reason for rejecting this indent request. The
                lab assistant will see this reason.
              </DialogDescription>
            </DialogHeader>
