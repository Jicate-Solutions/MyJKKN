/**
 * B2A — Resource management (event resource bundles) read-only list.
 * GET /api/b2a/resource-management
 *
 * NOTE: event_resource_bundles has no institution_id column (institution
 * context is derived via the parent events.institution_id). We gate by API
 * key validity + module permission, but still log institutionId for audit.
 */
...
const MODULE_KEY = 'resource-management' as const;
const TABLE_NAME = 'event_resource_bundles';
const ENDPOINT = '/api/b2a/resource-management';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    // event_resource_bundles has no institution_id column — list is global per key validity.
