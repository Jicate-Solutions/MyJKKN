...
- Wire 2/3 live in the routes: non-verified regress verdicts and watchdog findings (silent >26h managed routines, errored last_status, non-verified audits) fan out to super admins via fanoutNotification (idempotent per IST day). /admin/loops renders the same conditions RED live (card last-run line + tower tested badge).
...
- `max_lane_chat_requests.delivered_at` (NEW column). `fn_max_chat_inbox()` — requester-scoped atomic read-and-stamp (UPDATE..RETURNING over a FOR UPDATE SKIP LOCKED picker; status='done', undelivered, last 24h, ≤5) so two tabs can't double-show an answer; `fn_max_chat_mark_delivered(uuid)` — requester-scoped stamp used by the route on live delivery. Both REVOKE anon/PUBLIC, GRANT authenticated.
- Consumers: GET /api/ai-query (inbox endpoint; RPC-absent → empty inbox, never an error) + use-ai-query hook (drains once on mount, renders Q&A pairs with the max_lane chip and an "answered while you were away" footer).
