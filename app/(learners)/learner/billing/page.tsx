...
import {
  useLearnerBilling,
  useLearnerBillingCharts,
  useLearnerBillingNotifications
} from '@/hooks/learner/use-learner-billing';
import { formatCurrency } from '@/lib/utils';

export default function BillingPage() {
  const { billingData, loading, error, filters, updateFilters, refreshData } =
    useLearnerBilling();
        ...
        <div className='p-4 md:p-6'>
          <div className='flex flex-col items-center justify-center text-center p-8 bg-gray-50 rounded-lg shadow-sm border border-dashed'>
            ...
            <p className='text-gray-600 mt-2 max-w-md'>
              No billing information found for your account. Please contact the
              administration if this seems incorrect.
            </p>
  ...
  return (
    <BillingErrorBoundary>
      <LearnerLayout>
        <LearnerPageHeader
          title='Billing & Payments'
          subtitle={`View your fees, bills, and payment status • ${formatCurrency(
            stats?.outstanding_amount || 0
          )} outstanding`}
        />

        <div className='p-3 sm:p-4 lg:p-6 space-y-4 sm:space-y-6 bg-gradient-to-br from-blue-50 via-gray-50 to-green-50 min-h-screen'>
          {/* Alerts Section */}
          ...
          {/* Main Content */}
          <div className='bg-white rounded-3xl shadow-2xl border-0 backdrop-blur-sm overflow-hidden'>
            <Tabs defaultValue='overview' className='w-full'>
              {/* Mobile-first responsive tabs */}
              <div className='border-b border-gray-200 bg-gradient-to-r from-[#0b6d41] to-green-600 p-1'>
                <TabsList className='grid w-full grid-cols-5 bg-white/20 backdrop-blur-md rounded-2xl border-0 h-auto p-1'>
                  <TabsTrigger
                    value='overview'
                  ...
                  >
                    ...
                    {stats &&
                      (stats.unpaid_bills || 0) + (stats.overdue_bills || 0) >
                        0 && (
                        <Badge
                          variant='destructive'
                          className='absolute -top-1 -right-1 h-5 w-5 rounded-full p-0 flex items-center justify-center text-xs bg-[#ffde59] text-black border-2 border-white font-bold'
                        >
              ...
              <TabsContent
                value='overview'
                className='space-y-4 sm:space-y-6 mt-4 sm:mt-6 p-4 sm:p-6'
              >
                  ...
                  <div className='space-y-4 sm:space-y-6 order-1 xl:order-2'>
                    ...
                    <Card className='bg-gradient-to-br from-green-50 to-green-100 border-green-200 shadow-lg hover:shadow-xl transition-shadow duration-300'>
                      <CardContent className='p-4 sm:p-6'>
                        ...
                        <div className='space-y-3'>
                          {billingData?.recent_transactions
                            .slice(0, 5)
                            .map((transaction, index) => (
                              <div
                                key={index}
                                className='flex items-center justify-between p-3 bg-white rounded-lg border border-blue-100 hover:border-blue-200 transition-colors duration-200'
                              >
                      ...
                      <CardContent className='p-4 sm:p-6'>
                        ...
                        <div className='space-y-3'>
                          <div className='flex items-center justify-between p-3 bg-white rounded-lg border border-blue-100'>
                            ...
                            <span className='font-bold text-[#0b6d41]'>
                              {stats?.total_bills || 0}
                            </span>
                          </div>
                          <div className='flex items-center justify-between p-3 bg-white rounded-lg border border-blue-100'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6'>
                  ...
                  <Card className='bg-gradient-to-br from-purple-50 to-pink-100 border-purple-200 shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105'>
                    <CardContent className='p-4 sm:p-6 text-center'>
                      ...
                      <h3 className='text-base sm:text-lg font-semibold mb-2 text-purple-800'>
                        Total Saved
                      </h3>
