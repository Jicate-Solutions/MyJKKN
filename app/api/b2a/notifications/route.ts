/**
 * B2A — Notifications.
 * Backed by `notifications`. Table has no institution_id column;
 * caller should filter by user_id query param for user-scoped views.
 */
...
import { getPaginationParams, isValidUuid } from '@/lib/api-keys/query-helpers';

const MODULE_KEY = 'notifications' as const;
const ENDPOINT = '/api/b2a/notifications';
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    if (isRead === 'true') query = query.not('read_at', 'is', null);
    if (isRead === 'false') query = query.is('read_at', null);
    ...
    return NextResponse.json(
      { error: { code: 'INTERNAL_ERROR', message: 'Failed to list notifications', detail: message } },
