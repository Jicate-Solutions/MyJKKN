...
- **2026-07-04** — Department Instagram **Monthly Cadence** engine (ships DARK). Migration `20260704120000_social_monthly_cadence.sql` (additive + idempotent, NOT yet applied to prod — pending Director review): (0) `ALTER TYPE okr_cycle_type ADD VALUE IF NOT EXISTS 'monthly'` (extends the OKR cycle grain); (1) NEW ledger table `social_monthly_cadence` — one row per (account_id, cadence_month) holding the objective→baseline→feedback→action→re-measure→close state machine; reach columns are SNAPSHOTS from `ig_monthly_audit` (never re-aggregated), feedback snapshot from `feedback_events`; `okr_objective_id` is **REQUIRED** (Director-locked OKR integration); UNIQUE(account_id, cadence_month) + 5 indexes + updated_at trigger; (2) canonical dynamic-permission RLS (`is_super_admin() OR is_admin() OR (user_has_permission('social.departments.view'/'.manage') AND role_has_institution_access(institution_id))`), anon revoked; (3) 4 SECURITY DEFINER RPCs — `fn_ig_monthly_reach(uuid,date)` (STABLE reader over ig_monthly_audit), `fn_social_cadence_open(uuid,text,date,uuid)` (snapshots baseline + creates/links a `cycle_type='monthly'` dept OKR objective + inserts ledger row, idempotent per account+month), `fn_social_cadence_record_action(uuid,text,jsonb)`, `fn_social_cadence_close(uuid,text)` (re-measure + metrics_source guard→`unmeasurable` + feeds linked OKR objective) — all self-gate (DEFINER bypasses RLS) and end `REVOKE anon,PUBLIC; GRANT authenticated`; (4) seeds 4 `social.cadence.*` policy rows into `platform_policies` (**`social.cadence.enabled=false`** ships DARK, `clock_mode='calendar_month'`, `period_days=30`, `win_delta_pct=10`); (5) merges `social.departments.view`+`.manage` into the scope='own' HOD role (Director-locked owner). Mirrored into `setup/{01_tables,02_functions,03_policies}.sql`. New cron `/api/cron/social-monthly-cadence` (`0 7 1 * *`, after the audit) STAGES elapsed open cycles to `awaiting_close`. UI: monthly section on `/admission/social/loop` (`_components/cadence-card.tsx`) + read-only card on the OKR department dashboard. Spec: `specs/dept-ig-monthly-cadence-2026-07-04.md`.
