...
- Known residual: the attendance blob is written as `staff.institution_email` falling back to `staff.email`, so a team member whose institution email differs from their profile email would have drip rows under the other address. The creation form surfaces this before the cycle is opened — the picker's `sessions_90d` uses the same email join, so a mismatch reads as `0 sessions / 90d`.
