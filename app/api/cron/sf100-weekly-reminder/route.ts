// =====================================================================
// SF100 "Solve for 100" — weekly check-in reminder sweep
// =====================================================================
// Fires the SF100 weekly-reminder engine on a schedule. For every ACTIVE
// program (sf100_programs.status = 'active') it runs, per program:
//   SF100Service.runWeeklyReminders(programId) — notifies every active/warning/
//       probation team that has NOT checked in within the last 7 days (a nudge
//       for the silent, not noise for the active).
//
// The service method reads from `this.supabase` (an AsyncLocalStorage-injected
...
// not abort the whole sweep. Scheduled 14:33 UTC Sundays (= 20:03 IST Sunday)
// via vercel.json (secret passed as ?secret=${CRON_SECRET}).
//
...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  for (const id of ids) {
    ...
    try {
      const reminder = await SF100Service.runWithClient(svc, () =>
        SF100Service.runWeeklyReminders(id)
      );
      results.push({ program_id: id, ok: true, reminder });
    } catch (err) {
      results.push({
        program_id: id,
        ok: false,
