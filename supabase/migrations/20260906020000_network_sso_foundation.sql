...
--
-- Migration class: ADD_ONLY. No CREATE OR REPLACE (all objects are new), no
-- DROP, no ALTER of an existing object, no data UPDATE/DELETE against an
-- existing table. No BEGIN/COMMIT in the file, so a reviewer's BEGIN … ROLLBACK
...
-- Purpose: Run monthly via cron. Ensures the next 3 months of partitions exist
--          for both network_sessions and network_audit_log. Each partition it
--          creates is born locked (anon/PUBLIC revoked, RLS on) — the same
--          treatment the hand-written partitions above get in BLOCK 4.
-- ----------------------------------------------------------------------------
  ...
  FOREACH parent_table IN ARRAY parents LOOP
    FOR i IN 1..3 LOOP
      ...
      IF NOT EXISTS (SELECT 1 FROM pg_class WHERE relname = partition_name) THEN
        ...
        EXECUTE format('REVOKE ALL ON TABLE %I FROM anon, PUBLIC', partition_name);
...
--          platform_policies (network.devices.max_per_role, keyed by role).
...
BEGIN
  IF NOT (
    ...
    OR is_super_admin()
    OR is_admin()
    OR (user_has_permission('network.sessions.manage') AND role_has_institution_access(p_institution_id))
  ) THEN
  ...
  -- Cap check: per-role cap from policy; 3 when the role is not listed.
  v_max_devices := COALESCE(
    (fn_get_policy_json('network.devices.max_per_role', '{}'::jsonb, p_institution_id) ->> p_user_role)::INT,
    3
  );
...
-- Supabase's default privileges hand anon a direct EXECUTE on every new
-- function; REVOKE FROM PUBLIC alone does not remove it. Every function is
-- revoked from anon AND PUBLIC here, then granted only to the caller it serves:
...
-- ============================================================================
REVOKE EXECUTE ON FUNCTION public.fn_network_create_monthly_partitions() FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.fn_network_create_monthly_partitions() TO service_role;

REVOKE EXECUTE ON FUNCTION public.fn_network_cleanup_pending_requests() FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.fn_network_cleanup_pending_requests() TO service_role;

REVOKE EXECUTE ON FUNCTION public.fn_network_log_audit(UUID, UUID, TEXT, TEXT, TEXT, UUID, JSONB, TEXT, TEXT) FROM anon, PUBLIC;
GRANT  EXECUTE ON FUNCTION public.fn_network_log_audit(UUID, UUID, TEXT, TEXT, TEXT, UUID, JSONB, TEXT, TEXT) TO service_role;

REVOKE EXECUTE ON FUNCTION public.fn_network_record_failed_attempt(UUID, UUID, TEXT, TEXT) FROM anon, PUBLIC;
...
REVOKE EXECUTE ON FUNCTION public.fn_network_detect_teleport() FROM anon, PUBLIC;
...
-- Master tables: any signed-in person reads (the portal shows them); writes need
...
-- RADIUS servers: institution_id is nullable (the JICATE-shared row), so no
-- institution scoping — the permission alone decides.
...
CREATE POLICY "radius_servers_admin_all" ON public.network_radius_servers FOR ALL
  USING (is_super_admin() OR is_admin() OR user_has_permission('network.routers.manage'))
  WITH CHECK (is_super_admin() OR is_admin() OR user_has_permission('network.routers.manage'));
...
-- Devices: a person manages their own devices (name, remove); network.devices.view
-- sees the institution's.

ALTER TABLE public.network_devices ENABLE ROW LEVEL SECURITY;
CREATE POLICY "devices_self_all" ON public.network_devices FOR ALL
  USING (user_id = auth.uid())
...
-- Auth methods (6 system rows × every institution)
INSERT INTO public.network_auth_methods (institution_id, code, display_name, description, icon_name, display_order, is_system)
SELECT i.id, m.code, m.display_name, m.description, m.icon_name, m.display_order, true
  FROM institutions i
 CROSS JOIN (VALUES
    ('email_password', 'MyJKKN Email + Password', 'Sign in with your MyJKKN account', 'mail', 10),
    ('google_sso',     'Sign in with Google',     'Use your Google account linked to MyJKKN', 'google', 20),
    ('microsoft_sso',  'Sign in with Microsoft',  'Use your Microsoft 365 account linked to MyJKKN', 'microsoft', 30),
    ('rfid',           'RFID Card Tap',           'Tap your JKKN ID card on the reader', 'credit-card', 40),
    ('biometric',      'Biometric (Mobile)',      'Face/fingerprint via mobile device', 'fingerprint', 50),
    ('guest_token',    'Guest Token',             'One-time code from reception desk', 'key', 100)
  ) AS m(code, display_name, description, icon_name, display_order)
...
SELECT s.policy_key, 'global', NULL, s.value::jsonb, s.description, s.data_type, true
  FROM (VALUES
    ...
    ('network.session.length_hours.by_role',
       '{"learner":8,"senior_learner":24,"admin":4,"warden":0,"security":0}',
       'Wi-Fi: hours before each role must sign in again; 0 = persistent session',                                             'object'),
    ('network.devices.max_per_role',
       '{"learner":3,"senior_learner":10,"admin":15}',
       'Wi-Fi: registered devices allowed per person, by role; roles not listed get 3',                                        'object'),
...
COMMENT ON FUNCTION public.fn_network_create_monthly_partitions IS 'Create rolling 3-month-ahead partitions (born locked: anon revoked, RLS on). Schedule via pg_cron monthly.';
...
COMMENT ON FUNCTION public.fn_network_register_device           IS 'Register or update a device for a person. Enforces the per-role max-devices policy. Returns NULL if cap exceeded. Self, network.sessions.manage, super/admin, or service_role.';
