...
// form uses, so the GB picker matches it exactly.
...
export default function NewGoverningBodyMeetingPage() {
  return (
    <PermissionGuard module='academic.bos-governing-body' action='manage'>
      <NewGoverningBodyMeeting />
    </PermissionGuard>
  );
}

function NewGoverningBodyMeeting() {
  ...
  // Step 1 result: the GB body + its (post-snapshot) roster.
  ...
  const bodyCompositionId = body?.composition.id;
  const { data: liveMembers } = useBosMembersByComposition(bodyCompositionId);
  const members = liveMembers ?? body?.members ?? [];
  ...
  const handlePrepare = async () => {
    ...
    setPreparing(true);
    try {
      const res = await fetch('/api/bos/governing-body', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ institutions_id: institutionsId, academic_year: academicYear.trim() }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error ?? 'Failed to prepare governing body');
      setBody(json);
      ...
      // Suggest the next meeting number for this body (per-composition
      // sequence). Best-effort — falls back to 1 if the count query fails.
      try {
        const mres = await fetch(
          `/api/bos/meetings?compositionId=${json.composition.id}&meetingType=governing_body&limit=100`,
        );
      ...
      toast.success('Governing body ready — Principal added as Member Secretary. Add the Chairman and other members below.');
    } catch (e) {
  ...
  const handleSchedule = async () => {
    if (!body || !institutionsId) return;
    try {
      // board_id is null for a Governing Body meeting — the shared meetings
      // route accepts that for meeting_type 'governing_body'.
      const payload = {
        institutions_id: institutionsId,
        composition_id: body.composition.id,
        board_id: null,
        meeting_type: 'governing_body',
        meeting_number: meetingNumber === '' ? undefined : Number(meetingNumber),
        academic_year: academicYear.trim(),
        meeting_title: title.trim() || `Governing Body Meeting ${academicYear.trim()}`,
      ...
      toast.success('Governing Body meeting scheduled.');
  ...
  return (
    <div className='w-full max-w-5xl mx-auto space-y-6 overflow-x-hidden'>
      <PageHeader
        title='New Governing Body Meeting'
        description='Prepare the governing body roster, then schedule the meeting.'
      >
        <Button variant='outline' size='sm' onClick={() => router.push('/bos/governing-body')}>
          <ArrowLeft className='mr-2 h-4 w-4' />
          Back
        </Button>
      </PageHeader>

      {/* ── Step 1: Prepare the governing body ───────────────────────────── */}
      <Card>
        <CardHeader>
          <CardTitle className='text-base'>1 · Governing Body & Academic Year</CardTitle>
        </CardHeader>
        <CardContent className='space-y-4'>
          ...
          <Button onClick={handlePrepare} disabled={preparing || !institutionsId}>
            ...
            {body ? 'Refresh' : 'Prepare Governing Body'}
          </Button>
        </CardContent>
      </Card>

      {/* ── Governing body roster (add member like a composition) ────────── */}
      {body && (
        <Card>
          ...
          <CardContent>
            {members.length === 0 ? (
              <p className='text-sm text-muted-foreground'>
                No members yet. The Principal is added as Member Secretary automatically —
                use “Add Member” to include the Chairman, Deans, HoDs, or external experts.
              </p>
            ) : (
                  ...
                  <li key={m.id} className='flex items-center justify-between gap-3 py-2'>
                    ...
                    <div className='flex shrink-0 items-center gap-2'>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          removeMember.mutate(
                            { id: m.id, compositionId: body.composition.id },
      ...
      {/* AddMemberDialog — the exact composition-page component. Member Type
          ...
          /api/bos/members (council-branched to authorize the principal). */}
      {body && institutionsId && (
        <GbAddMemberDialog
          open={showAddMember}
          onClose={() => setShowAddMember(false)}
          compositionId={body.composition.id}
          ...
          <CardContent className='space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              <Input
                ...
                placeholder={`Governing Body Meeting ${academicYear.trim()}`}
              />
            </div>
            <div className='grid gap-4 sm:grid-cols-2'>
              <div className='space-y-2'>
            ...
            <div className='space-y-2'>
              <Label>Venue</Label>
              <Input value={venue} onChange={(e) => setVenue(e.target.value)} placeholder='Board Room' />
            </div>
            <div className='space-y-2'>
              <Label>Agenda</Label>
              <Textarea
