...
// ============================================
// Created: 2025-02-10
// Purpose: Bulk approve multiple student profile change requests
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify user has an allowed role — only super_admin and hod can bulk approve
    const allowedRoles = ['super_admin', 'hod'];
    const hasAllowedRole = allowedRoles.some((r) => effectiveRoles.has(r));
    if (!hasAllowedRole) {
      return NextResponse.json(
        { message: 'Insufficient permissions. Only Super Admin and HOD can perform bulk approvals.' },
        { status: 403 }
      );
    }
    ...
    // 6. Bulk approve
    const result = await LearnerProfileChangeService.bulkApproveRequests(
      request_ids,
      { review_comments },
      user.id
    );
    ...
    console.error('[api/learners/change-requests/batch/approve] Error:', error);
    return NextResponse.json(
      {
        success: false,
        message: error.message || 'Failed to bulk approve requests',
      },
      { status: 500 }
    );
  }
