...
import type { JkknSemesterUpstreamResponse } from '@/types/jkkn-api/semesters';
...
/**
 * GET /api/jkkn/semesters
 *
 * Secure server-side proxy for the JKKN semesters endpoint.
 * JKKN_API_KEY is a server-only env var — never sent to the browser.
 * Accepts ?page, ?limit, ?search, ?institution_id, ?program_id, ?isActive.
 *
...
export async function GET(request: NextRequest) {
  ...
  try {
    const res = await fetch(
      `${JKKN_API_BASE_URL}/api-management/organizations/semesters?${upstreamParams}`,
      {
        headers: {
