...
const APPROVED_REDEFINITIONS: Record<string, { fns: string[]; why: string }> = {
  ...
  '20261212110000': {
    ...
    why: "WALL 1b gains two keys the role-write sweep found after the audit and course-events modules landed: audit.external_auditor.manage (authorises fn_extend_institution_access and revoke_all_user_institution_access, which write user_institution_access) and courses.applications.decide (authorises fn_course_approve_application, which mints the participant's role). Body is the spine's text verbatim — verified equal to production via pg_get_functiondef on 2026-09-14 — plus exactly the two WHEN … THEN true lines. Director yes 2026-09-14 09:2x.",
  },
  '20260927020000': {
