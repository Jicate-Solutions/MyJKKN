...
-- ================================================================================
-- 2026-04-27 — notifications RLS lockdown (Bug C, security incident)
-- ================================================================================
...
-- only "is logged in".
--
-- Fix: drop both broken policies, install per-user (singular + array shapes),
-- super-admin governance view, and admin-only writes.
-- ================================================================================
...
-- Per-user SELECT — honors both targeting shapes ({user_id: uuid} singular,
-- {user_ids: [uuid]} array) plus optional {broadcast: true} flag for
-- system-wide notifications.
CREATE POLICY notifications_select_own ON notifications FOR SELECT USING (
  auth.uid() IS NOT NULL AND (
    (targeting->>'user_id')::uuid = auth.uid()
    OR (targeting->'user_ids' ? auth.uid()::text)
    OR targeting->>'broadcast' = 'true'
  )
);
