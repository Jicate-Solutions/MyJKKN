...
// Academic Council "Add Member" dialog.
//
// Unlike a Board of Studies (where member_type conflates position AND source),
// the Academic Council separates the two:
...
// so a "Member" can be an internal HoD OR an external Governing-Body / University
...
type AcPosition = Extract<BosMemberType, 'internal_member' | 'member_secretary'>;
type AcSource = 'staff' | 'expert';

interface AcAddMemberDialogProps {
  open: boolean;
  onClose: () => void;
...
export function AcAddMemberDialog({
  open,
  onClose,
  compositionId,
  institutionsId,
  existingMembers = [],
}: AcAddMemberDialogProps) {
  const addMember = useAddBosMember();

  const [position, setPosition] = useState<AcPosition>('internal_member');
  const [source, setSource] = useState<AcSource>('staff');
  ...
  // People already on the council — hidden from the pickers.
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addMember.mutate(
      ...
      },
      {
        ...
        onError: (err) => {
          logger.error('academic/bos', 'Failed to add council member', err);
      ...
      <DialogContent className='max-w-md max-h-[85vh] overflow-y-auto'>
        ...
        <form onSubmit={handleSubmit} className='space-y-4'>
          ...
          <div className='space-y-2'>
            ...
            <Select value={position} onValueChange={(v) => setPosition(v as AcPosition)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
            ...
            <p className='text-xs text-muted-foreground'>
              The Chairman is the Principal and is added automatically.
            </p>
          </div>
          ...
          <div className='space-y-2'>
            ...
            <Select value={source} onValueChange={(v) => setSource(v as AcSource)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
