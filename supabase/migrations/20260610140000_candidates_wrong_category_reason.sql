-- 20260610140000_candidates_wrong_category_reason.sql
-- Candidates preview: precise exclusion reason for the category-vs-reservation conflict.
-- Real case 2026-06-10: BDS 2-Year are pinned to Girls Hostel C rooms 30-34 (all Deluxe),
-- but their fee band resolves Classic Room — both conditions can never intersect, and the
-- generic message ("reserved rooms are in another block") misdiagnosed it. New flag
-- detects "rooms they may physically occupy here exist, but none is in their eligible
-- room category" and reports it explicitly so operators fix the rule or the fee band.
-- Rebuilt from 20260610130000 (verified live); only the flag + message branch added.
...
AS $function$
  ...
  base AS (
    SELECT
      ...
      -- Diagnostic: rooms they may PHYSICALLY occupy here exist, but none is in their
      -- eligible room category (reservation rooms vs Category-Eligibility band conflict).
      EXISTS (
        SELECT 1 FROM hostel_rooms rm
        WHERE rm.block_id=p_block_id AND rm.room_purpose='student'
