-- ============================================================================
-- 20260813100011 — Allow server-side callers into the school fee resolvers
-- ============================================================================
-- FOUND DURING VERIFICATION of 20260813100010: both RPCs are SECURITY DEFINER
-- and authorize on auth.uid(). A Supabase service-key call (API route, cron,
-- the parent-portal proxy) has NO auth.uid(), so every one of them was denied
-- with 42501 — including callers that are more privileged than any user.
--
-- This adds NO exposure. Anything holding the service key can already read
-- school_fee_plans, the concession tables and learners_profiles directly,
-- because the service role bypasses RLS. All the old behaviour achieved was
-- blocking legitimate server-side use.
--
-- The predicate mirrors fn_late_charge_accrue (20260815010000), which solved
-- exactly this for the cron-invoked late-charge engine:
--     COALESCE(auth.role(), '') = 'service_role'   -- Supabase service key
--     OR session_user <> 'authenticator'            -- direct SQL / psql
--
-- Both function bodies are otherwise BYTE-IDENTICAL to 20260813100010 —
-- CREATE OR REPLACE FUNCTION requires the whole body, so only the four-line
-- authorization preamble differs. Nothing in the resolution or concession
-- maths changes.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.school_fee_caller_is_privileged()
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.school_fee_caller_is_privileged() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.school_fee_caller_is_privileged() TO authenticated, service_role;

COMMENT ON FUNCTION public.school_fee_caller_is_privileged() IS
  'True for super admins, admins, Supabase service-key callers and direct SQL sessions. Used by the school fee RPCs, which are SECURITY DEFINER and therefore cannot rely on RLS. Adds no exposure: the service role already bypasses RLS on the underlying tables.';
...
DECLARE
    v_allowed boolean;
BEGIN
    SELECT public.school_fee_caller_is_privileged()
        OR (
             public.user_has_permission('school_fees.read')
...
GRANT EXECUTE ON FUNCTION public.school_fee_resolve_for_learner(uuid) TO authenticated, service_role;

REVOKE ALL ON FUNCTION public.school_fee_resolve_preview_for_class(uuid, uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.school_fee_resolve_preview_for_class(uuid, uuid, uuid) TO authenticated, service_role;
