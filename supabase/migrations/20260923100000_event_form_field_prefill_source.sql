-- ============================================================================
-- event_registration_form_fields.prefill_source — "Prefill from profile"
-- ----------------------------------------------------------------------------
-- A custom registration-form field may name ONE profile attribute. When a
-- signed-in MyJKKN user opens the public form (/p/event/[id]/register), the
-- page fills that field from their profile / staff / learner record; a guest
-- types it as before. The catalog of allowed values lives in
-- lib/services/events/registration/form-prefill.ts and is mirrored in the
-- CHECK below.
--
-- The two RPCs that write fields are rewritten to carry the column:
--   • save_event_registration_form DELETEs and reinserts every field, so a
--     column it does not copy is wiped on the next unrelated edit.
--   • clone_event_registration_form copies a form for next month's run. It
--     did not copy media_url either (an image_display field lost its picture
--     on duplicate); both columns are carried now.
-- ============================================================================
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'event_registration_form_fields_prefill_source_check'
  ) THEN
    ALTER TABLE public.event_registration_form_fields
      ADD CONSTRAINT event_registration_form_fields_prefill_source_check
      CHECK (prefill_source IS NULL OR prefill_source IN (
        'full_name','email','phone','person_type','institution','department',
        'gender','date_of_birth','degree','program','roll_number','register_number',
        'staff_id','designation'
      ));
  END IF;
END $$;

COMMENT ON COLUMN public.event_registration_form_fields.prefill_source IS
  'Profile attribute a signed-in registrant''s answer is seeded from (editable). NULL = no prefill. Catalog: lib/services/events/registration/form-prefill.ts';

-- ── save RPC: carry prefill_source ─────────────────────────────────────────
...
-- ── clone RPC: carry media_url + prefill_source ────────────────────────────
