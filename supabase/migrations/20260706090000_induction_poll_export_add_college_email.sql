...
-- The poll Excel export returned only the personal student_email; coordinators
-- also need the institution-issued college_email (learners_profiles.college_email).
-- RETURNS TABLE shape changes, so CREATE OR REPLACE would fail with
-- "cannot change return type" — DROP first, then recreate + re-grant.

DROP FUNCTION IF EXISTS public.fn_induction_session_poll_export(uuid);

CREATE FUNCTION public.fn_induction_session_poll_export(p_poll_id uuid)
RETURNS TABLE (
  learner_id uuid,
