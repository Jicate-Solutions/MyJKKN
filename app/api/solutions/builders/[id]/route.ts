...
export const GET = withAuth(async (request, auth, context) => {
  ...
  const result = await BuildersService.getBuilderById(id)
  if (!result) return errorResponse('Builder not found', 404)
...
export const PATCH = withAuth(async (request, auth, context) => {
  ...
  const result = await BuildersService.updateBuilder(id, safeBody)
  return successApiResponse(result)
})

export const DELETE = withAuth(async (request, auth, context) => {
  const { id } = await context!.params!
  await BuildersService.deleteBuilder(id)
