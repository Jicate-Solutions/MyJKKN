...
- New: `pde_assess_read` (staff scoped by institution OR learner enrolled via `vac_enrollments` in a **published** case), `pde_assess_write` (creator+admin), `pde_questions_read` (mirrors parent assessment visibility), `pde_questions_write` (parent case's creator+admin), `pde_certs_admin_write`.
