...
import {
  ...
  useDeleteWorkload,
  useVerifyWorkload,
} from '@/hooks/hr/recruitment-need/use-workload';
...
export default function WorkloadPage() {
  ...
  const createMutation = useCreateWorkload();
  const updateMutation = useUpdateWorkload();
  const deleteMutation = useDeleteWorkload();
  const verifyMutation = useVerifyWorkload();
  ...
  async function handleSubmit() {
    if (editingId) {
      ...
      const insert: WorkloadInsert = {
        ...
        institution_id: '', // will be set by RLS or API context
  ...
  const stats = useMemo(() => {
    ...
    const avgContact =
      total > 0
        ? (workloads.reduce((sum, w) => sum + w.weekly_contact_hours, 0) / total).toFixed(1)
        : '-';
      ...
      <div className="mt-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Track weekly contact, admin, and research hours per faculty member.
            </p>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">
                Avg Contact Hrs/Week
              </CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-2xl font-bold">{stats.avgContact}</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              <div className="p-8 text-center text-muted-foreground">
                No workload records found. Click &quot;Add Workload&quot; to get started.
              </div>
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead className="text-right">Contact Hrs</TableHead>
                    <TableHead className="text-right">Admin Hrs</TableHead>
                    <TableHead className="text-right">Research Hrs</TableHead>
                  ...
                  {workloads.map((w) => (
                    <TableRow key={w.id}>
                      <TableCell className="font-mono text-xs">
                        {w.staff_id.slice(0, 8)}...
                      </TableCell>
                      <TableCell>{w.academic_year}</TableCell>
                      <TableCell>{w.semester}</TableCell>
                      <TableCell className="text-right font-medium">
                        {w.weekly_contact_hours}
                      </TableCell>
                      <TableCell className="text-right">
                        {w.weekly_admin_hours ?? '-'}
                      </TableCell>
                      <TableCell className="text-right">
                        {w.weekly_research_hours ?? '-'}
                      </TableCell>
                      <TableCell>
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {!w.verified_at && (
                            <Button
                              ...
                              title="Verify"
                              disabled={isMutating}
                              onClick={() => verifyMutation.mutate(w.id)}
                            >
                          ...
                          <Button
                            ...
                            disabled={isMutating}
                            onClick={() => {
                              if (confirm('Delete this workload record?')) {
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingId
                ? 'Update the workload hours for this faculty member.'
                : 'Enter workload data for a faculty member.'}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-4 py-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select
                  value={form.semester}
                  onValueChange={(v) => setForm({ ...form, semester: v as '1' | '2' })}
                >
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <Label htmlFor="contact_hours">Contact Hrs/Wk</Label>
                <Input
                  id="contact_hours"
                  type="number"
                  min={0}
                  step={0.5}
                  value={form.weekly_contact_hours}
                  onChange={(e) =>
                    setForm({ ...form, weekly_contact_hours: e.target.value })
                  }
                  placeholder="0"
                />
              </div>
              <div>
                <Label htmlFor="admin_hours">Admin Hrs/Wk</Label>
                <Input
                  id="admin_hours"
                  type="number"
                  min={0}
                  step={0.5}
                  value={form.weekly_admin_hours}
                  onChange={(e) =>
                    setForm({ ...form, weekly_admin_hours: e.target.value })
                  }
                  placeholder="0"
                />
              </div>
              <div>
                <Label htmlFor="research_hours">Research Hrs/Wk</Label>
                <Input
                  id="research_hours"
                  type="number"
                  min={0}
                  step={0.5}
                  value={form.weekly_research_hours}
                  onChange={(e) =>
                    setForm({ ...form, weekly_research_hours: e.target.value })
                  }
                  placeholder="0"
                />
              </div>
            ...
            <div>
              ...
              <Select
                value={form.source}
                onValueChange={(v) => setForm({ ...form, source: v as WorkloadSource })}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="timetable_import">Timetable Import</SelectItem>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={
                isMutating ||
                !form.weekly_contact_hours ||
                (!editingId && !form.staff_id)
              }
            >
