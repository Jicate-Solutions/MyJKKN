...
describe('adoption loop — resource-management features record their use', () => {
  ...
  it('the message route records a use on the session client, only when a message went out', () => {
    const src = read('app/api/resource-management/reservations/communicate/route.ts');
    expect(src).toMatch(
      /if\s*\(\s*sent\s*>\s*0\s*\)\s*\{\s*await recordFeatureUse\(\s*session\s*,\s*FEATURE_KEYS\.RESOURCES_MESSAGE_BOOKED_USERS\s*\)/,
    );
  });

  it('the tag route records a use on the session client, only when someone was tagged', () => {
    const src = read('app/api/resource-management/reservations/[id]/comment-mentions/route.ts');
    expect(src).toMatch(
      /if\s*\(\s*outcome\.tagged\.length\s*>\s*0\s*\)\s*\{\s*await recordFeatureUse\(\s*db\s*,\s*FEATURE_KEYS\.RESOURCES_TAG_COLLEAGUE\s*\)/,
    );
  });
});
